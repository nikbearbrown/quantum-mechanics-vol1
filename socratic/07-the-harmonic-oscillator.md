...
Pull a guitar string sideways by a small distance and release it. Here is a question before we go further: what is special about the restoring force? It is proportional to the displacement — Hooke's law — and from it follows simple harmonic motion. Now ask a more ambitious question. Take *any* smooth potential with a minimum and expand it in a Taylor series around that minimum. What survives at leading order? The first derivative vanishes — that is what a minimum means — and the constant term just shifts the energy zero. So what is left is a quadratic:

$$V(x) \approx \frac{1}{2}V''(x_0)\,(x-x_0)^2 = \frac{1}{2}m\omega^2(x-x_0)^2,$$

where $\omega = \sqrt{V''(x_0)/m}$ is set by the curvature. So here is the payoff of that little expansion: every stable equilibrium in every physical system, for sufficiently small displacements, is a harmonic oscillator. The guitar string, the diatomic molecule, the electromagnetic mode of a laser cavity, the vibration of a crystal lattice, the quantized field of the vacuum — all of them reduce to the same Hamiltonian:
...
You could attack this head-on: write $\hat{p} = -i\hbar\,\partial_x$, substitute into (7.1), and solve a second-order differential equation. That works, and produces Hermite polynomials, and is described below. But before committing to all that machinery, ask whether there is a way to get the entire spectrum without solving any differential equation at all. There is.
...
Here is the key question: what energy does the state $\hat{a}_+|n\rangle$ have? Compute it:

$$\hat{H}(\hat{a}_+|n\rangle) = (\hat{a}_+\hat{H} + [\hat{H},\hat{a}_+])|n\rangle = (E_n + \hbar\omega)\,\hat{a}_+|n\rangle.$$

So applying $\hat{a}_+$ to an eigenstate produces a new eigenstate with energy $\hbar\omega$ *higher*. Applying $\hat{a}_-$ produces one $\hbar\omega$ *lower*. The operators form a ladder: climb up one rung at a time, or descend one rung at a time, each rung exactly $\hbar\omega$ above the previous.

Now a worry. If you can keep applying $\hat{a}_-$, descending by $\hbar\omega$ each time, what stops you from reaching negative energies? The Hamiltonian (7.1) is a sum of squares of Hermitian operators, so $\langle\hat{H}\rangle \geq 0$ in every state — energy cannot go negative. But $\hat{a}_-$ subtracts $\hbar\omega$ indefinitely. Both cannot be true unless the descent terminates. The resolution: there exists a ground state $|0\rangle$ that the lowering operator kills:
...
Look again at $E_0 = \hbar\omega/2 \neq 0$. Is this a technicality — a quirk of the formalism — or does it describe something you could actually measure in a laboratory? Sit with that before reading on, because the honest answer is the more surprising one.

First, why is it nonzero at all? Classically, the minimum energy of a harmonic oscillator is zero — the particle at rest at the bottom of the well. Quantum mechanically, the uncertainty principle forbids this. A particle localized at $x = 0$ with $\sigma_x \to 0$ would require $\sigma_p \to \infty$ by the Kennard inequality $\sigma_x\sigma_p \geq \hbar/2$. The zero-point energy is the minimum kinetic energy that confinement demands. It is not optional.

And it is measurable. Liquid helium does not solidify at atmospheric pressure at any temperature, because the zero-point kinetic energy of helium atoms — confined to the inter-atom spacing — exceeds the interatomic binding energy. The atoms cannot settle into a lattice. Helium is a quantum liquid *because* of zero-point motion, not despite it. More directly still: the Casimir force between two conducting plates arises from the difference in zero-point energies of electromagnetic modes in the presence and absence of the plates. Lamoreaux measured it in 1997, matching the theoretical prediction to within five percent. So the answer to the question is clear. Zero-point energy is not a formalism artifact. It has an invoice, and you can read it in the lab.
...
You might be tempted to treat the prefactors $\sqrt{n+1}$ and $\sqrt{n}$ as cosmetic — but drop them and every expectation value you compute will be wrong. Where do they come from? $\hat{a}_+|n\rangle$ has norm $\sqrt{n+1}$ (from $\langle n|\hat{a}_-\hat{a}_+|n\rangle = n+1$), so dividing by the norm gives the unit-normalized $|n+1\rangle$.
...
The algebra gave us the spectrum without ever writing down a wave function. But the wave functions are there, waiting. Where do we find the ground state? Not by solving the full differential equation — by exploiting the condition $\hat{a}_-|0\rangle = 0$, which in position space reads:
...
A Gaussian. Now here is a question worth pausing on: this is the same minimum-uncertainty shape from Chapter 3, the free-particle wave packet — so will it spread in time the way that packet did? The answer is no, and the difference is crucial. The free-particle Gaussian spreads because it is not an eigenstate of the free Hamiltonian. The harmonic-oscillator ground state stays a Gaussian forever, because the potential provides a restoring force that exactly balances the spreading tendency.
...
Two things to notice, and both are worth predicting before reading them. First: how many nodes does $\psi_n$ have? Exactly $n$ — the ground state has none, the first excited state has one, the same counting rule as the infinite square well. Second, and less expected: what fraction of the ground-state probability density lies *outside* the classical turning points $x = \pm\sqrt{\hbar/m\omega}$? Roughly 16%. So tunneling into the classically forbidden region is not exotic behavior reserved for high energy — it is present in the very first eigenstate, a default feature of the quantum ground state.

---

## Eigenstates Do Not Oscillate

Here is a misconception that must be addressed before the simulation, and it is worth stating as a prediction first. A harmonic oscillator is, classically, an oscillator. So you naturally expect the quantum version to oscillate. Does it? Form the time-dependent eigenstate and find out:
...
Static. The phase rotates in the complex plane at frequency $E_n/\hbar$, but $|\Psi|^2$ — everything you can measure with a position detector — does not change at all. So the expectation was wrong: energy eigenstates are *stationary* states. If quantum mechanics contained only eigenstates, classical oscillation would never appear.

Then where does the oscillation hide? In superpositions. Take the equal mixture $|\Psi\rangle = (|0\rangle + |1\rangle)/\sqrt{2}$:
...
The two-state superposition sloshes, but it deforms as it goes. Is there a quantum state that oscillates like a clean classical particle — moving back and forth without changing shape? There is, and it is the eigenstate of the *lowering* operator. A **coherent state** $|\alpha\rangle$ satisfies:
...
Roy Glauber introduced coherent states in 1963 to describe the quantum state of laser light. An ideal laser produces field modes in coherent states. Here is a question whose answer is more than a coincidence: why are the photon-counting statistics of ideal laser light Poisson? Not because lasers are classical, but because Poisson is the photon-number distribution of a coherent state. The harmonic oscillator algebra is not an analogy for laser physics; it *is* laser physics. Glauber received the Nobel Prize in 2005 for this connection.
