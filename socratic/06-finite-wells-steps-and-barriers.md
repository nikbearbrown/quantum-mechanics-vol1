...
Here is a question worth sitting with before reading on. A nitrogen molecule binds its two atoms in a potential well roughly 9.8 eV deep. Thermal energy at room temperature is about 0.025 eV — smaller by a factor of nearly four hundred. So: is the molecule stable? What does your intuition say, and what does classical mechanics say?

In 1928 Friedrich Hund stared at exactly this and gave the obvious answer first — of course it is stable, the activation energy dwarfs any thermal kick. But then he noticed something that made the obvious answer incomplete. The wave function does not vanish at the edge of the classically allowed region. It decays into the forbidden zone and arrives at the other side with non-zero amplitude. So ask yourself: if the barrier is not actually a wall but a region of exponential suppression — and exponential suppression, however ferocious, is never exactly zero — what stops the molecule from falling apart without ever paying the activation energy? Nothing in principle. Here the barrier happens to be wide and deep enough that the dissociation rate is negligibly small. But the rules had changed, and that is the point.

Now push the same idea harder. Two years later, George Gamow faced a puzzle that had stumped nuclear physicists for a decade. A nucleus traps an alpha particle behind a Coulomb barrier roughly 30 MeV high, yet the escaping alpha carries only 4–8 MeV. Classically it is stuck permanently. So how does it get out? Gamow's answer was the same mathematics Hund had glimpsed: it leaks. And the leaking is so sensitive to barrier height and particle energy that a factor of two in energy produces twenty-four orders of magnitude of difference in half-life. The decay rate of uranium-238 differs from that of polonium-212 by a factor of $10^{21}$, and the tunneling formula accounts for it.
...
Begin with a prediction. Take the infinite square well from Chapter 5 and lower the walls to a finite depth. The potential is $V = -V_0$ for $|x| < L/2$ and $V = 0$ outside, with $V_0 > 0$. Bound states have $-V_0 < E < 0$. Before doing any algebra, guess: how many bound states does this well have — finitely many, or infinitely many as the infinite well had? Hold your answer.
...
Notice what just changed. In the infinite well, the walls forced $\psi$ to zero at $x = \pm L/2$, and quantization fell straight out of that. Here $\psi$ does *not* vanish there. So where does quantization come from now? It has to come from somewhere else — from demanding continuity of $\psi$ and $\psi'$ at both walls, and that matching is what selects the allowed energies.
...
Now return to your prediction and count the crossings. A quarter-circle of finite radius $z_0$ can cross only finitely many tangent branches — so the finite well has a **finite** number of bound states. The count is roughly $z_0/(\pi/2)$ rounded up, which is $N \approx (L/\pi\hbar)\sqrt{2mV_0}$ in physical units. Make the well shallower or narrower and levels disappear from the top; make it deeper or wider and new levels appear. But here is a subtler question: is there always at least one bound state, no matter how shallow the well? The graph answers it. The quarter-circle always intersects the first tangent branch at least once, however small $z_0$ — so yes, the finite square well always has a ground state.

What does the wave function look like outside the well? It decays as $e^{-\kappa|x|}$. The characteristic length $1/\kappa = \hbar/\sqrt{2m|E|}$ is the penetration depth. Ask which state leaks farther: the tightly-bound ground state with large $|E|$, or a loosely-bound state near $E = 0$? The large-$|E|$ state has a *small* penetration depth — its wave function hugs the well. The loosely-bound state has a *large* penetration depth, leaking far beyond the classical turning points, into space where the particle classically has no business being.
...
And what happens in the limit $V_0 \to \infty$? The penetration depth goes to zero and the energy levels recover the infinite-well values $n^2\pi^2\hbar^2/(2mL^2)$. The infinite well is just the limit of the finite well with infinitely impenetrable walls.

---

## The Potential Step: Partial Reflection from Nothing

Switch from bound states to scattering. The potential is a step: $V = 0$ for $x < 0$, $V = V_0$ for $x > 0$. A particle comes in from the left with $E > V_0$ — enough energy to clear the step easily. Classical question: how much of it gets reflected? The classical answer is none, obviously. A particle with energy to spare keeps going. Keep that answer in mind; the quantum mechanics is about to disagree with it.
...
Now look at $R$. It is *not zero*, even though the particle has more than enough energy to clear the step. So the classical answer was wrong. Why? The thing that matters is not whether the particle can clear the barrier energetically, but how sharply $k$ changes at the boundary. Any abrupt change in $k$ produces partial reflection — the same way an impedance mismatch in a transmission line reflects part of a signal. Reflection vanishes only when $k_0 = k_1$, which requires $V_0 = 0$. Even a step *downward* ($V_0 < 0$) partially reflects. The lesson is that the quantum particle is a wave, and waves reflect at discontinuities whether or not a classical particle would.
...
Total reflection — as you would expect. But here is a question that is not so obvious: is the wave function in region II zero? It is not. There is an evanescent tail penetrating the forbidden region with characteristic length $1/\kappa$. Probability density is present there — yet no net current flows. How do both hold at once? The probability sloshes in and out of the step without transmitting. This is not tunneling. The barrier has infinite extent, so the evanescent tail never reaches a far edge where it could launch a transmitted wave. That distinction — present amplitude, zero net current — is exactly what changes in the next section.

---

## The Rectangular Barrier: Tunneling

Now give the barrier a finite width. $V = V_0$ for $0 < x < L$ and $V = 0$ elsewhere. A particle with $E < V_0$ comes in from the left. What is your prediction? Classically the answer is unambiguous: total reflection, the particle does not have the energy. But we just saw that an evanescent tail penetrates the forbidden region — and now that region *ends*. So before computing: do you expect anything to come out the far side?
...
So the answer is yes: $T \neq 0$. Something does come out the far side, even with $E < V_0$. This formula contains everything — the dependence on barrier height $V_0$, width $L$, and energy $E$, all exact, no approximations. Let me unpack what it says.
...
**Above the barrier** ($E > V_0$): here is a place to test your intuition again. A particle with more than enough energy — does it always transmit perfectly? Set $\kappa = ik_2$ with $k_2 = \sqrt{2m(E-V_0)}/\hbar$, so $\sinh(i\theta) = i\sin\theta$. The formula becomes:

$$T_\text{exact} = \left[1 + \frac{V_0^2\sin^2(k_2 L)}{4E(E - V_0)}\right]^{-1}.$$

Now $T = 1$ whenever $\sin(k_2 L) = 0$, i.e., when $k_2 L = n\pi$. So the answer is: only sometimes. The barrier is perfectly transparent precisely when its width is an integer number of half-wavelengths at the energy inside the barrier — the wave fits the barrier, constructive interference makes it as though the barrier is not there. These are **resonances**, the quantum analogue of anti-reflection coatings in optics, or of a Fabry-Pérot cavity at resonance. And *between* resonances, $T < 1$ even with $E > V_0$. The particle has plenty of energy and still partially reflects.
...
You have probably met the pop-science version of tunneling: "The particle borrows energy from the vacuum — permitted by the time-energy uncertainty principle — crosses the barrier before the debt is called in, and pays it back on the other side." Before reading further, ask yourself whether that story can be right. Where, exactly, would the borrowed energy go, and what does the wave function inside the barrier actually look like?

The story is wrong, and it is worth being precise about why. The uncertainty relation $\Delta E\,\Delta t \geq \hbar/2$ does not license energy borrowing for a time $\Delta t$ — that is simply not what the relation means. Energy is conserved at every moment inside the barrier; the total energy is $E$ throughout — in region I, inside the barrier, and in region III. The wave function inside the barrier, $Ce^{\kappa x} + De^{-\kappa x}$, is a perfectly valid solution to the time-independent Schrödinger equation for a particle of energy $E$ in a region where $V = V_0 > E$. The function is real and decaying — not oscillatory — but it is the correct solution. There is no negative kinetic energy; there is no energy overdraft; there is no payment schedule.

So what *does* happen? Something simpler and stranger than the loan story: the wave equation requires non-zero amplitude in the classically forbidden region, and when the barrier is finite, that amplitude reaches the far side and launches a transmitted wave. The transmitted amplitude is small because the decaying exponential has attenuated across the width $L$. That is all. The particle does not borrow anything. It passes through because the mathematics of partial differential equations does not know what "classically forbidden" means.
