...
You have already stared at something without being told what it is. A blue filled curve, drifting. An orange curve inside it, oscillating, dipping below zero, apparently unconcerned with any rule about staying positive. You dragged a slider and the blue shape spread. Somewhere between recognition and confusion, you had the feeling that you were watching *something* — but I withheld the label. I am going to give it to you now, but not by handing you a definition and asking you to believe it. Instead, let us reason out what the blob is *forced* to be if the theory is to be consistent at all. You will trust the answer more for having cornered it.

Start with the simplest possible question, and predict before reading: what is the blob's *unit*?

Most people's first instinct is that the blob is a probability — a number between 0 and 1. Hold that thought and test it. If the particle moves along a line measured in nanometers, and the blob is a quantity plotted against that line, then the blob has units. The moment you ask "which units?", the probability reading collapses, because a probability is dimensionless and this thing is not. The blob is a probability *per nanometer*. A density. Like population per square kilometer, not a headcount. So can you read "the particle is at position $x$" from the blob's height at a single point? No more than you can read a city's population from a density map at a single address. To extract a real, dimensionless probability you have to integrate over a region.

That is the entire content of the Born rule. Given a wave function $\psi(x,t)$, the probability of finding the particle somewhere in the interval $[a,b]$ at time $t$ is:

$$P\bigl(\text{particle in }[a,b]\bigr) = \int_a^b |\psi(x,t)|^2\,dx.$$

Max Born introduced this in 1926, in a paper on quantum scattering. Here is a detail worth pausing on: his original draft wrote the probability as proportional to $\psi$ itself — *not* $|\psi|^2$. In a footnote added in proof, he corrected it. That footnote is one of the most consequential edits in the history of physics. Born received the Nobel Prize in 1954 for the statistical interpretation it introduced.
...
So can the density $|\psi|^2$ exceed 1? Predict, then check the unit. Yes — it is per unit length, not a dimensionless probability, so nothing stops it from reading 2 or 200 at a point. Confusing the density's *value* at a point with a probability is a unit error, and it is the single most common first mistake. Memorize the distinction now and you will dodge a whole class of wrong answers for the rest of the course.

---

Now the second question, and it is one most students never think to ask: why is $\psi$ *complex* at all?

Here is the reasonable objection. If what we measure is $|\psi|^2$, and $|\psi|^2$ is real and non-negative, why not just use a real-valued function and throw away the imaginary part? It seems like pure bookkeeping. Test that objection against the equation of motion and watch it fail.

The time-dependent Schrödinger equation is:

$$i\hbar \frac{\partial \psi}{\partial t} = \hat{H}\psi.$$

That $i$ on the left is structural, not cosmetic. Suppose $\psi$ were purely real. Then $\partial\psi/\partial t$ is real, and $i\hbar$ times something real is purely *imaginary*. So the left side would be imaginary while the right side is real (for real $\psi$ and real $V$). The equation would demand that a real quantity equal a purely imaginary one — impossible unless everything vanishes. So the dynamics themselves push $\psi$ into the complex plane. You cannot remove the imaginary part and still have a theory.

You saw this on the screen, even if you did not know it. The orange curve was Re$\,\psi$. The gray dashed curve was Im$\,\psi$. Both were non-trivial; the imaginary part led the real part by a quarter cycle. Here is a prediction to test your grasp: if you froze the wave function, set Im$\,\psi = 0$, and took one time step under the Schrödinger equation, what would happen? The imaginary part would rebuild itself immediately. The orange curve cannot live alone.
...
But what does the imaginary part actually *carry*? Let us extract it. Consider the Gaussian wave packet with central momentum $\hbar k_0$:
...
Both oscillate; both carry the information encoded in $k_0$. Now compute the density and predict what survives:

$$|\psi|^2 = A^2\,e^{-x^2/a^2}.$$

The oscillations vanish. The density is a smooth Gaussian with no trace of $k_0$ anywhere. So where did the momentum go — the direction of travel, the central velocity? It is invisible in $|\psi|^2$. It lives entirely in the phase, in the relationship between Re$\,\psi$ and Im$\,\psi$. Here is the consequence that should land: discard the imaginary part and you can no longer tell a wave packet moving right from one moving left. Both have the same $|\psi|^2$. Both produce identical position histograms in every experiment.

This is not a mathematical subtlety. It *is* the physical content of the imaginary part.

---

Now confront something the blob looked like it was doing but was not.

When the blue curve spread wider over time, the natural reading is that the particle is physically expanding — spreading out in space like a gas. Before you accept that reading, ask: when you actually *detect* the particle, where is it? It is at one definite place, every time. So what spread? Not the particle — the *probability distribution* of where that place might turn out to be. Here is the operational meaning. Run the same experiment — same initial conditions, same Hamiltonian, same $\psi(x,0)$ — ten thousand times. Measure position each time. The histogram of results matches $|\psi|^2$. The spread of the histogram is the spread of the blob.

So $\psi$ is not the particle. $\psi$ is the instruction set for computing histograms.

Read the spreading blue blob in that light: it was telling you that if you had run this experiment ten thousand times and measured position at this late moment, the measurements would scatter across this wide distribution. A statement about many repetitions — not about the fate of one particle.
...
If $|\psi|^2$ is a probability density and the particle must be found *somewhere*, what must the total integral equal? It must equal one:

$$\int_{-\infty}^{\infty} |\psi(x,t)|^2\,dx = 1.$$

A wave function satisfying this is normalized. In practice, wave functions are often written without a normalization constant. To fix that: compute the integral of $|\tilde{\psi}|^2$, take its square root, divide. Done.

Now the more interesting question — and one where "obviously yes" is too quick. Normalize $\psi$ at $t = 0$ and let it evolve under the Schrödinger equation. Does $\int|\psi|^2\,dx$ *stay* equal to 1 at later times? It *has* to — otherwise the probability of finding the particle anywhere would drift away from 1, which is incoherent. But having to be true and being *provably* true are different things. So prove it.

Differentiate $\int|\psi|^2\,dx$ with respect to time. Use the Schrödinger equation to replace $\partial_t\psi$ and $\partial_t\psi^*$. Watch the potential-energy contributions: they appear with opposite signs and cancel exactly. Only the kinetic terms survive, and they combine into a perfect derivative in $x$:
...
Do you recognize the form? This is the continuity equation — the same equation that governs the flow of any conserved quantity: mass, charge, energy. Probability flows like a fluid with current $J$. Integrate over all $x$: the boundary terms at $\pm\infty$ vanish for any normalizable $\psi$, so $\frac{d}{dt}\int|\psi|^2\,dx = 0$. Normalization is permanent.

And the derivation pays a practical dividend. In a numerical simulation, $\int|\psi|^2\,dx$ becomes a diagnostic. If it drifts from 1, something is wrong — either the time-stepping scheme is non-unitary (explicit Euler does this) or probability is leaking at the grid boundaries. The normalization indicator is a bug detector. The conservation law you just proved is the standard against which the simulation is judged.

---

Born's rule gives you the probability distribution for position measurements. So what is the *average* position across many experiments? It is the centroid of that distribution:

$$\langle x\rangle = \int_{-\infty}^{\infty} x\,|\psi(x,t)|^2\,dx.$$

Each possible position weighted by how probable it is — the mean of the distribution $|\psi|^2$, in the ordinary statistical sense.

Now momentum, which is trickier — and here is the trap. You might guess $\langle p\rangle = \int p\,|\psi(x)|^2\,dx$ by analogy. Try it and notice it makes no sense: in position space, momentum is not a function of $x$ at all. What is the right route? The Fourier transform. Define $\phi(p,t)$ as the Fourier transform of $\psi(x,t)$. Then $|\phi(p,t)|^2$ is the probability density for momentum measurements, and $\langle p\rangle = \int p\,|\phi|^2\,dp$. Translate this back to position space via Parseval's theorem, and multiplication by $p$ turns into differentiation. The momentum operator in position space is:

$$\hat{p} = -i\hbar\frac{\partial}{\partial x}.$$

Is the sign a free convention? Test it. For a wave packet moving right with $k_0 > 0$, this formula gives $\langle p\rangle = \hbar k_0 > 0$ — correct. Write $\hat{p} = +i\hbar\,\partial_x$ by mistake and you flip every direction. The simulation catches this immediately: a wave packet sent rightward registers negative average momentum. The sign is physics, not convention.
...
Now measure both spreads at once. Define the widths of the position and momentum distributions in the usual statistical way:

$$\sigma_x = \sqrt{\langle x^2\rangle - \langle x\rangle^2}, \qquad \sigma_p = \sqrt{\langle p^2\rangle - \langle p\rangle^2}.$$

Here is the **Kennard inequality**, proved by E. H. Kennard in 1927:

$$\sigma_x\,\sigma_p \geq \frac{\hbar}{2}.$$

Before interpreting it, predict what it is *about*. Almost everyone reads it as a statement about measurement: "measure position precisely and you disturb momentum." Hold that reading; we are going to break it. Here is the cleanest experiment the inequality actually describes. Prepare ten thousand identical copies of the same state $\psi$. Measure position on five thousand of them — record $\sigma_x$. Measure momentum on the *other* five thousand — record $\sigma_p$. No single particle is measured twice. No measurement disturbs any other. And yet the product $\sigma_x\sigma_p$ still cannot be smaller than $\hbar/2$.

So if it is not measurement disturbance, what is it? Fourier analysis. A function narrow in $x$ must be broad in its Fourier transform, and vice versa. The Kennard inequality is the precise, rigorous quantification of that unavoidable relationship between a function and its transform — with $\hbar/2$ giving the exact bound, not an order-of-magnitude estimate.

There *is* a separate Heisenberg measurement-disturbance relation — the one that genuinely concerns measuring position and then momentum on the same particle. It has a different form, a different bound, different physical content. Ozawa formalized it in 2003; Erhart et al. tested it experimentally in 2012. The Kennard inequality and the measurement-disturbance relation are different animals. Conflating them is the most persistent conceptual error in introductory quantum mechanics — which is exactly why the "measurement" reading you may have predicted is worth catching now.

Which state, if any, achieves the absolute minimum $\sigma_x\sigma_p = \hbar/2$? The Gaussian. For
...
Exactly $\hbar/2$. Every other normalizable wave function gives a larger product. That is why the Gaussian appears everywhere in quantum mechanics: it is the tightest possible state, balancing position and momentum uncertainty as evenly as the physics permits.

Look at the trade-off directly in the formulas. $\sigma_x \propto a$; $\sigma_p \propto 1/a$. Narrow the packet in space — smaller $a$ — and the momentum spread widens proportionally. The product is locked. Is this a limitation of your instruments? No. It follows from the mathematics of Fourier transforms; you cannot engineer around it.
...
Let us make the Born rule concrete with a calculation before you try one yourself.
...
Why split at $x = 0$? Because the absolute value means $e^{-|x|/a} \neq e^{-x/a}$ for $x < 0$ — skip the split and you get the wrong integral. Evaluating:

$$2A^2\cdot\frac{a}{2} = A^2 a = 1 \implies A = \frac{1}{\sqrt{a}}.$$

Check units, as a habit: $x$ in nm, $a$ in nm, $A$ in nm$^{-1/2}$, so $|\psi|^2$ in nm$^{-1}$. Correct for a one-dimensional probability density.

Now find the probability of catching the particle in $[-a, a]$. Predict first — more or less than half?

$$P = \frac{1}{a}\int_{-a}^{a}e^{-2|x|/a}\,dx = \frac{2}{a}\int_0^{a}e^{-2x/a}\,dx = 1 - e^{-2} \approx 0.865.$$

About 86.5%. The interval $[-a,a]$ straddles the peak of the distribution, so a large value is plausible; the remaining ~14% lives in the tails $|x| > a$.

One more thing to notice — and it is the unit trap returning. At $x = 0$, $|\psi(0)|^2 = 1/a$. For $a = 0.5$ nm this equals 2 nm$^{-1}$ — a number greater than 1. Is something broken? Nothing. The density is measured in nm$^{-1}$ and is not required to be $\leq 1$. Expecting $|\psi|^2 \leq 1$ everywhere is the same unit confusion as expecting a population-density map to never exceed 1 person per square kilometer.

---

This wave function $e^{-|x|/a}$ is real-valued, so Im$\,\psi = 0$ everywhere. What does that force the probability current $J = (\hbar/m)\,\mathrm{Im}(\psi^*\,\partial_x\psi)$ to be? Zero, everywhere. A real wave function has zero current — it cannot describe motion in a definite direction. So how do you set the particle moving with central momentum $p_0$? Multiply by $e^{ip_0 x/\hbar}$. The density $|\psi|^2$ is unchanged, but now $\langle p\rangle = p_0$. The density stays the same; the current changes; the physics changes. All the directionality lives in the phase.
