...
This chapter builds a configurable one-dimensional Schrödinger solver. Given any potential $V(x)$ specified on a grid, it finds the bound-state energies and eigenfunctions, or time-evolves an initial wave packet. The result is a machine that makes the Schrödinger equation computable for any one-dimensional problem.

Building the machine is half the work. The other half is verifying it. Visually plausible output is not correct physics. The discipline of running the solver on a problem with a known answer and demanding numerical agreement is what separates physical simulation from numerical theater, and this chapter teaches that discipline as explicitly as it teaches the algorithms.

The completed sandbox reproduces every bound-state spectrum in this book and is verified against the one benchmark that checks everything at once: the infinite square well, with energies $E_n = n^2\pi^2\hbar^2/2mL^2$, no fitting parameters.
...
The sandbox has two operational modes.

**Eigensolver mode.** Given a potential $V(x)$ on a spatial grid, find the discrete bound-state energies $E_n$ and eigenfunctions $\psi_n(x)$. The output is a list of energy levels and wave-function shapes — the stationary states, whose probability distribution does not change in time. Every analytic result in Chapters 4 through 10 is a special case.

**Time-evolution mode.** Given an initial wave function $\Psi(x, 0)$ and a potential $V(x)$, propagate $\Psi(x,t)$ by solving the time-dependent Schrödinger equation $i\hbar\,\partial_t\Psi = \hat{H}\Psi$. The output is an animation of the probability density drifting, spreading, reflecting, or tunneling.

Both modes share the same infrastructure: $N$ uniformly spaced grid points $x_j = x_\text{min} + j\,h$ with $h = (x_\text{max} - x_\text{min})/(N-1)$, a complex array of length $N$ for the wave function, and a real array of length $N$ for the potential. Everything that follows operates on those arrays.
...
On the grid, the second derivative is the central-difference stencil:

$$\psi''(x_j) \approx \frac{\psi_{j+1} - 2\psi_j + \psi_{j-1}}{h^2}.$$

Applying this at every interior point gives a matrix equation
...
Write the kinetic hopping coefficient as $t_k = \hbar^2/(2mh^2)$; then the diagonal is $2t_k + V_j$ and the off-diagonals are $-t_k$. With $V_j = 0$ everywhere the matrix is pure kinetic energy — the coupling between adjacent grid points that lets the wave function propagate.

Hard-wall boundary conditions ($\psi_0 = \psi_{N-1} = 0$) are enforced by building only the $(N-2)\times(N-2)$ interior submatrix; the boundary rows and columns are omitted.

Diagonalize to get $N-2$ eigenvalues and eigenvectors. The eigenvalues are the energies; the eigenvectors, normalized to $\sum_j|\psi_j|^2 \cdot h = 1$, are the discretized wave functions. The central-difference error in the $n$-th eigenvalue scales as $(nh/L)^2$; for $N = 500$ and the ground state it is below $10^{-6}$ fractionally — more than sufficient here.
...
The debugging number to remember: the diagonal is $\hbar^2/(mh^2) = 2t_k$, not $t_k$, and the off-diagonal is $-t_k = -\hbar^2/(2mh^2)$. Confuse the two and the ground-state energy is off by a factor of two.

---

## Two Paths to Eigenvalues

The matrix eigensolver returns all eigenvalues at once and is the right choice for the full spectrum, but it requires a linear algebra library. There is an arithmetic-only alternative.

**The Numerov shooting method** uses a more accurate $\psi''$ approximation to reach $O(h^6)$ error — four orders better than central difference — with only a three-point recursion. Rewrite the TISE as $\psi'' = f(x)\psi$ where $f_j = (2m/\hbar^2)(V_j - E)$. The Numerov recursion propagates one step at a time:
...
The shooting strategy: guess an energy $E$, integrate from the left with $\psi_0 = 0$, $\psi_1 = \epsilon$, and from the right with $\psi_{N-1} = 0$, $\psi_{N-2} = \epsilon$. At the midpoint, an eigenvalue exists exactly when the left and right logarithmic derivatives agree — equivalently, when a Wronskian determinant changes sign. Sweep $E$, locate the sign changes, and bisect to arbitrary precision.

The trade-off: Numerov finds eigenvalues one at a time, needs careful bracketing, and is slightly more code than calling `math.eigs()`. For three to five eigenstates it is faster to implement and more instructive — you integrate the equation directly rather than calling a black box. For twenty or more states, use the library.

For this capstone, start with Numerov. For all eigenvalues at once, add math.js (from CDN) and call `math.eigs()` on the tridiagonal matrix. Both validate against the same benchmark.

---

## Time Evolution: Why the Stepper Choice Is Not Optional

The time-dependent Schrödinger equation is $i\hbar\,\partial_t\Psi = \hat{H}\Psi$, with exact solution $\Psi(t) = e^{-i\hat{H}t/\hbar}\Psi(0)$ for a time-independent Hamiltonian. The operator $e^{-i\hat{H}t/\hbar}$ is unitary: it preserves the norm exactly, for all time. Any non-unitary scheme is fundamentally wrong, because it either creates or destroys probability.

**Why explicit Euler is forbidden.** The explicit Euler update is $\Psi^{n+1} = \Psi^n - (i\Delta t/\hbar)\mathbf{H}\Psi^n$, with update matrix $\mathbf{I} - (i\Delta t/\hbar)\mathbf{H}$ whose eigenvalues are $1 - i\Delta t E_k/\hbar$. Each has modulus $\sqrt{1 + (\Delta t E_k/\hbar)^2} > 1$, so every energy component grows exponentially each step. The normalization indicator climbs above 1 within fifty steps and the simulation diverges. The sandbox's corner indicator catches this immediately — if it reads above 1.001 after ten steps, the stepper is wrong. Explicit Euler is unconditionally unstable for the Schrödinger equation and must never be used.
...
This is the Cayley approximation to the exact propagator. Since $\mathbf{H}$ is Hermitian, the left and right matrices are complex conjugates, so the update is exactly unitary — normalization is preserved every step, for any $\Delta t$. The scheme is second-order in time and space, unconditionally stable, and natural for hard-wall boundaries because the tridiagonal structure of $\mathbf{H}$ carries straight into the linear system. Each step solves a tridiagonal system, which the Thomas algorithm does in $O(N)$.

### Split-Step Fourier

For free-space or slowly varying potentials on a large periodic domain, the split-step Fourier method is faster and spectrally accurate in space. The Hamiltonian splits as $\hat{H} = \hat{T} + \hat{V}$. Since $\hat{T}$ and $\hat{V}$ do not commute, $e^{-i\hat{H}\Delta t/\hbar}$ cannot be split exactly; the Trotter-Suzuki decomposition gives a second-order approximation:

$$e^{-i\hat{H}\Delta t/\hbar} \approx e^{-i\hat{V}\Delta t/2\hbar}\,e^{-i\hat{T}\Delta t/\hbar}\,e^{-i\hat{V}\Delta t/2\hbar}.$$

The per-step algorithm: multiply $\psi_j$ by $e^{-iV_j\Delta t/2\hbar}$; Fourier transform; multiply $\hat{\psi}_m$ by $e^{-i\hbar k_m^2\Delta t/2m}$; inverse Fourier transform; multiply again by $e^{-iV_j\Delta t/2\hbar}$. Each multiplication applies a phase of modulus exactly 1, so the method is exactly unitary — normalization is machine-precision exact.

One FFT $k$-grid detail breaks implementations that ignore it. After an FFT of length $N$, the output index $m$ does not equal the physical wave vector. The correct mapping is:

$$k_m = \frac{2\pi}{Nh}\times\begin{cases} m & m < N/2 \\ m - N & m \geq N/2 \end{cases}$$

The second half of the FFT output (indices $N/2$ through $N-1$) corresponds to *negative* wave vectors. Applying the kinetic phase with the raw index $m$ gives the wrong kinetic energy to every negative-momentum component. The simulation looks correct for the first few steps — the error lives in the high-frequency tails — then gradually corrupts the whole wave function. The fix is five lines, and the CLAUDE.md amendment below includes it.
...
Building the sandbox is the lesser half. The greater half is running the validation suite and confirming the simulation computes correct physics. Concretely:

**Units.** Every displayed quantity must carry correct units. This is not decoration — wrong units mean wrong physics. Commit to one system (SI with distances in nanometers and energies in electron-volts works well), label every axis, and use it consistently. Sanity check: an infinite square well of width $L = 1$ nm with an electron has ground-state energy $E_1 = \pi^2\hbar^2/2m_eL^2 \approx 0.376$ eV. A reading of 376 eV or 0.000376 eV is a units error.

**Normalization.** For every eigenstate returned, verify:
$$\sum_{j=0}^{N-1}|\psi_j|^2 \cdot h = 1.000 \pm 0.001.$$

Note the $h$ weighting. Some eigensolvers return Euclidean-unit-norm vectors ($\sum|\psi_j|^2 = 1$) rather than physics-norm ($\sum|\psi_j|^2 h = 1$); divide the returned eigenvector by $\sqrt{h}$ to convert. Forgetting $h$ makes the integral read $1/h$ instead of 1 — for $h = 0.004$ nm, a reading of 250. The corner indicator catches this immediately.

During time evolution the same indicator must stay within $\pm 0.001$ of $1.000$ every frame. Drift above 1 means the stepper creates probability; drift below means it destroys probability. Both are wrong.

**Orthogonality.** For any two eigenstates:
$$\left|\sum_j \psi_j^{(n)*}\psi_j^{(m)} \cdot h\right| < 10^{-8}, \quad m \neq n.$$

Violation means an eigensolver bug or unnormalized eigenvectors causing floating-point overflow to pollute the orthogonality.

**The infinite square well benchmark.** This is the primary test. Set $V(x) = 0$ for $x \in (0, L)$ with hard walls. Run the eigensolver with $L = 2$ nm, $m = m_e$, $N = 500$. The analytic spectrum is

$$E_n = \frac{n^2\pi^2\hbar^2}{2m_eL^2} \approx n^2 \times 0.094\ \text{eV.}$$

Report a comparison table — for $n = 1, 2, 3, 4, 5$, list analytic and numerical energies and fractional error. The error should be below $10^{-5}$ for $n = 1$ and below 1% for $n = 10$ at $N = 500$.

A dimensionless check bypasses units entirely: the ratios $E_n/E_1$ should be exactly $n^2$. Verify $E_2/E_1 = 4.000$, $E_3/E_1 = 9.000$, $E_4/E_1 = 16.000$. If these are correct to three decimals, the eigenvalue algorithm works regardless of the value of $\hbar$ in electron-volts — the ratios depend only on the numerics, not on physical constants.
...
**Harmonic oscillator.** Set $V(x) = \frac{1}{2}m\omega^2 x^2$ on a grid wide enough for the first ten states. The analytic spectrum is $E_n = \hbar\omega(n + \frac{1}{2})$ — equally spaced, ground state at $\hbar\omega/2$. Verify: spacing uniform to within 1%; ground-state wave function Gaussian; ground-state uncertainty product $\sigma_x\sigma_p = \hbar/2$ (the oscillator ground state saturates the Kennard bound). This check connects directly to Chapter 9 and closes a loop open since Chapter 1.

**Free-particle time evolution.** Set $V = 0$. Initialize a Gaussian packet with center $x_0$, width $a$, mean wavenumber $k_0$. The analytic evolution gives a centroid at $x_0 + (\hbar k_0/m)t$ and width $\sigma(t)^2 = a^2/2 + \hbar^2t^2/(2m^2a^2)$. Run and compare; both centroid and width should agree to within 1% over several spreading times.

**Energy conservation.** Under time evolution with any time-independent $V(x)$, compute $\langle\hat{H}\rangle = \sum_j\Psi_j^*(t)(\mathbf{H}\vec{\Psi})_j \cdot h$ each frame. It must not drift by more than 0.1% over the window. Growth means the stepper is not unitary.

---

## What Each Previous Chapter Built

The sandbox does no new physics. Every feature reuses a tool you already have.

The Born rule from Chapter 3 is why you display $|\Psi|^2$ rather than $\Psi$, and why the normalization integral must equal 1. The TISE from Chapter 4 is what the matrix eigensolver solves. The argument that quantization falls out of boundary conditions — made for the infinite well in Chapter 5 — is reproduced automatically by the matrix formulation: Dirichlet conditions enforce $\psi_0 = \psi_{N-1} = 0$, and the only solutions are the discrete eigenvectors. The harmonic oscillator spectrum of Chapter 7 is recovered by setting $V_j = \frac{1}{2}m\omega^2 x_j^2$. The Gaussian wave-packet dynamics of Chapter 8 — group velocity, spreading, the $1/\sigma_0^2$ rate — run in the time-evolution panel. The operators and expectation values of Chapter 9 give $\sigma_x$, $\sigma_p$, and $\langle\hat{H}\rangle$ from the numerical eigenstates. The tunneling of Chapter 6 appears when you evolve a packet against a finite barrier and watch the transmitted fraction.

The synthesis is the point. The sandbox is a single machine that runs everything you already know, then verifies it agrees with what you already know.
...
Each failure below has a specific symptom in the normalization indicator or the validation table. Read the indicator before debugging anything else.

**$h$ vs. $h^2$ in the kinetic coefficient.** Writing `h` instead of `h*h` in the denominator of $t_k$ makes the kinetic energy wrong by a factor of $1/h$, so the ground-state energy scales as $1/N$ instead of $1/N^2$. The ratio $E_2/E_1$ is still 4 (the error is uniform across modes), but $E_1$ itself is wrong. The validation table catches it in the first line.

**Unnormalized eigenvectors.** math.js returns Euclidean-normalized eigenvectors, $\sum_j|\psi_j|^2 = 1$; the physics normalization is $\sum_j|\psi_j|^2 h = 1$. For $h = 0.004$ nm the Euclidean vector has physics-norm $1/h = 250$. Multiply by $1/\sqrt{h}$ to correct. Symptom: indicator reads $250$ or $0.004$ instead of $1$.

**Wrong FFT $k$-ordering.** The second half of the FFT output wraps to negative wave vectors. Applying the kinetic phase with the raw index $m$ gives the wrong energy to every negative-momentum component. Symptom: time evolution is correct for the first ten steps, then grows a grid-scale oscillation. Fix: use the $k_m$ formula above.

**Explicit Euler.** Normalization climbs above 1 within fifty steps; the indicator catches it at step ten. Fix: use Crank-Nicolson or split-step. Explicit Euler is never acceptable for the Schrödinger equation.

**Grid too narrow.** For the harmonic oscillator, a grid that does not reach $\pm 5 x_0$ (where $x_0 = \sqrt{\hbar/m\omega}$) truncates the wave functions at the boundary and the eigenvalues come out wrong. Symptom: eigenvalues slightly too high, wave functions visibly hitting the edge. Fix: widen the grid.

**Spurious reflections in time evolution.** A packet that reaches the grid edge reflects, creating interference unrelated to the physics. Fix: make the grid five to ten packet-widths wider than necessary so the packet never reaches the boundary in the window. Alternatively add a complex absorbing potential $-i\gamma(|x| - x_\text{abs})^2$ near both edges — but then the normalization indicator will decrease over time, which is correct physics (probability flowing out of the domain), not a bug.

---

## What Comes After

The sandbox is one-dimensional and non-relativistic. What it cannot handle points to what comes next.

For periodic potentials — an array of wells, a crystal lattice — the eigenstates are not bound states in one well but Bloch waves extending across the lattice, with a spectrum of bands separated by gaps. The tridiagonal matrix still works, but you need periodic boundary conditions (a circulant matrix) and Bloch's theorem to interpret the results. That is the band-structure problem, the content of solid-state physics.

In two dimensions the Hamiltonian becomes $(N_x N_y) \times (N_x N_y)$. For $N_x = N_y = 500$ that is a $250000 \times 250000$ matrix, undiagonalizable in full by any browser method. Iterative eigensolvers — Lanczos, Arnoldi — find the lowest few eigenvalues without building the full matrix. Time evolution in 2D via split-step works with two sequential FFT passes. The infrastructure is unchanged from 1D; the algorithms compose.

For relativistic particles, $\omega = \sqrt{(\hbar k)^2 c^2 + m^2c^4}/\hbar$ replaces the free-particle $\omega = \hbar k^2/2m$. The dispersion is no longer quadratic, the group and phase velocities relate differently, and for fermions the wave function gains spinor components that the Dirac equation mixes. The split-step method still applies — the kinetic phase uses the relativistic dispersion — but the output physics is richer.

The harder fact is that most research-level quantum mechanics is neither one-dimensional nor exactly solvable. The hydrogen atom, the helium atom, the benzene molecule — none have clean analytic many-body solutions. The tools here — finite-difference discretization, matrix diagonalization, unitary time evolution — are the basis of every serious quantum simulation code. Density functional theory, quantum Monte Carlo, tensor network methods: all solve discretized Schrödinger equations, all need unitary time steppers, and all validate against exactly the benchmarks in this chapter.

You have been doing quantum mechanics in one dimension with one electron. The codes that design semiconductors, model drug-target binding, and simulate quantum computers do it in thousands of dimensions with millions of electrons. The logic is the same; the scale is the difference.
