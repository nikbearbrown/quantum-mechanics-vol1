...
In January 1926, Erwin Schrödinger set out to find a wave equation for matter, prompted by de Broglie's 1924 thesis. The requirement was that the equation reproduce the Bohr hydrogen energy levels as a consequence, not a postulate. After several false starts he found it, and within six months submitted four foundational papers to *Annalen der Physik*. The central equation of all of them is

$$i\hbar\,\frac{\partial \Psi}{\partial t} = \hat{H}\,\Psi.$$

This is the time-dependent Schrödinger equation: a partial differential equation, first order in time, second order in space, generally hard to solve. There is one tractable case — a time-independent potential — where a single substitution reduces the PDE to a solvable problem. That case covers nearly every system in this book: the infinite well, the harmonic oscillator, the hydrogen atom, all with static potentials. This chapter treats $V = V(x)$.
...
The left side carries only the time derivative; the right side mixes a spatial derivative with $V(x)$. The coupling is what makes it a PDE.

The product ansatz is the substitution $\Psi(x, t) = \psi(x)\,\phi(t)$. Substitute:
...
The left side depends only on $t$, the right side only on $x$. For them to be equal at every $x$ and $t$, both must equal a constant. Call it $E$.
...
Equation (4.1) solves immediately: $\phi(t) = e^{-iEt/\hbar}$, a phase rotating at angular frequency $E/\hbar$. The form is universal, independent of potential or energy level.

Equation (4.2) is the **time-independent Schrödinger equation** (TISE):

$$\boxed{-\frac{\hbar^2}{2m}\,\frac{d^2\psi}{dx^2} + V(x)\,\psi(x) = E\,\psi(x).}$$

This is an eigenvalue problem: find the functions $\psi$ for which $\hat{H}\psi$ returns $\psi$ times a constant. Those functions are the **stationary states** (energy eigenstates); the constants $E$ are the **energy eigenvalues**. Time evolution has been factored out entirely — a rotating phase attached to each state — and the physics lives in the spatial equation.

---

## What the Constant $E$ Actually Means

The constant $E$ is the energy, in a precise operational sense. For any state $\Psi$, the expectation value of the Hamiltonian is

$$\langle \hat{H} \rangle = \int \Psi^*(x,t)\,\hat{H}\,\Psi(x,t)\,dx.$$

For a stationary state $\Psi_n = \psi_n(x)\,e^{-iE_n t/\hbar}$, substitute. The phase $e^{+iE_n t/\hbar}$ from the conjugate and $e^{-iE_n t/\hbar}$ from the state multiply to 1. The eigenvalue equation $\hat{H}\psi_n = E_n\psi_n$ extracts $E_n$ from the integral, and normalization sets $\int|\psi_n|^2\,dx = 1$. The result:

$$\langle \hat{H} \rangle = E_n.$$

This is more than the average energy. The variance also vanishes: $\langle \hat{H}^2\rangle - \langle \hat{H}\rangle^2 = 0$. Every energy measurement on a particle in state $\psi_n$ returns exactly $E_n$ — no spread, no distribution. That is what an eigenstate of an operator is: a state of definite energy.

A consistency check confirms reality of $E$. If $E$ had a nonzero imaginary part, the factor $|e^{-iEt/\hbar}|^2 = e^{2\,\mathrm{Im}(E)\,t/\hbar}$ would make the probability grow or decay in time, which conservation of probability forbids. Self-adjoint Hamiltonians — what you get with real potentials and physical boundary conditions — have only real eigenvalues. The $E_n$ are always real.
...
The time dependence is gone, so the probability of finding the particle in any region is constant. That is the meaning of "stationary."

What "stationary" does not mean is that the wave function itself is time-independent. $\Psi_n$ rotates in the complex plane at angular frequency $E_n/\hbar$. Its real part goes as $\psi_n(x)\cos(E_n t/\hbar)$ and its imaginary part as $-\psi_n(x)\sin(E_n t/\hbar)$ — two components oscillating 90 degrees out of phase, like the two coordinates of a clock hand. The wave function is a rotating clock; the probability density is the clock's stationary shadow.

The chapter's simulation makes this visible. For a single eigenstate, the Re $\Psi$ and Im $\Psi$ panels animate while the $|\Psi|^2$ panel stays frozen. If $|\Psi|^2$ moves with only one eigenstate populated, the phase is not canceling correctly and the code is wrong.

---

## Why the Laser Cavity Is the Same Problem

A laser cavity is two mirrors facing each other with a gain medium between them. A pump beam enters; a single monochromatic, phase-coherent beam exits.

The cavity sustains only specific frequencies because it sustains only standing-wave modes that fit exactly between the mirrors. A wave that does not vanish at both mirror surfaces interferes destructively with itself on each round trip and dies out; a wave that does vanish builds up coherently and produces the output.

The cavity selects, from a continuous range of frequencies, the discrete set satisfying its boundary conditions. Quantization in quantum mechanics is the same phenomenon at a smaller scale with a different wave equation. The confining potential supplies the walls; only spatial modes satisfying the boundary conditions survive; those modes are the stationary states; their discrete frequencies are the discrete energies. The Schrödinger equation is the wave equation that determines which modes fit.

---

## Global Phase and Relative Phase

A global phase — multiplying the entire wave function by $e^{i\alpha}$ for real $\alpha$ — is unobservable. It cancels in every $|\Psi|^2$ and every expectation value. States are defined only up to a global phase.

The rotating phase $e^{-iE_n t/\hbar}$ of a stationary state is a global phase for that single state, hence unobservable. This is precisely why a stationary state is stationary: the rotating phase carries no measurable information.

Combining two stationary states changes everything. Form the superposition
...
The first two terms are frozen. The third oscillates at angular frequency $\omega_{12} = (E_2 - E_1)/\hbar$, the beat frequency between the levels. This is the interference term and the signature of superposition. Two states of different energy rotate at different rates; the angle between them in the complex plane grows steadily; the resulting interference pattern shifts continuously across space. That is the sloshing.

The energy does not slosh. The expectation value of the Hamiltonian in any superposition is

$$\langle \hat{H}\rangle = \sum_n |c_n|^2\,E_n.$$

This is a weighted average of the eigenvalues with weights $|c_n|^2$. The cross terms — the ones carrying time dependence — vanish by orthogonality of the eigenstates, leaving a result that is exactly time-independent. The probability distribution sloshes across the box while the total energy stays fixed. This is conservation of energy in the quantum language.

---

## Worked Example — A Single Eigenstate Against a Superposition

Use this comparison to see when $|\Psi|^2$ is static and when it oscillates.
...
The stationary state is $\Psi_1(x,t) = \psi_1(x)\,e^{-iE_1 t/\hbar}$, with probability density

$$|\Psi_1(x,t)|^2 = \frac{2}{L}\sin^2\!\left(\frac{\pi x}{L}\right).$$

No $t$ appears. The position expectation is

$$\langle x\rangle = \int_0^L x\cdot\frac{2}{L}\sin^2\!\left(\frac{\pi x}{L}\right)dx = \frac{L}{2},$$

by symmetry, fixed for all time.

Now add the first excited state:

$$\Psi(x,t) = \frac{1}{\sqrt{2}}\,\psi_1(x)\,e^{-iE_1 t/\hbar} + \frac{1}{\sqrt{2}}\,\psi_2(x)\,e^{-iE_2 t/\hbar}.$$

Since $\psi_1$ and $\psi_2$ are real, the probability density is

$$|\Psi(x,t)|^2 = \frac{1}{2}\!\left[\psi_1^2 + \psi_2^2 + 2\,\psi_1\psi_2\,\cos\!\left(\frac{(E_2-E_1)t}{\hbar}\right)\right].$$

Now $|\Psi|^2$ oscillates: the probability sloshes left and right across the well at the beat frequency. The "stationary" property is gone — not because of an error, but because a superposition of eigenstates with different energies is genuinely not a stationary state. Stationarity belongs to a pure energy eigenstate, and pure energy eigenstates are special.

---

## Building a General Solution

Suppose $\hat{H}$ has a complete, orthonormal set of eigenstates $\{\psi_n\}$ with eigenvalues $\{E_n\}$. Complete means any physically reasonable wave function can be written as a superposition of the $\psi_n$. Orthonormal means $\langle\psi_m|\psi_n\rangle = \delta_{mn}$. Both properties hold for the self-adjoint operators of quantum mechanics, guaranteed by the spectral theorem of functional analysis — a mathematical theorem, not a quantum postulate.

For the infinite well, the completeness of the eigenstates is the Fourier sine series theorem, proved by Fourier (for the heat equation) in 1822. The quantum application adds only the probabilistic interpretation and the phase rotation; the completeness predates quantum mechanics by a century. There is no quantum mystery here, only old mathematics in new dress.

Given an initial state $\Psi(x, 0)$, expand it:

$$\Psi(x, 0) = \sum_n c_n\,\psi_n(x), \qquad c_n = \int \psi_n^*(x)\,\Psi(x, 0)\,dx.$$

Orthonormality makes each projection exact and unique. Then attach the time-evolution phases:

$$\Psi(x, t) = \sum_n c_n\,\psi_n(x)\,e^{-iE_n t/\hbar}.$$

This is the complete general solution of the TDSE for any time-independent potential. Three steps: solve the eigenvalue problem for $\{\psi_n, E_n\}$; project the initial condition for $\{c_n\}$; attach the rotating phases. Step one is the hard one; steps two and three are routine calculation.

A student who stops at step one has found the stationary states but missed everything that happens when the particle is not in one. Finding the eigenstates ends the setup, not the problem.
