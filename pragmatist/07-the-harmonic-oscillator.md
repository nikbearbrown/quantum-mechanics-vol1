...
Any smooth potential with a minimum reduces to a harmonic oscillator for small displacements. Expand $V(x)$ in a Taylor series about the minimum $x_0$. The first-derivative term vanishes there; the constant term shifts the energy zero; the leading nontrivial term is quadratic:

$$V(x) \approx \frac{1}{2}V''(x_0)\,(x-x_0)^2 = \frac{1}{2}m\omega^2(x-x_0)^2,$$

with $\omega = \sqrt{V''(x_0)/m}$ set by the curvature. This is why the harmonic oscillator is the generic, not the special, case: a guitar string, a diatomic molecule, a laser-cavity mode, a lattice vibration, the quantized vacuum field — all reduce, near equilibrium, to the same Hamiltonian:

$$\hat{H} = \frac{\hat{p}^2}{2m} + \frac{1}{2}m\omega^2\hat{x}^2. \tag{7.1}$$

Solve (7.1) once and you have solved every system in the class. The remainder of the chapter is the solution.

---

## The Trick

Substituting $\hat{p} = -i\hbar\,\partial_x$ into (7.1) gives a second-order differential equation whose solutions are Hermite polynomials; that route is described below. The algebraic route delivers the full spectrum without solving any differential equation.

Define two operators:

$$\hat{a}_\pm = \frac{1}{\sqrt{2\hbar m\omega}}\!\left(\mp i\hat{p} + m\omega\hat{x}\right). \tag{7.2}$$

These are the **raising** ($\hat{a}_+$) and **lowering** ($\hat{a}_-$) operators, or ladder operators. Form the product $\hat{a}_-\hat{a}_+$:

$$\hat{a}_-\hat{a}_+ = \frac{1}{2\hbar m\omega}\!\left(i\hat{p} + m\omega\hat{x}\right)\!\left(-i\hat{p} + m\omega\hat{x}\right).$$

The $\hat{p}^2$ and $m^2\omega^2\hat{x}^2$ terms reassemble into $\hat{H}/(\hbar\omega)$. The cross terms give $im\omega(\hat{p}\hat{x} - \hat{x}\hat{p}) = im\omega(-i\hbar) = m\omega\hbar$, contributing $1/2$:
...
This commutator is the entire algebraic content of the problem. Every energy level, eigenstate relation, and expectation value follows from it. In quantum field theory the same relation — with $\hat{a}_-$ the annihilation operator and $\hat{a}_+$ the creation operator — opens quantum electrodynamics. Here it appears in the simplest setting.

---

## Climbing the Ladder

Let $|n\rangle$ be an energy eigenstate, $\hat{H}|n\rangle = E_n|n\rangle$. From (7.3) and $\hat{H} = \hbar\omega(\hat{a}_+\hat{a}_- + \tfrac{1}{2})$:

$$[\hat{H}, \hat{a}_+] = \hbar\omega\,\hat{a}_+, \qquad [\hat{H}, \hat{a}_-] = -\hbar\omega\,\hat{a}_-.$$

The energy of $\hat{a}_+|n\rangle$ follows:

$$\hat{H}(\hat{a}_+|n\rangle) = (\hat{a}_+\hat{H} + [\hat{H},\hat{a}_+])|n\rangle = (E_n + \hbar\omega)\,\hat{a}_+|n\rangle.$$

So $\hat{a}_+$ raises the energy by $\hbar\omega$ and $\hat{a}_-$ lowers it by $\hbar\omega$. The operators step up and down a ladder with rung spacing exactly $\hbar\omega$.

The ladder cannot descend without limit. Equation (7.1) is a sum of squares of Hermitian operators, so $\langle\hat{H}\rangle \geq 0$ in every state. Repeated application of $\hat{a}_-$ would eventually produce negative energies — a contradiction. The resolution is a ground state $|0\rangle$ annihilated by the lowering operator:

$$\hat{a}_-|0\rangle = 0. \tag{7.4}$$

Apply $\hat{H} = \hbar\omega(\hat{a}_+\hat{a}_- + \tfrac{1}{2})$:
...
The ground-state energy is $E_0 = \hbar\omega/2$, not zero. Applying $\hat{a}_+$ repeatedly from $|0\rangle$ gives:

$$\boxed{E_n = \left(n + \frac{1}{2}\right)\hbar\omega, \quad n = 0, 1, 2, \ldots} \tag{7.5}$$

The spectrum is equally spaced with gap $\hbar\omega$ and a ground state $\hbar\omega/2$ above the classical minimum. No differential equation was solved: the spectrum follows from one commutator and the non-negativity of energy.

---

## Zero-Point Energy Is Real

The ground-state energy $E_0 = \hbar\omega/2 \neq 0$ is a physical fact, not a bookkeeping artifact. Classically the minimum energy is zero — particle at rest at the bottom. Quantum-mechanically that is forbidden: a particle localized at $x = 0$ with $\sigma_x \to 0$ requires $\sigma_p \to \infty$ by the Kennard inequality $\sigma_x\sigma_p \geq \hbar/2$. The zero-point energy is the minimum kinetic energy that confinement demands.

It is measurable. Liquid helium does not solidify under atmospheric pressure at any temperature because the zero-point kinetic energy of helium atoms, confined to the inter-atom spacing, exceeds the interatomic binding energy; the atoms cannot lock into a lattice. Helium is a quantum liquid because of zero-point motion.

The Casimir force is a second instance: it arises from the difference in zero-point energies of electromagnetic modes with and without the plates present. Lamoreaux measured it in 1997 and matched the prediction to within five percent. Zero-point energy carries an invoice that the lab can read.
...
The factors $\sqrt{n+1}$ and $\sqrt{n}$ are mandatory; dropping them corrupts every expectation value. They come from the norm of $\hat{a}_+|n\rangle$, which is $\sqrt{n+1}$ (from $\langle n|\hat{a}_-\hat{a}_+|n\rangle = n+1$); dividing by the norm gives the unit-normalized $|n+1\rangle$.

Express $\hat{x}$ and $\hat{p}$ in terms of the ladder operators:
...
Expectation values in $|n\rangle$ then follow without integration. Since $\hat{a}_\pm|n\rangle$ is orthogonal to $|n\rangle$:

$$\langle n|\hat{x}|n\rangle = 0, \qquad \langle n|\hat{p}|n\rangle = 0.$$

For $\hat{x}^2 = (\hbar/2m\omega)(\hat{a}_+ + \hat{a}_-)^2$, the $\hat{a}_+^2$ and $\hat{a}_-^2$ terms connect $|n\rangle$ to $|n\pm 2\rangle$ and drop out. The surviving terms give:
...
At $n = 0$, $\sigma_x\sigma_p = \hbar/2$ — the Kennard bound, saturated exactly. The oscillator ground state is the unique minimum-uncertainty state.

The off-diagonal matrix element obeys $\langle m|\hat{x}|n\rangle \propto \delta_{m,n\pm 1}$: position connects only states differing by one quantum. This is the dipole selection rule — only $\Delta n = \pm 1$ transitions are allowed — which is why the infrared vibrational spectrum of a diatomic molecule is a set of evenly spaced lines at frequency $\omega$.

---

## What the Eigenstates Look Like

The algebra fixes the spectrum; the wave functions follow from $\hat{a}_-|0\rangle = 0$, which in position space is:
...
A Gaussian — the minimum-uncertainty shape from Chapter 3, but now an energy eigenstate of the oscillator rather than a free-particle packet. The distinction matters: the free-particle Gaussian spreads because it is not an eigenstate of the free Hamiltonian; the oscillator ground state stays Gaussian indefinitely because the restoring force exactly offsets the spreading tendency.

Higher states come from applying $\hat{a}_+$ to $\psi_0$, each application multiplying the Gaussian by a polynomial one degree higher in $x$. Those polynomials are the Hermite polynomials $H_n(\xi)$ with $\xi = \sqrt{m\omega/\hbar}\,x$:
...
Two facts. First, $\psi_n$ has exactly $n$ nodes — the ground state none, the first excited state one — the same counting rule as the infinite square well. Second, about 16% of the ground-state probability density lies outside the classical turning points $x = \pm\sqrt{\hbar/m\omega}$. Penetration into the forbidden region is not a high-energy effect; it is present in the very first eigenstate, as a default feature of the ground state.

---

## Eigenstates Do Not Oscillate

A common error must be addressed before the simulation: the expectation that the quantum oscillator oscillates in its eigenstates. Form the time-dependent eigenstate:
...
Static. The phase rotates at frequency $E_n/\hbar$, but $|\Psi|^2$ — everything a position detector sees — does not change. Energy eigenstates are stationary states. A theory containing only eigenstates would never exhibit classical oscillation.

Classical oscillation requires superpositions. Take the equal mixture $|\Psi\rangle = (|0\rangle + |1\rangle)/\sqrt{2}$:

$$|\Psi(x,t)|^2 = \tfrac{1}{2}(|\psi_0|^2 + |\psi_1|^2) + \psi_0\psi_1\cos\!\left(\frac{(E_1-E_0)t}{\hbar}\right).$$

The cross term beats at $(E_1 - E_0)/\hbar = \omega$, the classical frequency. The packet sloshes and also deforms — it is a superposition whose interference pattern shifts, not a rigid shape sliding back and forth.

The simulation shows both. The eigenstate panel is static; the superposition panel sloshes. If the eigenstate panel animates, the phase is not canceling correctly.
...
Coherent states are not energy eigenstates; they are superpositions of all $|n\rangle$ with Poisson weights $P(n) = e^{-|\alpha|^2}|\alpha|^{2n}/n!$ and mean quantum number $\langle n\rangle = |\alpha|^2$.

Under time evolution a coherent state stays coherent, with its amplitude rotating:
...
Position and momentum oscillate sinusoidally at $\omega$, as a classical oscillator does, and the packet keeps its shape with $\sigma_x\sigma_p = \hbar/2$ at all times. The coherent state is the minimum-uncertainty Gaussian riding its classical orbit without spreading.

Glauber introduced coherent states in 1963 to describe laser light. An ideal laser produces field modes in coherent states, and the photon-counting statistics of ideal laser light are Poisson — not because lasers are classical, but because Poisson is the photon-number distribution of a coherent state. The oscillator algebra is laser physics, not an analogy for it. Glauber received the Nobel Prize in 2005 for the connection.

---

## Worked Example — Ladder Algebra for HCl

**Given.** The HCl molecule vibrates near the bottom of its internuclear potential with $\omega \approx 5.63 \times 10^{14}$ rad/s and reduced mass $\mu \approx 1.63 \times 10^{-27}$ kg.

**Find.** Zero-point energy, level spacing and its thermal consequence, the dipole selection rule, and the ground-state $\langle\hat{x}^2\rangle$ — all from the ladder algebra, no integrals.
...
*Level spacing:* $\hbar\omega \approx 0.37$ eV. Thermal energy at 300 K is $k_BT \approx 0.025$ eV. Since $k_BT \ll \hbar\omega$, essentially all HCl molecules occupy the vibrational ground state at room temperature, and the infrared spectrum shows a single absorption line at $\lambda = hc/(\hbar\omega) \approx 3.4\ \mu$m — as observed.

*Selection rule.* The dipole matrix element is $\langle m|\hat{x}|n\rangle$. With $\hat{x} = \ell(\hat{a}_+ + \hat{a}_-)$, $\ell = \sqrt{\hbar/2\mu\omega}$, the operators give $\hat{a}_+|n\rangle \propto |n+1\rangle$ and $\hat{a}_-|n\rangle \propto |n-1\rangle$, both orthogonal to $|m\rangle$ unless $m = n\pm 1$:

$$\langle m|\hat{x}|n\rangle = \ell\!\left(\sqrt{n+1}\,\delta_{m,n+1} + \sqrt{n}\,\delta_{m,n-1}\right).$$

Only $\Delta n = \pm 1$ transitions survive. The spectrum is a single frequency, an evenly spaced ruler, confirmed by the HCl absorption spectrum.

*Ground-state $\langle\hat{x}^2\rangle$.* From (7.8) at $n = 0$:

$$\langle 0|\hat{x}^2|0\rangle = \frac{\hbar}{2\mu\omega} = \ell^2.$$

**Check.** The root-mean-square displacement is $\ell = \sqrt{\hbar/2\mu\omega} \approx 6.7$ pm, about 5% of the bond length (0.13 nm) — small, nonzero, and not obtainable by setting $E = 0$.

---

## Where the Ladder Leads

One commutator, $[\hat{a}_-,\hat{a}_+] = 1$, generates the entire structure: an equally spaced spectrum, selection rules, zero-point energy, coherent states, and minimum-uncertainty states. The method — factor the Hamiltonian into operators whose commutator is a scalar, then derive everything algebraically — is general.

In Chapter 10 the angular-momentum operators $\hat{L}_\pm$ satisfy a different algebra, but the logic is identical: a commutator with $\hat{L}_z$ sets the rung spacing, a non-negativity argument terminates the ladder, and the spectrum follows with no differential equation. In quantum field theory $\hat{a}_+$ becomes the creation operator that adds one particle to the vacuum; the $n$-photon state is $(\hat{a}_+)^n/\sqrt{n!}$ applied to the vacuum $|0\rangle$, the same state killed by $\hat{a}_-$. The commutator $[\hat{a},\hat{a}^\dagger] = 1$ is the foundation of quantum electrodynamics. This chapter is the template.
