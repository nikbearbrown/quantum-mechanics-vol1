...
In 1993, physicists at IBM's Almaden Research Center used the tip of a scanning tunneling microscope to push 48 individual iron atoms into a ring on a copper surface — a structure they called a quantum corral — and then imaged the electron density trapped inside. The image was not a smooth puddle of charge but a bull's-eye of concentric rings: a standing-wave pattern in the probability density, exactly what the Schrödinger equation predicts for an electron confined to a circular domain.

Crommie, Lutz, and Eigler did not impose the rings; the walls did. Confine an electron and it occupies only modes that fit the container. Those modes are standing waves, and the allowed energies are discrete because only certain spatial frequencies satisfy the geometry at the boundaries. The quantization was not assumed or inserted by hand. It follows from the mathematics of confinement.

This chapter carries the one-dimensional version of that argument — the infinite square well — through in closed form, from the Schrödinger equation to the energy spectrum to the dynamics of superpositions. The aim is to locate precisely where the discreteness comes from.
...
A guitar string fixed at both ends vibrates at a fundamental frequency and its integer harmonics. The reason is geometric: a mode survives only if it vanishes at both fixed endpoints. A half-wavelength fits, a full wavelength fits, three half-wavelengths fit — but one-and-a-third half-wavelengths cannot vanish at both walls simultaneously and collapses. The surviving frequencies are those that fit, and the fitting condition is discrete.

The analogy with a quantum particle in a box holds to this point and then stops. For the guitar string, the modes are literal physical displacements of a medium. For the electron, the "wave" is the wave function $\psi(x)$ — a complex-valued function whose squared modulus gives the probability of finding the particle at position $x$. No medium vibrates. The wave is not a wave in space but a wave of probability amplitude over possible positions. The boundary conditions share the same mathematical structure, but what is waving is entirely different.

With that distinction recorded, the calculation proceeds exactly like the guitar-string argument in quantum language.
...
Where the potential is infinite, the wave function must vanish. This is not an extra assumption; it is what $V = \infty$ forces in the TISE. If $\psi \neq 0$ where $V = \infty$, the equation $-(\hbar^2/2m)\psi'' + V\psi = E\psi$ cannot hold — the left side would be infinite while the right side is finite. So $\psi$ is zero outside the well, and by continuity of $\psi$:

$$\psi(0) = 0, \qquad \psi(L) = 0.$$

These are the boundary conditions: the walls, expressed in the language of the wave function.

Inside the well, $V = 0$, and the TISE is

$$-\frac{\hbar^2}{2m}\frac{d^2\psi}{dx^2} = E\psi.$$

The question is which values of $E$ are allowed.
...
This single equation introduces quantization. A sine vanishes only at integer multiples of $\pi$, so $kL = n\pi$ for $n = 1, 2, 3, \ldots$ The continuous set of all positive real $k$ collapses to a discrete ladder indexed by a positive integer.

This is not a postulate, an assumption about angular momentum, or a law inserted to fix a problem. It is the only way a sine wave can vanish at both walls simultaneously. Bohr postulated quantization in 1913; Schrödinger derived it in 1926 from a differential equation and two boundary conditions.
...
The energies scale as $n^2$, giving ratios $E_1 : E_2 : E_3 : E_4 = 1 : 4 : 9 : 16$. Nothing lives between these values. For $n = 0$: $k_0 = 0$ gives $\psi = A\sin(0) = 0$ — no particle, excluded. For negative $n$: $\sin(-n\pi x/L) = -\sin(n\pi x/L)$ produces the same probability density as positive $n$ — the same physical state up to an unobservable sign.

**Normalization.** The unnormalized eigenstate is $A\sin(n\pi x/L)$. Requiring $\int_0^L|\psi_n|^2\,dx = 1$ and using $\int_0^L\sin^2(n\pi x/L)\,dx = L/2$ (from $\sin^2\theta = (1-\cos2\theta)/2$, integrated over a whole number of half-periods):
...
So $E_2 \approx 1.51$ eV and $E_3 \approx 3.39$ eV. These are chemical and optical energy scales — visible photons carry roughly 2 eV, and room-temperature thermal energy is $k_BT \approx 0.025$ eV. The level spacings are large compared to thermal fluctuations, so quantum effects do not merely matter; they dominate.

For contrast, take the ground-state energy of a 1 g marble in a 1 cm box:

$$E_1 \approx \frac{\pi^2(10^{-34})^2}{2(10^{-3})(10^{-2})^2} \approx 5\times10^{-62}\ \text{J} \approx 3\times10^{-43}\ \text{eV.}$$

This is twenty orders of magnitude below anything measurable. The quantum discreteness is present in principle and invisible in practice. The classical limit is a calculation, not a philosophical choice.
...
For $m \neq n$: both integrands are cosines over an integer number of full periods on $[0, L]$, so both integrals vanish and $\langle\psi_m|\psi_n\rangle = 0$. For $m = n$: the first cosine becomes $\cos(0) = 1$, integrating to $L$; the second is a cosine over $n$ full periods, integrating to zero. So $\langle\psi_n|\psi_n\rangle = 1$.

No numerical eigensolver, no Gram-Schmidt. This is exact, analytic, and follows directly from classical Fourier analysis. Fourier established it in 1822 for the heat equation, more than a century before quantum mechanics. The mathematics of standing waves on a bounded interval was already complete; quantum mechanics added the physical interpretation.
...
The Heisenberg uncertainty principle explains why. If $E = 0$, the kinetic energy is zero, the momentum is zero, and the momentum uncertainty is $\sigma_p = 0$. But the particle is confined to a box of width $L$, so $\sigma_x \leq L$. The Kennard inequality $\sigma_x\sigma_p \geq \hbar/2$ then gives $L \cdot 0 \geq \hbar/2$, which is false. A particle with zero momentum uncertainty cannot also be confined in space; the uncertainty principle forbids $E = 0$.

This is not peculiar to quantum mechanics but a consequence of the wave nature of matter. A guitar string stretched flat has zero energy and zero frequency; the minimum non-trivial mode has positive frequency and positive energy. The same logic applies to any wave confined to a finite domain.

Quantitatively: for the ground state, $\sigma_p = \pi\hbar/L$ (from $\langle p^2\rangle = \pi^2\hbar^2/L^2$ with $\langle p\rangle = 0$ by symmetry), and $\sigma_x \approx 0.181L$. So $\sigma_x\sigma_p \approx 0.568\hbar \approx 1.136 \times (\hbar/2)$ — about 14% above the minimum allowed. The ground state of the infinite well is close to the most tightly localized state quantum mechanics permits.
...
The eigenstates $\psi_n$ are stationary states — their probability densities $|\psi_n|^2$ do not change in time. The full time-dependent eigenstate is $\Psi_n(x,t) = \psi_n(x)e^{-iE_n t/\hbar}$; the time factor is a pure phase rotation that cancels in $|\Psi_n|^2 = |\psi_n|^2$.

Dynamics appear only when two or more eigenstates of different energy are superposed. Take the equal-weight combination of the first two:
...
For the 1 nm electron, $E_1 \approx 6.0\times10^{-20}$ J, giving $T \approx 3.66$ femtoseconds. The probability distribution oscillates from one side of the well to the other on a femtosecond timescale.

**Where is the particle, on average?** Compute $\langle x\rangle(t) = \int_0^L x|\Psi(x,t)|^2\,dx$. Since $\psi_1$ and $\psi_2$ are both symmetric about $L/2$, their individual contributions give $\langle x\rangle_{1} = \langle x\rangle_{2} = L/2$. Only the cross term contributes time dependence:

$$\langle x\rangle(t) = \frac{L}{2} + \cos\!\left(\frac{(E_2-E_1)t}{\hbar}\right)\int_0^L x\,\psi_1(x)\,\psi_2(x)\,dx.$$

The cross integral is evaluated by integration by parts: expand $\psi_1\psi_2$ with the product-to-sum identity, then integrate $\int_0^L x\cos(n\pi x/L)\,dx$ by parts. The result is
...
At $t = 0$, $\cos(0) = 1$, giving $\langle x\rangle(0) \approx L/2 - 0.180L \approx 0.320L$. The initial state is left-heavy: the probability distribution peaks near $x = L/4$, because $\psi_1 + \psi_2$ — a half-sine plus a full-sine — has its constructive peak in the left half of the well. Since $d\langle x\rangle/dt|_{t=0} = 0$ (the cosine is at a turning point), the center of mass starts at rest, then moves rightward, reaching $\langle x\rangle \approx 0.680L$ at half-period before returning. The total swing is about $0.360L$.

Throughout, the energy expectation value is

$$\langle\hat{H}\rangle = \frac{1}{2}E_1 + \frac{1}{2}E_2 \approx \frac{0.377 + 1.508}{2} \approx 0.943\ \text{eV.}$$

Constant. The probability sloshes; the energy does not. The $|c_n|^2$ — the weights on each eigenstate — are fixed by the initial state and do not change in time. What evolves is the relative phase between the terms, which modulates the interference pattern in $|\Psi|^2$ but leaves the energy budget unchanged.
...
This is the Fourier sine series. The coefficients $c_n$ are projections — inner products of the initial state onto each eigenstate. Given an initial condition $\Psi(x, 0)$, find the $c_n$ by integration, then attach the time-evolution phases:

$$\Psi(x,t) = \sum_{n=1}^{\infty} c_n\,\psi_n(x)\,e^{-iE_n t/\hbar}.$$

Each eigenstate rotates at its own angular frequency $E_n/\hbar$. With two or more eigenstates of different energy present, the relative phases evolve, producing interference that changes $|\Psi|^2$ over time — the sloshing.

The probability of measuring energy $E_n$ is $|c_n|^2$. Note the consequence: the $|c_n|^2$ are set by the initial state, do not change in time, and are the only observable consequences of the $c_n$ for energy measurements. The phases matter only for position and momentum measurements, where the interference terms in $|\Psi|^2$ carry the relative-phase information.

A narrow initial state, concentrated near one point, requires many nonzero $c_n$ — representing a sharp feature as a sum of sine waves takes many harmonics. Many harmonics means many distinct frequencies, many beat frequencies between pairs of eigenstates, and rapid dephasing of the initial spatial structure. A particle initially localized near $x = L/4$ will not stay there; it spreads, sloshes, and eventually becomes nearly uniform. The classical limit corresponds to packets so heavy and slow that the dephasing timescale is astronomically long — effectively infinite.

---

## What Comes Next

The infinite square well gives clean answers because its walls are perfectly rigid: no wave function leaks through $V = \infty$, no energy tunnels out. Real systems — semiconductor quantum wells, quantum dots, atoms in optical lattice traps — have finite barriers, and wave functions do penetrate them. The discrete levels of a finite-depth well are qualitatively similar to the infinite well's but shifted downward, and they are finite in number: a well of depth $V_0$ and width $L$ supports only finitely many bound states. As $V_0 \to \infty$, those levels approach the infinite-well values from below.

The question the infinite well leaves open — why the probability interpretation is correct, and what it means for the wave function to "collapse" upon measurement — is not answered by solving the TISE. That is the measurement problem, which sits at the foundation of quantum mechanics in a way the Schrödinger equation does not resolve. Chapter 3 laid out Born's rule as a postulate; it remains a postulate. What the infinite well shows is that Born's rule combined with the Schrödinger equation makes experimentally exact predictions. Davisson–Germer diffraction, the quantum corral, and the semiconductor quantum-well energy levels measured spectroscopically by Dingle and colleagues in 1974 all confirm that the quantization derived here is not a mathematical artifact. The walls impose it, and nature obeys.
