...
The simulation from Chapter 0 showed a blue filled curve drifting, an orange curve inside it oscillating and going negative, and a gray dashed curve alongside. This chapter assigns the labels and derives what these objects must be for the theory to be consistent.

Begin with the units of the blue curve.

The particle moves along a line measured in nanometers, and the blue curve is plotted against that line, so it has units. The units are not dimensionless. The blue curve is not a probability; it is a probability *per nanometer* — a density, like population per square kilometer rather than a headcount. You cannot read "the particle is at position $x$" from the curve's value at a single point, any more than you can read a city's population from a density map at one address. To extract a probability — a dimensionless number between 0 and 1 — integrate over a region.

That is the Born rule. Given a wave function $\psi(x,t)$, the probability of finding the particle in the interval $[a,b]$ at time $t$ is

$$P\bigl(\text{particle in }[a,b]\bigr) = \int_a^b |\psi(x,t)|^2\,dx.$$

Max Born introduced this in 1926 in a paper on quantum scattering. His original draft wrote the probability as proportional to $\psi$, not $|\psi|^2$; he corrected it in a footnote added in proof. That footnote is one of the most consequential edits in the history of physics. Born received the 1954 Nobel Prize for the statistical interpretation it introduced.
...
The density $|\psi|^2$ can exceed 1: it is per unit length, not a dimensionless probability. Confusing the density's value at a point with a probability is a unit error and the single most common first mistake. Fix the distinction now and avoid a class of wrong answers for the rest of the course.

---

Now address why $\psi$ is complex.

If the measured quantity is $|\psi|^2$, which is real and non-negative, a real-valued function might seem to suffice. It does not, and the reason is the equation of motion, not convention.

The time-dependent Schrödinger equation is

$$i\hbar \frac{\partial \psi}{\partial t} = \hat{H}\psi.$$

The $i$ on the left is structural. Suppose $\psi$ were real. Then $\partial\psi/\partial t$ is real, and $i\hbar$ times a real quantity is purely imaginary, so the left side is imaginary while the right side (for real $\psi$ and real $V$) is real. The equation would force a real quantity to equal an imaginary one — possible only if everything vanishes. The dynamics push $\psi$ into the complex plane; the imaginary part cannot be removed.

This appeared on the screen. The orange curve was Re$\,\psi$, the gray dashed curve Im$\,\psi$. Both were non-trivial, with the imaginary part leading the real part by a quarter cycle. Freeze the wave function, set Im$\,\psi = 0$, and take one time step under the Schrödinger equation: the imaginary part rebuilds itself immediately. The orange curve cannot exist alone.
...
To see what the imaginary part carries, take the Gaussian wave packet with central momentum $\hbar k_0$:
...
Both oscillate and both carry the information in $k_0$. The density, however, is

$$|\psi|^2 = A^2\,e^{-x^2/a^2}.$$

The oscillations are gone. The density is a smooth Gaussian with no trace of $k_0$. The momentum — direction of travel, central velocity — is invisible in $|\psi|^2$. It lives entirely in the phase, in the relation between Re$\,\psi$ and Im$\,\psi$. Discard the imaginary part and a packet moving right cannot be distinguished from one moving left: both have the same $|\psi|^2$ and the same position histogram.

This is the physical content of the imaginary part, not a mathematical subtlety.

---

State what the blue curve was not doing, despite appearances.

When the blue curve spread over time, it was tempting to read the particle as physically expanding, like a gas. That reading is wrong. When detected, the particle is at one definite place. What spread was the *probability distribution* of that place. Run the same experiment — same initial conditions, same Hamiltonian, same $\psi(x,0)$ — ten thousand times and measure position each time. The histogram of results matches $|\psi|^2$, and the spread of the histogram is the spread of the blue curve.

$\psi$ is not the particle. $\psi$ is the instruction set for computing histograms.

The spreading blue curve states: had you run this experiment ten thousand times and measured position at this late moment, the results would scatter across this wide distribution. It is a statement about many repetitions, not about the fate of one particle.

This is what Born's footnote established — not a force, field, or new matter, but a *connection* between a mathematical object and an experimental distribution. The connection is the rule: compute $|\psi|^2$, integrate, get probabilities.

---

Because $|\psi|^2$ is a probability density and the particle is found *somewhere*, the total must equal one:

$$\int_{-\infty}^{\infty} |\psi(x,t)|^2\,dx = 1.$$

A wave function satisfying this is normalized. Wave functions are often written without a normalization constant; to fix this, compute the integral of $|\tilde{\psi}|^2$, take its square root, and divide.

The interesting question is whether normalization persists. If $\psi$ satisfies the Schrödinger equation and is normalized at $t=0$, does $\int|\psi|^2\,dx$ stay equal to 1 as time evolves? It must — otherwise the probability of finding the particle anywhere would drift from 1, which is incoherent — but necessity and provability are different.

Differentiate $\int|\psi|^2\,dx$ with respect to time and use the Schrödinger equation to replace $\partial_t\psi$ and $\partial_t\psi^*$. The potential-energy contributions cancel exactly, appearing with opposite signs. Only the kinetic terms survive, combining into a perfect spatial derivative:
...
This is the continuity equation, the same form that governs the flow of any conserved quantity — mass, charge, energy. Probability flows like a fluid with current $J$. Integrate over all $x$: the boundary terms at $\pm\infty$ vanish for any normalizable $\psi$, so $\frac{d}{dt}\int|\psi|^2\,dx = 0$. Normalization is permanent.

The derivation has a practical use. In a numerical simulation, $\int|\psi|^2\,dx$ is a diagnostic. Drift from 1 signals a fault: a non-unitary time-stepping scheme (explicit Euler does this) or probability leaking at the grid boundaries. The normalization indicator is a bug detector, and the conservation law just proved is the standard it is judged against.

---

Born's rule gives the probability distribution for position measurements. The average position across many experiments is the centroid of that distribution:

$$\langle x\rangle = \int_{-\infty}^{\infty} x\,|\psi(x,t)|^2\,dx.$$

Each position weighted by its probability — the mean of $|\psi|^2$ in the ordinary statistical sense.

Momentum is less direct. In position space, momentum is not a function of $x$; writing $\langle p\rangle = \int p\,|\psi(x)|^2\,dx$ yields nothing sensible. The Fourier transform supplies the connection: define $\phi(p,t)$ as the Fourier transform of $\psi(x,t)$. Then $|\phi(p,t)|^2$ is the probability density for momentum measurements and $\langle p\rangle = \int p\,|\phi|^2\,dp$. Translating back to position space via Parseval's theorem turns multiplication by $p$ into differentiation. The momentum operator in position space is

$$\hat{p} = -i\hbar\frac{\partial}{\partial x}.$$

The sign is not convention. For a packet moving right with $k_0 > 0$, this formula gives $\langle p\rangle = \hbar k_0 > 0$. Write $\hat{p} = +i\hbar\,\partial_x$ by mistake and every direction flips. The simulation catches this immediately: a rightward packet registers negative average momentum.
...
Now measure both spreads. Define them statistically:
...
State it cleanly. Prepare ten thousand identical copies of $\psi$. Measure position on five thousand and record $\sigma_x$; measure momentum on the other five thousand and record $\sigma_p$. No particle is measured twice, and no measurement disturbs another. Yet the product $\sigma_x\sigma_p$ cannot fall below $\hbar/2$.

The cause is Fourier analysis, not physical disturbance. A function narrow in $x$ is broad in its Fourier transform, and vice versa. The Kennard inequality is the precise quantification of that unavoidable relation between a function and its transform, with $\hbar/2$ an exact bound, not an order-of-magnitude estimate.

A separate Heisenberg measurement-disturbance relation — which *does* concern measuring position and then momentum on the same particle — has a different form, bound, and physical content. Ozawa formalized it in 2003; Erhart et al. tested it in 2012. The Kennard inequality and the measurement-disturbance relation are distinct. Conflating them is the most persistent conceptual error in introductory quantum mechanics.
...
The product is exactly $\hbar/2$. Every other normalizable wave function gives a larger product. This is why the Gaussian recurs throughout quantum mechanics: it is the tightest state, balancing position and momentum uncertainty as evenly as the physics permits.

Read the trade-off off the formulas. $\sigma_x \propto a$ and $\sigma_p \propto 1/a$. Narrow the packet in space — smaller $a$ — and the momentum spread widens proportionally. The product is locked. This is not a technological limit; it follows from the mathematics of Fourier transforms, not from imperfect instruments.
...
## Worked Example — Normalizing $e^{-|x|/a}$ and a Probability

Use this calculation as the template for applying the Born rule: normalize a given wave function, then integrate $|\psi|^2$ over an interval to get a probability.

**Given.** The un-normalized wave function $\tilde{\psi}(x) = e^{-|x|/a}$ for real $a > 0$.

**Find.** (1) the normalization constant $A$, and (2) the probability of finding the particle in $[-a, a]$.

**Solution.**

Normalization. Require $A^2\int_{-\infty}^{\infty}|\tilde\psi|^2\,dx = 1$:

$$A^2\int_{-\infty}^{\infty}e^{-2|x|/a}\,dx = 2A^2\int_0^{\infty}e^{-2x/a}\,dx.$$

The split at $x = 0$ is mandatory — the absolute value means $e^{-|x|/a} \neq e^{-x/a}$ for $x < 0$. Evaluating the integral:

$$2A^2\cdot\frac{a}{2} = A^2 a = 1 \implies A = \frac{1}{\sqrt{a}}.$$

Probability in $[-a, a]$. With $A^2 = 1/a$:

$$P = \frac{1}{a}\int_{-a}^{a}e^{-2|x|/a}\,dx = \frac{2}{a}\int_0^{a}e^{-2x/a}\,dx = 1 - e^{-2} \approx 0.865.$$

**Check.** First, units: $x$ in nm, $a$ in nm, so $A$ is in nm$^{-1/2}$ and $|\psi|^2$ in nm$^{-1}$ — correct for a one-dimensional probability density. Second, the magnitude: the interval $[-a,a]$ contains the peak of the distribution, so a probability of about 86.5% is plausible, with the remaining ~14% in the tails $|x| > a$. Third, the density at the origin: $|\psi(0)|^2 = 1/a$, which for $a = 0.5$ nm equals 2 nm$^{-1}$, a value greater than 1. Nothing is wrong — the density is measured in nm$^{-1}$ and is not required to be $\leq 1$. Expecting $|\psi|^2 \leq 1$ everywhere is a unit confusion, equivalent to expecting a population-density map never to exceed 1 person per square kilometer.

---

This wave function $e^{-|x|/a}$ is real-valued, so Im$\,\psi = 0$ everywhere and the probability current $J = (\hbar/m)\,\mathrm{Im}(\psi^*\,\partial_x\psi)$ vanishes everywhere. A real wave function carries zero current and cannot describe motion in a definite direction. To set the particle moving with central momentum $p_0$, multiply by $e^{ip_0 x/\hbar}$: $|\psi|^2$ is unchanged but $\langle p\rangle = p_0$. The density stays the same; the current changes; the physics changes. All directionality lives in the phase.
