...
Take the simplest problem in all of quantum mechanics. No potential anywhere; a particle drifting through empty space. You know how to set up the Schrödinger equation, and you have every right to expect a clean answer waiting at the end. What you actually get is one of the early shocks the theory had in store — the one that taught its founders that a sharp answer and a physical one are not the same thing.

The time-independent equation in a region where $V = 0$ is

$$-\frac{\hbar^2}{2m}\frac{d^2\psi}{dx^2} = E\psi,$$

and the solutions arrive at once: $\psi_k(x) = Ae^{ikx}$ with $k = \sqrt{2mE}/\hbar$. Every positive real $k$ qualifies. Every momentum $p = \hbar k$ is a legitimate eigenstate. The time-dependent version reads
...
Constant. Everywhere. The probability density is the same at $x = 0$ and at $x = 10^{26}$ meters. Integrate over all space: $\int_{-\infty}^\infty |A|^2\,dx = \infty$. The plane wave refuses to be normalized on the whole real line for any nonzero $A$. A free particle with perfectly sharp momentum $p = \hbar k$ is, quite literally, everywhere at once — spread uniformly across all of space with no preference for any place in it.

This is not a mathematical wart to be filed off. It is the uncertainty principle speaking plainly. If the momentum is perfectly sharp, $\sigma_p = 0$, then the Kennard inequality $\sigma_x\sigma_p \geq \hbar/2$ drives $\sigma_x$ to infinity. Non-normalizability is nothing but infinite position uncertainty wearing a different costume.

So what is to be done with a free particle? You build something the world can actually contain: a **wave packet**, assembled by superposing plane waves of nearby momenta so that they reinforce one another in a single region and annihilate everywhere else. The result is a localized, normalizable object that moves and spreads in a way that makes exact contact with classical mechanics — in the proper limit. This was the move Schrödinger and others made early on, and the calculation of how such a packet spreads was carried out by Charles Galton Darwin in 1927, only months after wave mechanics appeared.
...
The most general solution of the free-particle Schrödinger equation is a superposition of all the plane waves:

$$\Psi(x,t) = \frac{1}{\sqrt{2\pi}}\int_{-\infty}^{\infty}\phi(k)\,e^{i(kx-\omega(k)t)}\,dk, \qquad \omega(k) = \frac{\hbar k^2}{2m}.$$

The function $\phi(k)$ is the **momentum-space wave function** — equivalently, the Fourier amplitude. It records how much of each plane wave enters the mix. At $t = 0$ it forms a Fourier transform pair with $\Psi$:

$$\Psi(x,0) = \frac{1}{\sqrt{2\pi}}\int_{-\infty}^{\infty}\phi(k)\,e^{ikx}\,dk, \qquad \phi(k) = \frac{1}{\sqrt{2\pi}}\int_{-\infty}^{\infty}\Psi(x,0)\,e^{-ikx}\,dx.$$

Hand the recipe any initial wave function: compute $\phi(k)$ by Fourier transform, attach the phase $e^{-i\omega(k)t}$ to each component, and transform back. Free-particle time evolution is exact and complete in this form — nothing is approximated.

Why is the packet normalizable when not one of its ingredients is? If $\phi(k)$ clusters near $k_0$ with spread $\Delta k$, the Fourier theorem guarantees that $\Psi(x,0)$ is localized with spatial spread $\Delta x \sim 1/\Delta k$. A localized function is normalizable. The plane waves cancel each other everywhere save near the packet's center; destructive interference does the work that no individual wave could do on its own.

What does $\phi(k)$ mean physically? Apply the Born rule in momentum space: $|\phi(k)|^2\,dk$ is the probability of measuring momentum between $\hbar k$ and $\hbar(k+dk)$. And here is the load-bearing fact — $|\phi(k)|^2$ is **time-independent**. The momentum distribution never shifts during free propagation. The phase of $\phi(k)$ winds as $e^{-i\omega(k)t}$, but its modulus stands still. Whatever momentum distribution you begin with, you keep forever. The packet spreads in position; in momentum it does not budge.
...
A single plane wave $e^{i(kx-\omega t)}$ carries a crest — a surface of constant phase — at $kx - \omega t = \text{const}$. That crest travels at
...
The classical velocity is $v_{cl} = p/m$. The phase velocity is precisely half of it. The crests amble along at half the speed you would classically pin on the particle. Something is amiss — or rather, the phase velocity was never the thing to watch.

The particle is not a single crest. The particle is the *envelope* of the packet — the blob of probability. To find how that envelope moves, take a packet sharply peaked at $k_0$ and Taylor-expand the dispersion relation about $k_0$:

$$\omega(k) = \omega_0 + \omega_0'(k-k_0) + \tfrac{1}{2}\omega_0''(k-k_0)^2 + \cdots$$

Drop this into the superposition integral. The factor $e^{i(k_0 x - \omega_0 t)}$ lifts out as a carrier wave, and the leftover integral over the envelope turns into a function of the combination $x - \omega_0' t$. The envelope sits where that combination vanishes: at $x = \omega_0' t$. The **group velocity** is therefore

$$v_g = \frac{d\omega}{dk}\bigg|_{k_0}.$$

For the free particle, $d\omega/dk = \hbar k/m$, giving

$$v_g = \frac{\hbar k_0}{m} = \frac{p_0}{m}.$$

This is the classical velocity. The packet's center of mass marches along exactly as Newton's first law would have it. The free-particle wave packet is classical, in the sense that its expectation value of position obeys the classical equation of motion.

Now set the two speeds against each other. For the free particle:

$$\frac{v_g}{v_{ph}} = \frac{\hbar k_0/m}{\hbar k_0/2m} = 2.$$

The group velocity is exactly twice the phase velocity. The consequence is something you can watch: the envelope (the $|\Psi|^2$ blob) advances at $v_g$, while the individual crests inside it — the wiggles of Re $\Psi$ — creep along at $v_{ph} = v_g/2$. The envelope outruns its own internal oscillations. Crests pile in at the back of the packet and dissolve out the front. It is not a numerical artifact; it is what the mathematics demands, and experiment bears it out.

The phase velocity has no direct meaning for particle motion. It is not where the particle is, not how fast the particle moves, not a speed any detector in the path would register. The group velocity is the physically meaningful speed. The phase velocity is an internal feature of the wave's architecture, nothing more.
...
Nonzero. Different Fourier components travel at slightly different group velocities — the higher-$k$ pieces outpace the lower-$k$ ones. As time passes the components drift apart, and the packet spreads.

For a Gaussian initial state — the natural pick, since it saturates the uncertainty bound and the integral closes in elementary functions — the spreading can be computed exactly. Take

$$\Psi(x,0) = \left(\frac{1}{\pi\sigma_0^2}\right)^{1/4}\exp\!\left(-\frac{x^2}{2\sigma_0^2}\right)e^{ik_0 x},$$

a Gaussian of position-space width $\sigma_0$, centered at the origin, with mean wavenumber $k_0$. Its Fourier transform is again a Gaussian:

$$\phi(k) = \left(\frac{\sigma_0^2}{\pi}\right)^{1/4}\exp\!\left(-\frac{\sigma_0^2(k-k_0)^2}{2}\right),$$

of momentum-space width $\Delta k = 1/(2\sigma_0)$, so that $\sigma_x(0)\,\sigma_p = \sigma_0 \cdot \hbar/(2\sigma_0) = \hbar/2$. The Gaussian opens its life sitting right on the uncertainty bound — minimum uncertainty, as tight as quantum mechanics will allow.

Attach $e^{-i\omega(k)t}$ and grind the Gaussian integral by completing the square. The result:
...
The packet stays Gaussian for all time, but its width swells. Several things read off at a glance.

The center sits at $x = v_g t = (\hbar k_0/m)t$ — classical motion, to the letter. The momentum distribution $|\phi(k)|^2$ does not appear in this expression at all, because it has not changed. Only the position distribution has spread.
...
The spreading is caused by **dispersion** — $d^2\omega/dk^2 \neq 0$ — not by the uncertainty principle. The uncertainty principle is a constraint on the state at each instant; it says nothing about how the state evolves. A light pulse in vacuum has $\omega = ck$, so $d^2\omega/dk^2 = 0$, and it propagates without spreading at all. A coherent state of the harmonic oscillator (Chapter 7) likewise refuses to spread, despite living in a dispersive system — the restoring potential cancels the dispersion exactly. For the free particle there is no potential, no restoring force, and the spreading cannot be evaded.
...
It scales as $\sigma_0^2$: a packet twice as wide takes four times as long to double. And it is independent of $k_0$ — the mean momentum dictates where the packet goes, not how quickly it spreads. A fast packet and a slow packet of equal initial width spread at identical rates.
...
About 70 femtoseconds. An ultrafast pump-probe experiment resolves timescales of 10–100 fs, so this spreading is squarely observable. The electron does not ooze outward over microseconds; it smears across the quantum dot in less time than light takes to cross a human hair.

Now the limiting cases. If the dot is twice as narrow — $\sigma_x(0) = 1$ nm — the doubling time drops to about 17 fs: four times shorter, because $t_{2x} \propto \sigma_0^2$. If the dot is twice as wide — 4 nm — the doubling time grows to about 280 fs. The tighter the initial confinement, the broader the momentum distribution, and the faster the Fourier components fly apart.

One more check: the mean momentum $k_0$ never enters $t_{2x}$. An electron ejected from the dot at 1 eV and one ejected at 100 eV, both of the same initial width, double their spatial width at the same moment. The packet's translation speed is set by $k_0$; the spreading rate is set entirely by $\sigma_0$ and $m$.

---

## The Scales That Make Quantum Mechanics Invisible

Drop a 1 mg grain of sand into the same formula, localized to $\sigma_x(0) = 1\,\mu\text{m}$:

$$t_{2x} = \frac{2\times10^{-6}\,\text{kg}\times(10^{-6}\,\text{m})^2}{10^{-34}\,\text{J\,s}} \approx 2\times10^{16}\,\text{s.}$$

That is roughly $6\times10^{8}$ years — hundreds of millions of years, dwarfing any conceivable laboratory experiment. The quantum spreading is present in principle and indistinguishable from zero in practice. Quantum mechanics keeps no special rule for large objects; it applies the same formula, and the formula spits out an absurdly long timescale. The classical limit is not a philosophical decree. It is a number.

The dividing line is the ratio of the doubling time to any timescale that matters. For an electron in an atom, $t_{2x}$ runs from femtoseconds to attoseconds — quantum effects rule outright. For a marble, $t_{2x}$ outlasts the age of the universe — classical mechanics is exact for every practical purpose. The crossover between the two regimes is smooth and computable, not mysterious.
...
The free particle is the purest test bed for the Fourier structure of quantum mechanics, and it leaves three lessons that carry into every harder problem.

First: energy eigenstates are not always normalizable. The infinite square well's eigenstates were localized by the walls; the free particle's are not. Physical states are always normalizable, but they are not always energy eigenstates. A physical free particle is a superposition — a wave packet — never a plane wave.

Second: time evolution is a phase rotation in Fourier space. Each $\phi(k)$ component picks up the factor $e^{-i\omega(k)t}$. The modulus $|\phi(k)|^2$ is left untouched. So the momentum distribution is a conserved quantity for the free particle — it cannot change if no force is present to change it. The classical statement is that momentum is conserved in the absence of a force. The quantum statement says the same thing, expressed as the constancy of $|\phi(k)|^2$.

Third: the relationship between localization and momentum spread is exact and quantitative. To pin a particle within $\Delta x$, you must superpose momenta over a range $\Delta p \sim \hbar/\Delta x$. Tight localization demands broad momentum content. Broad momentum content means the Fourier components travel at a spread of speeds and pull apart quickly. There is no way to localize a particle and also keep it from spreading, because the two demands stand in direct opposition. The uncertainty principle is not a shortfall of measurement technology; it is a structural feature of waves.

The Fourier relationship $\Delta x \cdot \Delta k \sim 1$ is a theorem of harmonic analysis, proved in the nineteenth century for thoroughly classical reasons. Quantum mechanics did not invent it. What quantum mechanics added was the identification $p = \hbar k$ and the reading of $|\phi(k)|^2$ as a probability distribution. The mathematics of wave packets was already on the shelf; what was new was the physical interpretation of what, exactly, was waving.
