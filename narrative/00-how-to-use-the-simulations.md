...
Before this book introduces a single new piece of physics, you will have a free-particle Gaussian wave packet animating in your browser. Getting there takes three governing Markdown files (`CLAUDE.md`, `DESIGN.md`, `PROJECT.md`) and one four-move prompt — Show / Say / Constrain / Verify — pasted into Claude. This chapter installs that toolchain; every later LLM Exercise inherits the files you write here. The physics the packet is doing — dispersion, group velocity, spreading — is not derived yet. That waits for Chapters 1 and 2. Here the packet is only a vehicle for getting the apparatus working.
...
One image recurs throughout this book: a blue filled curve drifting to the right while orange ripples slip backward through it, and a number in the corner reading `1.000`. None of it means anything to you yet, and that is deliberate — Chapter 1 supplies the meaning. The reason to look *first* is that quantum mechanics resists being learned from words alone. Its objects have no classical counterpart, and a printed page gives you nothing to grip, rotate, or prod with a slider. This book hands you a third object to set beside the prose and the algebra: a simulation you built, governed by physics you can derive, that answers back when you change a parameter.

The problem this chapter solves is not a physics problem at all. It is logistics. Before simulations can teach you physics, you need a dependable, repeatable way to make them. The instrument is Claude — or any capable large language model. The technique is the four-move prompt. The reliability comes from three Markdown files you write once and carry through the entire book. That apparatus is what we install today.

When you close this chapter you will have a file named `00-wave-packet.html` sitting on your machine. Open it. Press play. Watch the blue blob drift, watch the orange ripples slide backward through it, and sit with the mild bafflement of seeing something you cannot yet name. Naming it is Chapter 1's job.

---

## The Brutalist System — Three Files, One Apparatus

The label *Brutalist* comes from architecture, where it names a style that puts its structural materials on display rather than hiding them behind ornament — the concrete, the steel, the exposed pipe. The Brutalist system in this book carries that idea into code: the rules that govern the simulations live in plain Markdown you can read, edit, and place under version control. Nothing important is buried inside a tool you do not control.

Three files do all the work.

### `CLAUDE.md` — The Coding Constitution

This is the file Claude reads before any coding prompt. It pins down the runtime (a modern browser, no build step), the library (D3 v7 from a CDN), the drawing surface (SVG only — not Canvas, not WebGL), the way parameters are exposed (HTML `<input type="range">` sliders firing on `input`), the animation loop (`requestAnimationFrame`), how complex wave functions are stored (`{re, im}` objects or two parallel `Float64Array`s), the units convention (SI internally, physical units on screen), and the normalization indicator (every simulation prints $\int |\psi|^2\,dx$ and warns if it strays from 1 by more than 1%).

Three of these rules earn their explanation.

**SVG, not Canvas.** A one-dimensional wave function is a curve. A probability density is a curve. A potential is a curve. SVG draws curves natively — one `<path>` element per curve, a vector representation that scales cleanly. Canvas would force you to manage a pixel buffer and rebuild axes by hand. D3 v7 is organized around SVG. The match is natural, and the code that results stays readable.

**Functions, not classes.** At its core the simulation is mostly a pure function from parameters to an array of values: `computeWavefunction(x, t, params)` hands back a typed array. Wrapping that in a class hides the structure and invites lifecycle questions — when do you instantiate, when do you destroy? Plain functions are easier to read, change, and debug.

**The normalization indicator.** Every simulation shows $\int |\psi|^2\,dx$ in a corner. It should read `1.000`. When it drifts, something has gone wrong: the model slipped a numerical error in, or you pushed a parameter outside the physical range, or there is a bug. The indicator is the simulation's lie detector. Ship nothing without it.

### `DESIGN.md` — The Visual Constitution

`DESIGN.md` governs what the simulation *looks like*: colors, fonts, axis labels, slider styling, colormap choices. It lives apart from `CLAUDE.md` because visual decisions and behavioral decisions fail in different ways. To change the palette you edit `DESIGN.md` alone, and the physics never notices.

Here are the color commitments this book makes, and the reasons behind them:

- $|\psi|^2$ in blue (filled): it is the panel you watch most. Blue reads well against both light and dark backgrounds.
- $\mathrm{Re}\,\psi$ in orange (solid line): it oscillates and goes negative; orange on a neutral background stays legible.
- $\mathrm{Im}\,\psi$ in gray dashed: same envelope, quarter-cycle phase shift; the dashed style says "secondary information."
- Potential $V(x)$ in red.
- Energy levels in green, horizontal.
- Classical turning points in purple, dashed vertical.

For two-dimensional density maps: **Viridis** for unsigned quantities (monotonic in luminance — readable in grayscale, readable for color-blind viewers). **RdBu** for signed amplitudes (red positive, blue negative, centered on zero). Never rainbow/jet: those palettes misrepresent the data, inventing perceptual jumps at arbitrary values. The research here is settled (Kovesi 2015; Borland & Taylor 2007).

### `PROJECT.md` — The Project State

`PROJECT.md` is the file Claude reads to remember the project. It is short — a few hundred words — and it records which chapter you are on, which simulations exist and have been verified, which parameter ranges you used, and what is broken. Finish a simulation, add a line. Fix a bug, note it.

Students forget this file exists. Don't. The reason is plain: Claude carries no memory between sessions. Return in three weeks to extend the Chapter 3 simulation and the model has lost every parameter, every convention, every verified fact. `PROJECT.md` is that memory. Without it, every extension prompt restarts from nothing. With it, the extension prompt is five lines.

### Why Three Files, Not One

The obvious question: why not fold everything into a single `CLAUDE.md`?

Token economy. A simulation request in Chapter 7 should not force the model to re-read slider-styling rules. The chapter-7 prompt loads `CLAUDE.md` (behavior) and `PROJECT.md` (state), and names `DESIGN.md` without quoting it. The visual rules remain in force without burning context.

The same split helps you debug. When a simulation looks wrong, the first question is whether the trouble is behavioral (wrong physics, wrong animation) or visual (wrong color, wrong layout). Either way you know which file to open. The three-file split is as much about debugging as about tokens.

**The "bureaucracy" misconception.** The files are not bureaucracy. They are the reason every chapter's simulation comes out consistent and debuggable. Without them you rediscover the same conventions every week and end up with twelve visually incompatible simulations. The files make the rules explicit so you can stop relitigating them.
...
**"Phase velocity is the speed of the particle."** Phase velocity belongs to a single plane-wave component, which is not normalizable and therefore not a physical particle. A localizable particle travels at the group velocity $v_g = p/m$. The simulation supplies the proof: watch the orange ripples slip backward through the blob. If phase velocity were "the speed," the particle would appear to run *backward* while the probability blob drifts forward — which is nonsense. Phase velocity is bookkeeping about the oscillation pattern; it carries no momentum.

**"The Brutalist files are bureaucracy."** Strip out `CLAUDE.md` and every prompt rediscovers the same failure modes: wrong dispersion relation, Canvas instead of SVG, explicit Euler, a dropped imaginary part. The file is a written record of failures. Strip out `PROJECT.md` and you lose the parameter conventions mid-semester, then spend half an hour reconstructing context. The files are the opposite of bureaucracy: they are why one prompt works instead of seven.

**"A description is as good as the equation."** "A Gaussian wave packet evolving in time" leaves the model free to pick the wrong dispersion relation, drop the $\gamma(t)$ factor, set $\mathrm{Im}\,\psi = 0$, or compute $|\psi|^2$ from a real function. Paste the closed-form complex equation and seven-try problems become one-try problems. Show the artifact.
...
- The Brutalist file split (CLAUDE / DESIGN / PROJECT) rests on a clear principle, yet the boundaries blur in practice. Does a slider's keyboard-accessibility rule belong in `CLAUDE.md` (behavior) or `DESIGN.md` (appearance)? The answer is behavior — accessibility rules govern how the component responds, not how it looks. But the book admits the boundary is fuzzy and asks you to document your choice in `CLAUDE.md`.

- The verification stack catches arithmetic errors. It does not catch *conceptual* errors that produce numerically plausible output. Swap the labels "Re $\psi$" and "Im $\psi$" and the simulation passes every quantitative check. Only the visual check defends against that, and it works only if you already know what the correct picture looks like — which is exactly why Chapter 1 opens by pointing straight at what you built here.

- LLM-generated simulation code drifts in style between sessions even with `CLAUDE.md` in the prompt. The book treats re-running the same prompt as a normal part of the workflow. Whether this matters — whether LLM-assisted interactive learning actually improves conceptual gains over static text — has not been studied carefully in the quantum mechanics curriculum, and the claim awaits a proper replication at the upper-division level.

- This chapter does not teach JavaScript. It teaches you to read and modify what Claude produces. Whether that is enough for Chapter 8 (harmonic oscillator) and Chapter 12 (capstone sandbox) is an open empirical question that the first run of this course will answer.
