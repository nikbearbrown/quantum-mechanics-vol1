...
You have already looked at something on the screen without anyone telling you what it was. A blue filled curve, drifting. An orange curve inside it, oscillating, dipping below zero, plainly indifferent to any rule about staying positive. You dragged a slider and the blue shape spread. Somewhere between recognition and confusion, you probably felt you were watching *something* — but the label was held back. I want to give you that label now. Not by handing you a definition to take on faith, but by working out what the blob is forced to be if the theory is to hang together at all.

Begin with the simplest possible question: what is the blob's unit?

If the particle moves along a line measured in nanometers, and the blob is a quantity plotted against that line, then the blob has units. Asking which units is already the right question — because the answer is not "dimensionless." The blob is not a probability. It is a probability *per nanometer*. A density. Like population per square kilometer, not a headcount. You cannot read "the particle is at position $x$" off the blob's value at a single point, any more than you can read a city's population off a density map at one address. To extract a probability — a genuine, dimensionless number between 0 and 1 — you must integrate over a region.
...
The density $|\psi|^2$ can exceed 1 — it is per unit length, not a dimensionless probability. Mistaking the value of the density at a point for a probability is a unit error, and it is the single most common first mistake. Fix the distinction in your mind now and you will sidestep a whole class of wrong answers for the rest of the course.

---

Now the second question: why is $\psi$ complex at all?

If what we measure is $|\psi|^2$, and $|\psi|^2$ is real and non-negative, why not just use a real-valued function and drop the imaginary part? The answer is not a convention. It is forced by the equation of motion.

The time-dependent Schrödinger equation is:

$$i\hbar \frac{\partial \psi}{\partial t} = \hat{H}\psi.$$

That $i$ on the left is structural, not cosmetic. Suppose $\psi$ were purely real. Then $\partial\psi/\partial t$ is real, and $i\hbar$ times something real is purely imaginary. The left side would be imaginary; the right side would be real (for real $\psi$ and real $V$). The equation would demand that a real quantity equal a purely imaginary one — impossible unless everything is zero. The dynamics themselves drive $\psi$ into the complex plane. You cannot strip out the imaginary part and still have a theory.

You saw this on the screen. The orange curve was Re$\,\psi$. The gray dashed curve was Im$\,\psi$. Both were non-trivial; the imaginary part led the real part by a quarter cycle. Freeze the wave function, set Im$\,\psi = 0$, and take a single time step under the Schrödinger equation, and the imaginary part rebuilds itself at once. The orange curve cannot live alone.
...
The oscillations vanish. The density is a smooth Gaussian with no trace of $k_0$ anywhere. The momentum — the direction of travel, the central velocity — is invisible in $|\psi|^2$. It lives entirely in the phase, in the relationship between Re$\,\psi$ and Im$\,\psi$. Throw away the imaginary part and you can no longer tell a wave packet moving right from one moving left. Both would have the same $|\psi|^2$. Both would look identical in every experimental histogram of position measurements.

This is not a mathematical subtlety. It is the physical content of the imaginary part.

---

Here is something the blob was not doing, even though it looked as though it might be.

When the blue curve widened over time, it was tempting to read that as the particle physically expanding — spreading out in space like a gas. That reading is wrong, and the mistake matters. The particle, when you actually detect it, is at one definite place. What spread was the *probability distribution* of where that place might be. Run the same experiment — same initial conditions, same Hamiltonian, same $\psi(x,0)$ — ten thousand times. Measure position each time. The histogram of results will match $|\psi|^2$. The spread of the histogram is the spread of the blob.

$\psi$ is not the particle. $\psi$ is the instruction set for computing histograms.

The blue blob spreading on the screen was telling you: had you run this experiment ten thousand times and measured position at this late moment, the measurements would scatter across this wide distribution. It was a statement about many repetitions, not about the fate of a single particle.
...
The more interesting question is whether normalization survives at later times. If $\psi$ satisfies the Schrödinger equation and you normalize it at $t=0$, does $\int|\psi|^2\,dx$ stay equal to 1 as time goes on? It has to — otherwise the probability of finding the particle anywhere would drift away from 1, which would be incoherent. But having to be true and being provably true are different matters.
...
Born's rule hands you the probability distribution for position measurements. The average position across many experiments is the centroid of that distribution:
...
The sign is no matter of convention. For a wave packet moving right with $k_0 > 0$, this formula gives $\langle p\rangle = \hbar k_0 > 0$. Write $\hat{p} = +i\hbar\,\partial_x$ by mistake and you flip every direction. The simulation catches this error at once: a wave packet sent rightward registers negative average momentum.
...
Here is the cleanest way to read what it says. Prepare ten thousand identical copies of the same state $\psi$. Measure position on five thousand of them — record the standard deviation $\sigma_x$. Measure momentum on the other five thousand — record $\sigma_p$. No single particle is ever measured twice. No measurement disturbs any other. And still the product $\sigma_x\sigma_p$ cannot fall below $\hbar/2$.
...
The product is exactly $\hbar/2$. Every other normalizable wave function gives a larger product. This is why the Gaussian turns up throughout quantum mechanics: it is the tightest possible state, the one that splits position and momentum uncertainty as evenly as the physics allows.

Notice the trade-off directly in the formulas. $\sigma_x \propto a$; $\sigma_p \propto 1/a$. Narrow the wave packet in space — smaller $a$ — and the momentum spread widens in proportion. The product is locked. This is not a technological limitation. You cannot engineer around it, because it follows from the mathematics of Fourier transforms, not from imperfect instruments.
...
A worked calculation makes the Born rule concrete before you attempt it yourself.
...
About 86.5%. The interval $[-a,a]$ holds the peak of the distribution, so this is plausible. The remaining ~14% sits in the tails $|x| > a$.

One more thing to notice. At $x = 0$, $|\psi(0)|^2 = 1/a$. For $a = 0.5$ nm this equals 2 nm$^{-1}$ — a number greater than 1. Nothing is wrong. The density is measured in nm$^{-1}$ and is under no obligation to be $\leq 1$. Expecting $|\psi|^2 \leq 1$ everywhere is a unit confusion; it would be like expecting a population density map never to exceed 1 person per square kilometer.
...
**Saturate the bound.** Select the Gaussian. Sweep $a$ from 0.2 nm to 4 nm. Watch $\sigma_x$ shift. Watch $\sigma_p$ shift inversely. Watch $\sigma_x\sigma_p/(\hbar/2)$ stay nailed at 1.000. Write down what this confirms about the relationship between the width in position space and the width in momentum space.

**Exceed the bound.** Switch to the infinite-well eigenstate, $n = 1$, $L = 10$ nm. The ratio should read about 1.136. Now run $n$ from 2 to 10. As $n$ climbs, $\sigma_p$ grows (higher energy means a larger momentum spread) while $\sigma_x$ barely moves. Does $\sigma_x\sigma_p/(\hbar/2)$ grow without bound, approach a limit, or oscillate? Predict first, then check.

**The double-peaked trap.** Select the double Gaussian with $d = 2$ nm, $\sigma = 0.3$ nm. $\langle x\rangle$ reads 0. Look at $|\psi|^2$: the probability of finding the particle within $\pm 0.5$ nm of $x = 0$ is small — the peaks sit at $\pm 2$ nm. Write one sentence explaining what this tells you about using $\langle x\rangle$ as a prediction for a single measurement.

**The square-pulse singularity.** Select the square pulse, width $w = 2$ nm. $\sigma_x$ is finite. $\sigma_p$ should display as "undefined." This is not a bug — the Fourier transform of a discontinuous square pulse decays as $1/p$, which makes $\int p^2|\phi(p)|^2\,dp$ diverge. The divergence is real physics; it reflects the infinite momentum required to manufacture sharp edges in position space.

---

## Still Puzzling

**Can Born's rule be derived?** This book takes the Born rule as a postulate — a law to be assumed, not proved. Other research programs (Zurek's "envariance" argument, various many-worlds branch-counting schemes) claim to derive it from more primitive axioms. Whether those derivations genuinely succeed or merely move the mystery elsewhere is honestly contested among physicists. The chapter states the postulate plainly and does not pretend the matter is settled.

**What does it mean for $\psi$ to "collapse" on measurement?** Born's rule predicts the probability distribution for the result of a position measurement. Once you measure and obtain a result $x_0$, what is $\psi$ then? The standard answer (Copenhagen) is that $\psi$ "collapses" to a state localized near $x_0$. This is convenient for the calculations that follow, but the physical mechanism — if there is one — remains deeply debated.

**Why does which-path information destroy interference?** Detect which slit each electron passes through and the interference pattern disappears, replaced by two classical blobs. The Born rule predicts the outcome but offers no mechanism — it does not explain *why* knowing the path erases the fringes. Different interpretations (Copenhagen, many-worlds, pilot wave) give different answers.

**Is the probability current $J$ really "flowing" probability?** The continuity equation $\partial_t|\psi|^2 + \partial_x J = 0$ wears the form of a conservation law, and $J$ has every mathematical property of a current. But in a single-particle interpretation, what exactly is flowing? For one particle, $|\psi|^2$ is not the density of a fluid — it is a probability distribution. The fluid interpretation is evocative and useful; it is not unambiguous.
