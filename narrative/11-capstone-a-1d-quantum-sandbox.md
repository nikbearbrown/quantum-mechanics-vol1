...
There is a particular moment in learning physics when the thing you have been deriving suddenly turns solid. For most students that moment does not arrive when they crack the harmonic oscillator by hand. It arrives when they type a potential into a computer, press a button, and watch the wave function bloom on the screen.

This chapter is about engineering that moment on purpose. You are going to build a configurable one-dimensional Schrödinger solver. Hand it any potential $V(x)$ you can lay down on a grid, and it will return the bound-state energies and eigenfunctions, or propagate an initial wave packet forward in time. When it runs, you will hold a machine that makes the Schrödinger equation computable for any problem in one dimension.

But building the machine is only half the labor. The other half is making sure the machine is *right*. A program that produces visually plausible output is not the same animal as a program that computes correct physics. The discipline of checking — running the solver on a problem whose answer you already know and demanding numerical agreement — is the habit that separates physical simulation from numerical theater. This chapter teaches that habit as deliberately as it teaches the algorithms.
...
The sandbox runs in two operational modes. Before you write a line of code, you need to know what each one computes.

**Eigensolver mode.** Given a potential $V(x)$ on a spatial grid, find the discrete bound-state energies $E_n$ and the matching eigenfunctions $\psi_n(x)$. The output is a list of energy levels and wave-function shapes — the stationary states, the ones whose probability distribution does not change over time. Every analytic result in Chapters 4 through 10 is a special case of this single computation.

**Time-evolution mode.** Given an initial wave function $\Psi(x, 0)$ and a potential $V(x)$, propagate $\Psi(x,t)$ forward by solving the time-dependent Schrödinger equation $i\hbar\,\partial_t\Psi = \hat{H}\Psi$. The output is an animation: probability density drifting, spreading, bouncing off walls, tunneling through barriers.
...
Write the kinetic hopping coefficient as $t_k = \hbar^2/(2mh^2)$. Then the diagonal is $2t_k + V_j$ and the off-diagonals are $-t_k$. If $V_j = 0$ everywhere, the matrix is pure kinetic energy — the coupling between neighboring grid points that lets the wave function propagate.

Boundary conditions for a hard-wall problem: enforce $\psi_0 = \psi_{N-1} = 0$ by building only the $(N-2)\times(N-2)$ interior submatrix. The rows and columns belonging to the boundary points are simply left out.

Diagonalize this matrix and you get $N-2$ eigenvalues and eigenvectors. The eigenvalues are the energies; the eigenvectors, each normalized so that $\sum_j|\psi_j|^2 \cdot h = 1$, are the discretized wave functions. The central-difference approximation puts an error in the $n$-th eigenvalue that scales as $(nh/L)^2$ — for $N = 500$ points and the ground state, below $10^{-6}$ in fractional terms. More than sufficient for any physical purpose here.
...
The number to burn into memory for debugging: the diagonal entry is $\hbar^2/(mh^2)$, which is $2t_k$, not $t_k$. The off-diagonal is $-t_k = -\hbar^2/(2mh^2)$. Confuse the two and your ground-state energy comes out off by a factor of two.

---

## Two Paths to Eigenvalues

The matrix eigensolver finds all eigenvalues at once and is the right tool when you want the full spectrum. But it leans on a linear algebra library. There is an alternative that asks for nothing beyond arithmetic.

**The Numerov shooting method** exploits a sharper approximation of $\psi''$ to reach $O(h^6)$ errors — four orders better than central difference — using only a three-point recursion. Rewrite the TISE as $\psi'' = f(x)\psi$ where $f_j = (2m/\hbar^2)(V_j - E)$. The Numerov recursion marches the wave function one step at a time:
...
The shooting strategy: guess an energy $E$, integrate from the left boundary with $\psi_0 = 0$, $\psi_1 = \epsilon$, and at the same time from the right with $\psi_{N-1} = 0$, $\psi_{N-2} = \epsilon$. At the midpoint, an eigenvalue exists precisely when the logarithmic derivatives of the left and right solutions agree — equivalently, when a certain Wronskian determinant changes sign. Sweep $E$ from low to high, find the sign changes, and bisect to pin down each eigenvalue to whatever precision you want.

The trade-off is clear. Numerov finds eigenvalues one at a time, needs careful bracketing, and is a touch more code than calling `math.eigs()`. For three to five eigenstates it is quicker to implement and far more instructive — you are integrating the Schrödinger equation directly, not hiding behind a black box. For twenty or more states, load the library.

For this capstone, start with Numerov. If you need all eigenvalues at once, add math.js (from a CDN) and call `math.eigs()` on the tridiagonal matrix. Both approaches validate against the same benchmark.

---

## Time Evolution: Why the Stepper Choice Is Not Optional

The time-dependent Schrödinger equation is $i\hbar\,\partial_t\Psi = \hat{H}\Psi$. The exact solution for a time-independent Hamiltonian is $\Psi(t) = e^{-i\hat{H}t/\hbar}\Psi(0)$. The operator $e^{-i\hat{H}t/\hbar}$ is unitary — it preserves the norm of the wave function exactly, for all time. Any numerical scheme that is not unitary is fundamentally broken, because a non-unitary stepper either manufactures or destroys probability.

**Why explicit Euler is forbidden.** The explicit Euler update is $\Psi^{n+1} = \Psi^n - (i\Delta t/\hbar)\mathbf{H}\Psi^n$. The update matrix is $\mathbf{I} - (i\Delta t/\hbar)\mathbf{H}$, whose eigenvalues are $1 - i\Delta t E_k/\hbar$ for each energy eigenvalue $E_k$. The modulus of each is $\sqrt{1 + (\Delta t E_k/\hbar)^2} > 1$. Every energy component grows exponentially at every step. The normalization indicator climbs above 1 within fifty steps and the simulation blows up. The normalization indicator in the sandbox corner catches this on sight — if it reads above 1.001 after ten steps, the time-stepper is wrong. Explicit Euler is unconditionally unstable for the Schrödinger equation and must never be used.
...
Building the sandbox is the lesser half of the project. The greater half is running the validation suite and confirming that your simulation computes correct physics. Here is what that means in concrete terms.
...
Mind the $h$ weighting — some eigensolvers return unit-norm vectors in the Euclidean sense ($\sum|\psi_j|^2 = 1$) rather than the physics sense ($\sum|\psi_j|^2 h = 1$). To convert: divide the returned eigenvector by $\sqrt{h}$. Forget the $h$ and your normalization integral reads $1/h$ instead of 1 — which for $h = 0.004$ nm gives a reading of 250 instead of 1. The normalization indicator in the corner catches this on sight.

During time evolution, that same indicator must stay within $\pm 0.001$ of $1.000$ at every animation frame. Drift above 1 means the time-stepper is creating probability; drift below 1 means it is destroying it. Both are physically wrong.

**Orthogonality.** For any two eigenstates:
$$\left|\sum_j \psi_j^{(n)*}\psi_j^{(m)} \cdot h\right| < 10^{-8}, \quad m \neq n.$$

A violation means either the eigensolver has a bug or the eigenvectors were never normalized, letting floating-point overflow contaminate the orthogonality.
...
Report a comparison table: for $n = 1, 2, 3, 4, 5$, list the analytic and numerical energies and the fractional error. The error should fall below $10^{-5}$ for $n = 1$ and below 1% for $n = 10$ with $N = 500$.

There is a dimensionless check that sidesteps units entirely: the ratios $E_n/E_1$ should come out exactly $n^2$. Confirm that $E_2/E_1 = 4.000$, $E_3/E_1 = 9.000$, $E_4/E_1 = 16.000$. If those ratios are right to three decimal places, the eigenvalue algorithm is working regardless of whether you have the right value for $\hbar$ in electron-volts. The ratios depend only on the numerics, not on any physical constant.
...
**Harmonic oscillator.** Set $V(x) = \frac{1}{2}m\omega^2 x^2$ on a grid wide enough to contain the first ten states. The analytic spectrum is $E_n = \hbar\omega(n + \frac{1}{2})$ — equally spaced levels, ground state at $\hbar\omega/2$. Verify: the level spacing is uniform to within 1%; the ground-state wave function is Gaussian; the ground-state uncertainty product $\sigma_x\sigma_p = \hbar/2$ (the harmonic oscillator ground state saturates the Kennard bound). That last check ties directly back to Chapter 9 and closes a loop that has been open since Chapter 1.
...
The sandbox does no new physics. Every feature assembles a tool you already have in hand.

The Born rule from Chapter 3 is why you display $|\Psi|^2$ rather than $\Psi$ itself, and why the normalization integral must equal 1. The TISE from Chapter 4 is what the matrix eigensolver solves. The argument that quantization falls out of boundary conditions — made for the infinite well in Chapter 5 — is reproduced automatically by the matrix formulation: Dirichlet boundary conditions enforce $\psi_0 = \psi_{N-1} = 0$, and the only solutions are the discrete eigenvectors. The harmonic oscillator spectrum of Chapter 7 is recovered numerically by setting $V_j = \frac{1}{2}m\omega^2 x_j^2$. The Gaussian wave-packet dynamics of Chapter 8 — group velocity, spreading, the $1/\sigma_0^2$ spreading rate — can be watched live in the time-evolution panel. The operators and expectation values of Chapter 9 give $\sigma_x$, $\sigma_p$, and $\langle\hat{H}\rangle$ from the numerical eigenstates. The tunneling of Chapter 6 reappears when you time-evolve a wave packet against a finite barrier and watch the transmitted fraction surface on the far side.

The synthesis is the point. You are not building something new. You are building a single machine that runs everything you already know, and then proving it agrees with what you already know.
...
The sandbox as built is one-dimensional and non-relativistic. The physics it cannot reach points straight at what comes next.

For periodic potentials — an array of identical wells, a crystal lattice — the eigenstates are not bound states trapped in one well. They are Bloch waves spreading across the entire lattice, and the spectrum splits into bands separated by gaps. The tridiagonal matrix still works, but you need periodic boundary conditions (making it a circulant matrix) and Bloch's theorem to read the results. That is the band-structure problem, and it is the content of solid-state physics.

For two dimensions, the Hamiltonian matrix swells to $(N_x N_y) \times (N_x N_y)$. With $N_x = N_y = 500$, that is a $250000 \times 250000$ matrix no browser could fully diagonalize by any available method. Iterative eigensolvers — Lanczos, Arnoldi — extract the lowest few eigenvalues without ever assembling the full matrix. Time evolution in 2D via split-step works cleanly with two sequential FFT passes. This infrastructure is unchanged from the 1D version; the algorithms compose.

For relativistic particles, $\omega = \sqrt{(\hbar k)^2 c^2 + m^2c^4}/\hbar$ replaces the free-particle $\omega = \hbar k^2/2m$. The dispersion is no longer quadratic; group and phase velocities relate differently; and for fermions the wave function carries spinor components that the Dirac equation mixes. The split-step method still applies — the kinetic phase in step 3 uses the relativistic dispersion — but the physics of the output is richer.

The harder truth is that most quantum mechanics problems in research are not one-dimensional and not exactly solvable. The hydrogen atom, the helium atom, the benzene molecule — none of them yield clean analytic solutions to their many-body Hamiltonians. The tools in this sandbox — finite-difference discretization, matrix diagonalization, unitary time evolution — are the bedrock of every serious quantum simulation code in existence. Density functional theory, quantum Monte Carlo, tensor network methods: all of them solve discretized versions of the Schrödinger equation, all of them need unitary time steppers, and all of them validate against exactly the benchmarks in this chapter.
