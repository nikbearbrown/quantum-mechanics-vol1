...
Pluck a guitar string a small distance to the side and let it go. The restoring force grows in proportion to the displacement — Hooke's law — and from Hooke's law follows simple harmonic motion, the most thoroughly worn groove in all of classical physics. Now perform the same move on any smooth potential that has a minimum: expand it in a Taylor series about that minimum. The first derivative is gone, because that is what "minimum" means. The constant term only relabels the zero of energy. What is left, at leading order, is a quadratic:

$$V(x) \approx \frac{1}{2}V''(x_0)\,(x-x_0)^2 = \frac{1}{2}m\omega^2(x-x_0)^2,$$

where $\omega = \sqrt{V''(x_0)/m}$ is fixed by the curvature at the bottom. Every stable equilibrium in every physical system, for displacements small enough, becomes a harmonic oscillator. The guitar string, the diatomic molecule, the electromagnetic mode trapped in a laser cavity, the vibration running through a crystal lattice, the quantized field of the vacuum itself — every one of them reduces to the same Hamiltonian:

$$\hat{H} = \frac{\hat{p}^2}{2m} + \frac{1}{2}m\omega^2\hat{x}^2. \tag{7.1}$$

This is not a special case to be filed away. It is the generic case for any quantum system sitting near equilibrium. Solve it once and you have solved the lot. The only question is *how* to solve it — and the answer that proved most fertile was not the obvious one.

---

## The Trick

The brute-force route is the natural one: write $\hat{p} = -i\hbar\,\partial_x$, substitute into (7.1), and grind through a second-order differential equation. It works, it produces the Hermite polynomials, and it is laid out further down. But there is a slicker method, one that delivers the entire spectrum without solving a single differential equation. It came to prominence through Dirac, who saw that the whole problem could be repackaged as algebra.

Define two operators:

$$\hat{a}_\pm = \frac{1}{\sqrt{2\hbar m\omega}}\!\left(\mp i\hat{p} + m\omega\hat{x}\right). \tag{7.2}$$

These are the **raising** ($\hat{a}_+$) and **lowering** ($\hat{a}_-$) operators, or ladder operators. Form the product $\hat{a}_-\hat{a}_+$ by multiplying out:
...
This single commutator is the whole algebraic content of the problem. Every energy level, every relation between eigenstates, every expectation value will descend from it. The same relation — with $\hat{a}_-$ christened the annihilation operator and $\hat{a}_+$ the creation operator — is the opening line of quantum electrodynamics. Here it surfaces in the humblest possible setting, the single oscillating particle.

---

## Climbing the Ladder

Suppose $|n\rangle$ is an energy eigenstate, $\hat{H}|n\rangle = E_n|n\rangle$. From the commutator (7.3) together with $\hat{H} = \hbar\omega(\hat{a}_+\hat{a}_- + \tfrac{1}{2})$, a short calculation gives:

$$[\hat{H}, \hat{a}_+] = \hbar\omega\,\hat{a}_+, \qquad [\hat{H}, \hat{a}_-] = -\hbar\omega\,\hat{a}_-.$$

Now ask what energy belongs to $\hat{a}_+|n\rangle$:

$$\hat{H}(\hat{a}_+|n\rangle) = (\hat{a}_+\hat{H} + [\hat{H},\hat{a}_+])|n\rangle = (E_n + \hbar\omega)\,\hat{a}_+|n\rangle.$$

Apply $\hat{a}_+$ to an eigenstate and out comes a new eigenstate with energy $\hbar\omega$ higher. Apply $\hat{a}_-$ and you drop $\hbar\omega$. The two operators build a ladder: you ascend one rung at a time or descend one rung at a time, and the rungs are evenly spaced, each exactly $\hbar\omega$ above the one below.

But the ladder cannot fall forever. The Hamiltonian (7.1) is a sum of squares of Hermitian operators, so $\langle\hat{H}\rangle \geq 0$ in every state. Were $\hat{a}_-$ allowed to keep subtracting $\hbar\omega$ without end, you would eventually arrive at negative energies — a flat contradiction. The escape is forced: there must exist a lowest state $|0\rangle$ that the lowering operator annihilates outright:

$$\hat{a}_-|0\rangle = 0. \tag{7.4}$$

Apply $\hat{H} = \hbar\omega(\hat{a}_+\hat{a}_- + \tfrac{1}{2})$ to this bottom state:
...
The ground-state energy is $E_0 = \hbar\omega/2$ — not zero, but $\hbar\omega/2$. Starting from $|0\rangle$ and applying $\hat{a}_+$ over and over:

$$\boxed{E_n = \left(n + \frac{1}{2}\right)\hbar\omega, \quad n = 0, 1, 2, \ldots} \tag{7.5}$$

An equally spaced spectrum with gap $\hbar\omega$, resting $\hbar\omega/2$ above the classical floor of the potential. No differential equation was harmed. The whole spectrum issued from one commutator and the single demand that energy stay non-negative.

---

## Zero-Point Energy Is Real

That ground-state energy $E_0 = \hbar\omega/2 \neq 0$ is not a bookkeeping artifact. Classically the minimum energy of a harmonic oscillator is zero — the particle resting at the bottom of the well. Quantum mechanics forbids that rest. A particle pinned to $x = 0$ with $\sigma_x \to 0$ would, by the Kennard inequality $\sigma_x\sigma_p \geq \hbar/2$, demand $\sigma_p \to \infty$. The zero-point energy is exactly the minimum kinetic energy that confinement extorts.

And you can read the invoice in the laboratory. Liquid helium will not freeze at atmospheric pressure at any temperature, because the zero-point kinetic energy of helium atoms — squeezed to the interatomic spacing — outweighs the energy binding them into a lattice. The atoms simply cannot settle. Helium stays a quantum liquid *because* of zero-point motion, not in spite of it.

More directly still: the Casimir force between two conducting plates is born from the difference in zero-point energies of the electromagnetic modes with the plates present versus absent. Steve Lamoreaux measured that force in 1997, matching the theoretical prediction to within five percent. Zero-point energy is not a quirk of the formalism. It pushes on real plates, and the push has been weighed.
...
Those prefactors $\sqrt{n+1}$ and $\sqrt{n}$ are not optional ornament. Drop them and every expectation value you compute afterward comes out wrong. Where do they come from? $\hat{a}_+|n\rangle$ has norm $\sqrt{n+1}$ — read off from $\langle n|\hat{a}_-\hat{a}_+|n\rangle = n+1$ — so dividing by that norm produces the unit-normalized $|n+1\rangle$.

Now express $\hat{x}$ and $\hat{p}$ in terms of the ladder operators:
...
With these in hand, expectation values in any energy eigenstate $|n\rangle$ become almost effortless. Since $\hat{a}_\pm|n\rangle$ is orthogonal to $|n\rangle$:

$$\langle n|\hat{x}|n\rangle = 0, \qquad \langle n|\hat{p}|n\rangle = 0.$$

For $\hat{x}^2$: write $\hat{x}^2 = (\hbar/2m\omega)(\hat{a}_+ + \hat{a}_-)^2$. The terms $\hat{a}_+^2$ and $\hat{a}_-^2$ link $|n\rangle$ to $|n\pm 2\rangle$ and contribute nothing. What survives gives:
...
At $n = 0$: $\sigma_x\sigma_p = \hbar/2$ — the floor set by the Kennard inequality, hit exactly. The oscillator's ground state is the unique minimum-uncertainty state, the tightest packet quantum mechanics permits.

For the off-diagonal matrix element, $\langle m|\hat{x}|n\rangle \propto \delta_{m,n\pm 1}$: position links only states differing by a single quantum. This is the dipole selection rule — only $\Delta n = \pm 1$ transitions are allowed — and it is why the infrared vibrational spectrum of a diatomic molecule comes out as a clean ruler of evenly spaced absorption lines at frequency $\omega$.

---

## What the Eigenstates Look Like

The algebra gave us the spectrum. The wave functions follow from the annihilation condition $\hat{a}_-|0\rangle = 0$, which in position space reads:

$$\left(\hbar\frac{\partial}{\partial x} + m\omega x\right)\psi_0(x) = 0.$$

Separate variables, normalize, and:

$$\psi_0(x) = \left(\frac{m\omega}{\pi\hbar}\right)^{1/4}\exp\!\left(-\frac{m\omega x^2}{2\hbar}\right). \tag{7.10}$$

A Gaussian — the very minimum-uncertainty shape met back in Chapter 3, but now standing as an energy eigenstate of the oscillator rather than a free-particle packet. The distinction is everything. The free-particle Gaussian spreads in time because it is *not* an eigenstate of the free Hamiltonian. The oscillator ground state stays Gaussian for all time, because the potential supplies a restoring force that cancels the spreading tendency exactly.

Higher states are minted by applying $\hat{a}_+$ to $\psi_0$. Each application multiplies the Gaussian by a polynomial one degree higher in $x$. Those polynomials are the Hermite polynomials $H_n(\xi)$, with $\xi = \sqrt{m\omega/\hbar}\,x$:

$$\psi_n(x) = \left(\frac{m\omega}{\pi\hbar}\right)^{1/4}\frac{1}{\sqrt{2^n n!}}\,H_n(\xi)\,e^{-\xi^2/2}, \tag{7.11}$$

obeying the recursion $H_{n+1}(\xi) = 2\xi H_n(\xi) - 2n H_{n-1}(\xi)$, starting from $H_0 = 1$, $H_1 = 2\xi$.

Two features deserve a pause. First, $\psi_n$ has exactly $n$ nodes — the ground state none, the first excited state one — the same counting rule that governed the infinite square well. Second, roughly 16% of the ground-state probability density spills outside the classical turning points $x = \pm\sqrt{\hbar/m\omega}$. Tunneling into the forbidden region is not some exotic behavior reserved for high energy. It is there in the very first eigenstate, a standing feature of the quantum ground state.

---

## Eigenstates Do Not Oscillate

Here is the misconception that has to be cleared before the simulation.

A harmonic oscillator, classically, oscillates. You naturally expect the quantum version to do the same. But form the time-dependent eigenstate:
...
Frozen. The phase spins in the complex plane at frequency $E_n/\hbar$, but $|\Psi|^2$ — every last thing a position detector could register — does not budge. Energy eigenstates are *stationary* states. If quantum mechanics held nothing but eigenstates, classical oscillation would never make an appearance.

Oscillation belongs to superpositions. Take the even mixture $|\Psi\rangle = (|0\rangle + |1\rangle)/\sqrt{2}$:

$$|\Psi(x,t)|^2 = \tfrac{1}{2}(|\psi_0|^2 + |\psi_1|^2) + \psi_0\psi_1\cos\!\left(\frac{(E_1-E_0)t}{\hbar}\right).$$

The cross term beats at frequency $(E_1 - E_0)/\hbar = \omega$ — the classical oscillation frequency, exactly. The packet sloshes back and forth, but it also deforms: not a rigid blob sliding to and fro, but a superposition whose interference pattern continuously reshapes.

The simulation puts both cases side by side. The eigenstate panel sits still; the superposition panel sloshes. If the eigenstate panel ever animates, the phase is not cancelling as it should — a bug, not physics.
...
Coherent states are not energy eigenstates. They are superpositions of every $|n\rangle$ with Poisson-distributed weights $P(n) = e^{-|\alpha|^2}|\alpha|^{2n}/n!$ and mean quantum number $\langle n\rangle = |\alpha|^2$.

Under time evolution a coherent state stays a coherent state, its amplitude merely rotating:
...
Position and momentum trace out sinusoids at frequency $\omega$, exactly as a classical oscillator would. And the packet holds its shape — $\sigma_x\sigma_p = \hbar/2$ at every instant. The coherent state is the minimum-uncertainty Gaussian riding its classical orbit forever without smearing.

It was Roy Glauber who introduced coherent states in 1963, and he did it to describe the quantum state of laser light. An ideal laser fills its field modes with coherent states. The photon-counting statistics of ideal laser light come out Poisson — not because lasers are somehow classical, but because Poisson *is* the photon-number distribution of a coherent state. The harmonic-oscillator algebra is not a metaphor for laser physics; it is laser physics. Glauber took the 2005 Nobel Prize for drawing that connection.

---

## Worked Example — Ladder Algebra for HCl

The HCl molecule vibrates near the floor of its internuclear potential with $\omega \approx 5.63 \times 10^{14}$ rad/s and reduced mass $\mu \approx 1.63 \times 10^{-27}$ kg.

**Zero-point energy:**

$$E_0 = \tfrac{1}{2}\hbar\omega = \tfrac{1}{2}(1.055\times10^{-34})(5.63\times10^{14}) \approx 2.97\times10^{-20}\ \text{J} \approx 0.185\ \text{eV}.$$

**Level spacing:** $\hbar\omega \approx 0.37$ eV. Thermal energy at 300 K: $k_BT \approx 0.025$ eV. Since $k_BT \ll \hbar\omega$, essentially every HCl molecule sits in the vibrational ground state at room temperature. The infrared spectrum shows a single absorption line at $\lambda = hc/(\hbar\omega) \approx 3.4\ \mu$m — precisely what is seen.

**Selection rule from ladder algebra.** The dipole transition matrix element is $\langle m|\hat{x}|n\rangle$. Write $\hat{x} = \ell(\hat{a}_+ + \hat{a}_-)$ with $\ell = \sqrt{\hbar/2\mu\omega}$. Then $\hat{a}_+|n\rangle \propto |n+1\rangle$ and $\hat{a}_-|n\rangle \propto |n-1\rangle$, both orthogonal to any $|m\rangle$ unless $m = n\pm 1$. The element vanishes for every other transition:

$$\langle m|\hat{x}|n\rangle = \ell\!\left(\sqrt{n+1}\,\delta_{m,n+1} + \sqrt{n}\,\delta_{m,n-1}\right).$$

Only $\Delta n = \pm 1$ transitions survive. The vibrational spectrum is a single frequency, not a thicket of lines — a ruler in frequency space, and the HCl absorption spectrum confirms it.
...
The root-mean-square displacement of HCl in its ground state is $\ell = \sqrt{\hbar/2\mu\omega} \approx 6.7$ pm — about 5% of the bond length (0.13 nm). Small, but not zero, and emphatically not obtainable by setting $E = 0$.

---

## Where the Ladder Leads

One commutator: $[\hat{a}_-,\hat{a}_+] = 1$. Out of it came a complete equally spaced spectrum, the selection rules, the zero-point energy, the coherent states, and the minimum-uncertainty state. The method itself — factor the Hamiltonian into operators whose commutator is a mere scalar, then derive everything algebraically — is not tied to this one problem.

In Chapter 10 the angular momentum operators $\hat{L}_\pm$ obey a different algebra, but the logic repeats note for note: a commutator with $\hat{L}_z$ sets the rung spacing, a non-negativity argument terminates the ladder, and the spectrum drops out with no differential equation in sight. In quantum field theory, $\hat{a}_+$ becomes the creation operator that adds one particle to the vacuum; the $n$-photon state is $(\hat{a}_+)^n/\sqrt{n!}$ acting on the vacuum, where that vacuum $|0\rangle$ is exactly the ground state killed by $\hat{a}_-$. The commutator $[\hat{a},\hat{a}^\dagger] = 1$ is the cornerstone of quantum electrodynamics. This chapter is the template from which all of it is cut.
