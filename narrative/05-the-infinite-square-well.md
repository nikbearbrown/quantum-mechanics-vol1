...
In 1993, physicists at IBM's Almaden Research Center spent several days nudging individual iron atoms across a copper surface with the tip of a scanning tunneling microscope. One at a time, they herded 48 atoms into a ring — a structure they named a quantum corral — and then imaged the electron density trapped inside. What they saw was not a smooth puddle of charge. It was a bull's-eye of concentric rings, a standing-wave pattern in the probability density, precisely what the Schrödinger equation predicts for an electron confined to a circular domain.

Crommie, Lutz, and Eigler did not put the rings in. The walls imposed them. Confine an electron, and it can inhabit only the modes that fit the container. The modes look like standing waves because they are standing waves. The allowed energies come out discrete because only certain spatial frequencies satisfy the geometry at the boundaries. The quantization was not assumed, postulated, or slipped in by hand. It fell out of the mathematics of confinement.

This chapter works through the one-dimensional version of the same argument — the infinite square well — where the geometry is simple enough to carry the derivation all the way, in closed form, from the Schrödinger equation to the energy spectrum to the dynamics of superpositions. The goal is not to memorize formulas. It is to watch, step by step, exactly where the discreteness comes from.
...
A guitar string fixed at both ends vibrates at a fundamental frequency and its integer harmonics. The reason is purely geometrical: a mode survives only if it vanishes at both fixed endpoints. A half-wavelength fits. A full wavelength fits. Three half-wavelengths fit. But one-and-a-third half-wavelengths cannot vanish at both walls at once — it contradicts itself at the boundary and collapses. The surviving frequencies are the ones that fit, and the fitting condition is discrete.

The analogy with a quantum particle in a box runs this deep and then stops. For the guitar string, the modes are literal physical displacements of a medium. For the electron, the "wave" is the wave function $\psi(x)$ — a complex-valued function whose squared modulus gives the probability of finding the particle at position $x$. No medium is vibrating. The wave is not a wave in space; it is a wave of probability amplitude in the space of possible positions. The boundary conditions are the same mathematical structure, but what is waving means something entirely different.

With that distinction flagged, the calculation proceeds exactly like the guitar string argument, dressed in quantum language.
...
Where the potential is infinite, the wave function must vanish. This is not an extra assumption; it is what $V = \infty$ forces in the time-independent Schrödinger equation (TISE). If $\psi \neq 0$ where $V = \infty$, the equation $-(\hbar^2/2m)\psi'' + V\psi = E\psi$ cannot hold: the left side would be infinite while the right side stays finite. So the wave function is zero outside the well, and by the requirement that $\psi$ be continuous:
...
This is not a postulate. It is not an assumption about angular momentum or a new law inserted to patch a problem. It is the only way a sine wave can vanish at both walls at once. Bohr postulated quantization in 1913; Schrödinger derived it in 1926 from a differential equation and two boundary conditions.
...
This is not a quirk peculiar to quantum mechanics — it is a consequence of the wave nature of matter. A guitar string stretched flat has zero energy and zero frequency. The lowest non-trivial mode has positive frequency, positive energy. The same logic governs any wave confined to a finite domain.
...
At $t = 0$, $\cos(0) = 1$, giving $\langle x\rangle(0) \approx L/2 - 0.180L \approx 0.320L$. The initial state is left-heavy: the probability distribution peaks near $x = L/4$, because $\psi_1 + \psi_2$ — a half-sine plus a full-sine — has its constructive peak in the left half of the well. Since $d\langle x\rangle/dt|_{t=0} = 0$ (the cosine sits at a turning point), the center of mass starts at rest and then moves rightward, reaching $\langle x\rangle \approx 0.680L$ at half-period before returning. The total swing is about $0.360L$.
