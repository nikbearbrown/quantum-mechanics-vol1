...
Here is an experiment you can run, at least on paper. Take an electron. Prepare it in exactly the same state a thousand times — same apparatus, same procedure, identical setup. Measure its position each time. You get a thousand different numbers. Histogram them. The histogram has a mean $\langle x\rangle$ and a spread $\sigma_x$.

Now repeat with a fresh thousand identically prepared copies, but measure momentum instead. Another histogram, another mean $\langle p\rangle$, another spread $\sigma_p$.

What you find is this: however cunningly you prepare the state, the product $\sigma_x\sigma_p$ is never below $\hbar/2$. Narrow the position distribution as far as you like, and the momentum distribution widens to make up for it. No copy of the electron is ever measured twice. No measurement disturbs any other. The limitation lives in the state itself, before any apparatus comes near it.

The question is what, in the mathematics, produces this bound. The answer is the structure of operators — specifically, the relationship between the operators for position and momentum. That relationship has a name, and once you see it, the inequality falls out in about half a page.

---

## Observables as Operators

In quantum mechanics every physical observable is a linear operator $\hat{A}$ that sends wave functions to wave functions:

$$\hat{A}(\alpha\psi + \beta\phi) = \alpha\hat{A}\psi + \beta\hat{A}\phi.$$

Why operators? Because we need a rule that extracts a number — an average measurement outcome — from a wave function. The expectation value $\langle\hat{A}\rangle = \int\psi^*(\hat{A}\psi)\,dx$ is that rule.

But not every linear operator counts as an observable. The requirement is **Hermiticity**: for any two normalizable wave functions $\psi$ and $\phi$,

$$\int_{-\infty}^{\infty}\phi^*\,(\hat{A}\psi)\,dx = \int_{-\infty}^{\infty}(\hat{A}\phi)^*\,\psi\,dx.$$

Hermiticity buys three things, each of which earns its keep. First: the eigenvalues of a Hermitian operator are real — and since measurement outcomes are real numbers, that is not negotiable. Second: eigenstates with distinct eigenvalues are orthogonal, so the outcomes live in an orthogonal basis. Third, the spectral theorem: the eigenstates form a complete set, so any state expands as $|\psi\rangle = \sum_n c_n|a_n\rangle$, with the probability of outcome $a_n$ equal to $|c_n|^2$. That is the Born rule, in operator language.
...
The position operator is the easy one: $(\hat{x}\psi)(x) = x\psi(x)$. Multiplication by the real number $x$. Hermitian by inspection.

The momentum operator is

$$\hat{p} = -i\hbar\frac{\partial}{\partial x}.$$

The $-i$ and the $\hbar$ are not decoration. Check Hermiticity by computing $\int\phi^*(\hat{p}\psi)\,dx$:
...
Hermitian. The $-i$ is exactly what makes it work: without it, $\partial_x$ would be anti-Hermitian, its eigenvalues imaginary — and imaginary numbers are not things you read off a meter.

Why is this the right momentum operator, and not some other Hermitian combination of derivatives? The cleanest argument is Fourier. A plane wave $e^{ipx/\hbar}$ is the eigenfunction of $-i\hbar\partial_x$ with eigenvalue $p$: differentiating $e^{ipx/\hbar}$ brings down $ip/\hbar$, and $-i\hbar \cdot ip/\hbar = p$. In momentum space $\hat{p}$ is multiplication by $p$; in position space it is differentiation. The Fourier transform links the two pictures, and $-i\hbar\partial_x$ is the unique Hermitian first-order differential operator that becomes multiplication-by-$p$ on the momentum side.
...
For position, $\hat{A} = \hat{x}$, this collapses to $\int x|\psi|^2\,dx$ — the centroid of the probability density. For momentum, $\hat{A} = \hat{p}$, it becomes $\int\psi^*(-i\hbar\partial_x\psi)\,dx$, and now the sign earns its pay.
...
Integrate against $\psi^*$. The term $i\hbar x/a^2$ is odd against the symmetric density $|\psi|^2$ and integrates to zero. What survives is $\hbar k_0 \int|\psi|^2\,dx = \hbar k_0$. A packet moving right with $k_0 > 0$ has $\langle p\rangle = \hbar k_0 > 0$.

Use $+i\hbar\partial_x$ — the wrong sign — and you get $-\hbar k_0 < 0$. Wrong direction. The minus sign in $\hat{p} = -i\hbar\partial_x$ is load-bearing, not a convention.
...
This is the spread of outcomes across many identically prepared copies. It is not uncertainty in any single measurement — it is the width of the histogram.
...
The product rule coughs up an extra $-i\hbar\psi$. First term minus second:
...
This is the **canonical commutation relation** — the one algebraic fact that divides quantum mechanics from classical mechanics. In classical mechanics, position and momentum Poisson-commute: $\{x, p\} = 1$, no $i\hbar$, no incompatibility. The canonical relation is the quantum version, and the $i\hbar$ does not come out. Everything that follows about uncertainty traces back to this single line.

Because $[\hat{x}, \hat{p}] \neq 0$, position and momentum share no common eigenbasis. A state with definite position (a Dirac delta in $x$) is infinitely spread in momentum; a state with definite momentum (a plane wave) is infinitely spread in position. The incompatibility has nothing to do with measurement — it is algebraic, written into the operators themselves.
...
Here is the theorem that ties the commutator to the uncertainty bound. No measurement is performed; no particle is disturbed. The proof is linear algebra.
...
Three moves. Define shifted operators $\hat{A}' = \hat{A} - \langle\hat{A}\rangle$ and $\hat{B}' = \hat{B} - \langle\hat{B}\rangle$, so that $\sigma_A^2 = \|\hat{A}'|\psi\rangle\|^2$ and $\sigma_B^2 = \|\hat{B}'|\psi\rangle\|^2$.

**Move 1** (Cauchy-Schwarz):

$$\sigma_A^2\sigma_B^2 = \|\hat{A}'|\psi\rangle\|^2\|\hat{B}'|\psi\rangle\|^2 \geq |\langle\hat{A}'\hat{B}'\rangle|^2.$$

**Move 2** (split the inner product): write $\hat{A}'\hat{B}' = \tfrac{1}{2}\{\hat{A}',\hat{B}'\} + \tfrac{1}{2}[\hat{A}',\hat{B}']$. The anticommutator $\{\hat{A}',\hat{B}'\}$ is Hermitian, so its expectation is real; the commutator $[\hat{A}',\hat{B}'] = [\hat{A},\hat{B}]$ is anti-Hermitian, so its expectation is purely imaginary. A real and an imaginary number add in quadrature:

$$|\langle\hat{A}'\hat{B}'\rangle|^2 = \frac{1}{4}\langle\{\hat{A}',\hat{B}'\}\rangle^2 + \frac{1}{4}|\langle[\hat{A},\hat{B}]\rangle|^2.$$

**Move 3** (throw away the non-negative anticommutator term for a lower bound):
...
Now plug in $\hat{A} = \hat{x}$, $\hat{B} = \hat{p}$, $[\hat{x},\hat{p}] = i\hbar$:

$$\sigma_x\,\sigma_p \geq \frac{1}{2}|i\hbar| = \frac{\hbar}{2}.$$

The **Kennard inequality** — proved by Kennard in 1927, set in this general algebraic frame by Robertson in 1929. It is a theorem, derived from the canonical commutation relation and Cauchy-Schwarz. No experiment appears anywhere in the proof. The bound is fixed by how the state is prepared, not by any measurement.

Notice what got discarded in Move 3: the anticommutator term $\tfrac{1}{4}\langle\{\hat{A}',\hat{B}'\}\rangle^2$, which is non-negative. Discarding it weakens the bound. Schrödinger (1930) kept it and got a sharper inequality. For the Gaussian the anticommutator term happens to vanish, so the two bounds coincide. For other states — the infinite square well, say — the Schrödinger bound is strictly tighter than Robertson. The exercises pursue this.
...
In Chapter 3 I got $\sigma_x\sigma_p = \hbar/2$ for the Gaussian. Let me run the same calculation on a different state — the infinite-square-well ground state — to show the bound is met but not saturated.

The state on $[0,L]$ is $\psi_1(x) = \sqrt{2/L}\,\sin(\pi x/L)$.

**Position mean.** By the symmetry of $\sin^2(\pi x/L)$ about $x = L/2$: $\langle x\rangle = L/2$.

**$\langle x^2\rangle$.** Integrating $x^2\sin^2(\pi x/L)$ over $[0,L]$ (use $\sin^2\theta = (1-\cos 2\theta)/2$ and integrate by parts twice):
...
**Momentum mean.** The ground state is a standing wave — an equal mix of $e^{i\pi x/L}$ (rightward) and $e^{-i\pi x/L}$ (leftward). Equal and opposite contributions cancel: $\langle p\rangle = 0$. Confirm by direct integral: the integrand $\sin(\pi x/L)\cdot\cos(\pi x/L) = \tfrac{1}{2}\sin(2\pi x/L)$ integrates to zero over a full period.
...
Why doesn't the square-well ground state saturate the bound? Saturation requires $\hat{A}'|\psi\rangle = i\lambda\hat{B}'|\psi\rangle$ for some real $\lambda$ — the Cauchy-Schwarz step must hold with equality, which demands the two vectors be proportional. For $\hat{A}' = \hat{x} - \langle x\rangle$ and $\hat{B}' = \hat{p} - \langle p\rangle$, this becomes a differential equation whose only normalizable solution is the Gaussian. The square-well ground state has hard walls forcing $\psi(0) = \psi(L) = 0$ — it cannot be Gaussian. So $\sigma_x\sigma_p$ sits strictly above $\hbar/2$.

Open `01-probability-explorer.html` from Chapter 3, select the infinite well, set $n = 1$, $L = 10$ nm. The ratio should read about $1.136$. That number is this calculation, done numerically.

---

## What the Uncertainty Principle Is Not About

There is a story told in many textbooks — Heisenberg's gamma-ray microscope — in which a photon used to locate an electron kicks it, disturbing its momentum in some uncontrollable way. The story is physically instructive. But it describes a different statement from the Kennard inequality.

The Kennard inequality is about *preparation*, not measurement. Prepare a million copies of the same state. Measure position on half, momentum on the other half. No copy is measured twice. No particle is kicked by any photon. And still $\sigma_x\sigma_p \geq \hbar/2$. The bound is set by the shape of $\psi$, before any measurement begins.

The microscope story describes an *error-disturbance* relation: measure position with precision $\epsilon$, and the act introduces a momentum disturbance $\eta$, with $\epsilon\cdot\eta$ obeying some bound. Ozawa formalized this in 2003 and proved the relation takes a different form from Kennard's. Erhart et al. tested both experimentally in 2012. They are different inequalities, with different mathematical forms and different physical content. Conflating them is the most stubborn conceptual error in introductory quantum mechanics.
...
The Robertson framework measures uncertainty through variance. But variance is not the only defensible measure of spread. The entropic uncertainty relations of Maassen and Uffink (1988) swap $\sigma_A^2$ for the Shannon entropy $H(A)$ of the measurement distribution, giving $H(x) + H(p) \geq \log(e\pi\hbar)$. Entropic bounds beat variance-based Robertson for non-Gaussian states and sit at the center of quantum information theory and cryptography. They are graduate material, but worth knowing they exist.

The error-disturbance relations — Ozawa (2003), tested by Erhart et al. (2012) and Rozema et al. (2012) — describe what happens when you measure one observable and ask how much that act disturbs a later measurement of the other. They demand weak measurements and quantum tomography to test. The conceptual line between preparation uncertainty (Robertson) and disturbance uncertainty (Ozawa-type) matters, and it is not yet standard in most undergraduate curricula.
