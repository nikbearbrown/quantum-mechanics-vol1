...
You have already stared at something on the screen without being told its name. A blue filled curve, drifting. An orange curve threaded through it, oscillating, dipping negative, indifferent to any rule about staying positive. You moved a slider and the blue shape spread. You probably felt something between recognition and bafflement, watching *something* you could not yet label, because the label was withheld. I will hand it over now — not as a definition to swallow, but by working out what the blob is *forced* to be if the theory is to hold together at all.

Begin with the plainest possible question: what is the blob's unit?

If the particle moves along a line marked off in nanometers, and the blob is a quantity plotted against that line, then the blob has units. Asking what units is already the right question, because the answer is not "dimensionless." The blob is not a probability. It is a probability *per nanometer* — a density. Like population per square kilometer, not a headcount. You cannot read "the particle is at position $x$" off the blob's value at a single point, any more than you can read a city's population off a density map at a single street address. To get a probability — a genuine dimensionless number between 0 and 1 — you must integrate over a region.
...
Max Born introduced this in 1926, in a paper on quantum scattering. His original draft made the probability proportional to $\psi$ itself — not $|\psi|^2$. He corrected it in a footnote added at the proof stage. That footnote is one of the most consequential edits in the history of physics. Born received the Nobel Prize in 1954 for the statistical interpretation it introduced.
...
The density $|\psi|^2$ can exceed 1 — it is per unit length, not a dimensionless probability. Confusing the density at a point with a probability is a unit error, and it is the single most common first mistake. Fix the distinction in your head now and you will sidestep an entire class of wrong answers for the rest of the course.

---

Now the second question: why is $\psi$ complex at all?

If what we measure is $|\psi|^2$, which is real and non-negative, why not use a real-valued function and dispense with the imaginary part? The answer is not convention. It is forced by the equation of motion.

The time-dependent Schrödinger equation is:

$$i\hbar \frac{\partial \psi}{\partial t} = \hat{H}\psi.$$

That $i$ on the left is structural, not decorative. Suppose $\psi$ were purely real. Then $\partial\psi/\partial t$ is real, and $i\hbar$ times a real thing is purely imaginary. The left side would be imaginary; the right side would be real (for real $\psi$ and real $V$). The equation would demand that a real quantity equal a purely imaginary one — impossible unless everything vanishes. The dynamics themselves drive $\psi$ into the complex plane. Strip out the imaginary part and you no longer have a theory.

You saw this on the screen. The orange curve was Re$\,\psi$. The gray dashed curve was Im$\,\psi$. Both were non-trivial; the imaginary part led the real part by a quarter cycle. Freeze the wave function, set Im$\,\psi = 0$, take one time step under the Schrödinger equation, and the imaginary part rebuilds itself immediately. The orange curve cannot live alone.
...
To see what the imaginary part is carrying, take the Gaussian wave packet with central momentum $\hbar k_0$:
...
Both oscillate; both carry the information stored in $k_0$. Now form the density:

$$|\psi|^2 = A^2\,e^{-x^2/a^2}.$$

The oscillations are gone. The density is a smooth Gaussian with no trace of $k_0$ anywhere in it. The momentum — the direction of travel, the central velocity — is invisible in $|\psi|^2$. It lives entirely in the phase, in the relationship between Re$\,\psi$ and Im$\,\psi$. Throw away the imaginary part and you can no longer tell a packet moving right from one moving left. They would share the same $|\psi|^2$, the same histogram of position measurements, in every experiment you could run.

This is not a mathematical nicety. It is the physical content of the imaginary part.

---

Here is something the blob was *not* doing, however much it looked as if it might be.

When the blue curve spread over time, it was tempting to read that as the particle physically expanding — billowing out in space like a gas. That reading is wrong, and the error matters. The particle, when you actually detect it, sits at one definite place. What spread was the *probability distribution* of where that place might turn out to be. Run the same experiment — same initial conditions, same Hamiltonian, same $\psi(x,0)$ — ten thousand times. Measure position each time. The histogram of results matches $|\psi|^2$. The spread of the histogram is the spread of the blob.

$\psi$ is not the particle. $\psi$ is the instruction set for computing histograms.

The blue blob spreading on the screen was saying: had you run this experiment ten thousand times and measured position at this late moment, the measurements would scatter across this wide distribution. It was a statement about many repetitions, not about the fate of a single particle.

That is what Born's footnote actually established. Not a new force, not a new field, not a new kind of matter — a new *connection* between a mathematical object and an experimental distribution. The connection is the rule: compute $|\psi|^2$, integrate, read off probabilities.

---

Because $|\psi|^2$ is a probability density and the particle must be found *somewhere*, the total has to equal one:

$$\int_{-\infty}^{\infty} |\psi(x,t)|^2\,dx = 1.$$

A wave function obeying this is normalized. In practice, wave functions often arrive without a normalization constant. To fix that: integrate $|\tilde{\psi}|^2$, take the square root, divide. Done.

The more interesting question is whether normalization survives the passage of time. If $\psi$ obeys the Schrödinger equation and you normalize it at $t=0$, does $\int|\psi|^2\,dx$ stay at 1 as it evolves? It had better — otherwise the probability of finding the particle anywhere would wander off from 1, which is incoherent. But "must be true" and "provably true" are different statements.

Differentiate $\int|\psi|^2\,dx$ with respect to time. Use the Schrödinger equation to replace $\partial_t\psi$ and $\partial_t\psi^*$. The potential-energy contributions cancel exactly — they show up with opposite signs and annihilate each other. Only the kinetic terms survive, and they assemble into a perfect spatial derivative:
...
The derivation has a practical dividend. In a numerical simulation, $\int|\psi|^2\,dx$ is a diagnostic. If it drifts from 1, something is broken — either the time-stepping scheme is non-unitary (explicit Euler will do exactly this) or probability is leaking out at the grid boundaries. The normalization indicator is a bug detector, and the conservation law you just proved is the standard it is judged against.

---

Born's rule gives you the probability distribution for position measurements. The average position over many experiments is the centroid of that distribution:
...
Momentum is less direct. In position space, momentum is not a function of $x$; you cannot write $\langle p\rangle = \int p\,|\psi(x)|^2\,dx$ and get anything sensible out. The Fourier transform supplies the link: define $\phi(p,t)$ as the Fourier transform of $\psi(x,t)$. Then $|\phi(p,t)|^2$ is the probability density for momentum measurements, and $\langle p\rangle = \int p\,|\phi|^2\,dp$. Translate this back to position space via Parseval's theorem and multiplication by $p$ turns into differentiation. The momentum operator in position space is:

$$\hat{p} = -i\hbar\frac{\partial}{\partial x}.$$

The sign is not up for negotiation. For a wave packet moving right with $k_0 > 0$, this formula delivers $\langle p\rangle = \hbar k_0 > 0$. Write $\hat{p} = +i\hbar\,\partial_x$ by mistake and you reverse every direction in the theory. The simulation catches it instantly: a packet sent rightward registers a negative average momentum.
...
Now measure both spreads. Define the widths of the position and momentum distributions in the usual statistical way:
...
Here is the cleanest reading of what it says. Prepare ten thousand identical copies of the same state $\psi$. Measure position on five thousand of them and record the standard deviation $\sigma_x$. Measure momentum on the other five thousand and record $\sigma_p$. No particle is measured twice. No measurement disturbs another. And still the product $\sigma_x\sigma_p$ cannot drop below $\hbar/2$.

The reason is Fourier analysis, not physical disturbance. A function narrow in $x$ must be broad in its Fourier transform, and the reverse. The Kennard inequality is the precise, rigorous statement of that unavoidable trade-off between a function and its transform — with $\hbar/2$ as the exact bound, not an order-of-magnitude guess.

A separate Heisenberg measurement-disturbance relation — which genuinely *is* about what happens when you measure position and then momentum on the same particle — has a different form, a different bound, and a different physical content. Ozawa formalized it in 2003; Erhart et al. tested it in 2012. The Kennard inequality and the measurement-disturbance relation are different animals. Conflating them is the most stubborn conceptual error in introductory quantum mechanics.
...
The product is exactly $\hbar/2$. Every other normalizable wave function gives something larger. That is why the Gaussian turns up everywhere in quantum mechanics: it is the tightest state available, the one that balances position and momentum uncertainty as evenly as the physics will allow.

The trade-off is right there in the formulas. $\sigma_x \propto a$; $\sigma_p \propto 1/a$. Squeeze the packet in space — smaller $a$ — and the momentum spread widens in proportion. The product is locked. This is not a limit of your equipment. You cannot engineer around it, because it follows from the mathematics of Fourier transforms, not from imperfect instruments.
...
Now the probability of catching the particle in $[-a, a]$:

$$P = \frac{1}{a}\int_{-a}^{a}e^{-2|x|/a}\,dx = \frac{2}{a}\int_0^{a}e^{-2x/a}\,dx = 1 - e^{-2} \approx 0.865.$$

About 86.5%. The interval $[-a,a]$ brackets the peak of the distribution, so this is reasonable. The remaining ~14% lives in the tails $|x| > a$.

One last detail. At $x = 0$, $|\psi(0)|^2 = 1/a$. For $a = 0.5$ nm this is 2 nm$^{-1}$ — a number larger than 1. Nothing is wrong. The density is measured in nm$^{-1}$ and is under no obligation to stay $\leq 1$. Expecting $|\psi|^2 \leq 1$ everywhere is a unit confusion, like expecting a population-density map never to exceed 1 person per square kilometer.

---

This wave function $e^{-|x|/a}$ is real-valued, so Im$\,\psi = 0$ everywhere, and the probability current $J = (\hbar/m)\,\mathrm{Im}(\psi^*\,\partial_x\psi)$ vanishes everywhere. A real wave function carries zero current. It cannot describe motion in a definite direction. To set the particle moving with central momentum $p_0$, multiply by $e^{ip_0 x/\hbar}$: the density $|\psi|^2$ is untouched, but $\langle p\rangle = p_0$. The density stays the same; the current changes; the physics changes. All the directionality lives in the phase.
