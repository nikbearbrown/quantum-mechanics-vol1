...
It is January 1926, and Erwin Schrödinger is holed up in a rented villa in Arosa, Switzerland, with a notebook, a mystery woman, and a problem. He has been reading de Broglie's 1924 thesis, the one claiming matter has wave properties. His question is the obvious one: if matter is a wave, what is its wave equation?

He tries several forms. He knows whatever he writes down must reproduce the Bohr energy levels of hydrogen — not as a postulate this time, but as a *consequence*. After weeks of dead ends he finds it, and within six months he submits four foundational papers to *Annalen der Physik*. At the heart of all of them sits

$$i\hbar\,\frac{\partial \Psi}{\partial t} = \hat{H}\,\Psi.$$

This is the time-dependent Schrödinger equation: a partial differential equation, first order in time, second order in space, and in general genuinely hard. But there is one case — the case where the potential does not depend on time — in which a single clever guess cracks it open and the hard PDE becomes a problem you can actually solve. That case covers nearly every system in this book: the infinite well, the harmonic oscillator, the hydrogen atom. All of them have static potentials. This chapter is about what happens when $V = V(x)$.
...
The left side carries only the time derivative; the right side mixes a spatial derivative with $V(x)$. They are coupled, and that coupling is what makes it a PDE.

The guess — the product ansatz — is to try $\Psi(x, t) = \psi(x)\,\phi(t)$. Substitute:

$$i\hbar\,\psi(x)\,\phi'(t) = \phi(t)\,\hat{H}\psi(x).$$

Divide through by $\psi(x)\phi(t)$:

$$i\hbar\,\frac{\phi'(t)}{\phi(t)} = \frac{\hat{H}\psi(x)}{\psi(x)}.$$

The left side depends only on $t$. The right side depends only on $x$. For two such expressions to be equal at every $x$ and every $t$, both must equal the same constant. Call it $E$.
...
Equation (4.1) is trivial: $\phi(t) = e^{-iEt/\hbar}$, a phase rotating at angular frequency $E/\hbar$. Universal form, indifferent to the potential or the energy level.

Equation (4.2) is the **time-independent Schrödinger equation** (TISE):

$$\boxed{-\frac{\hbar^2}{2m}\,\frac{d^2\psi}{dx^2} + V(x)\,\psi(x) = E\,\psi(x).}$$

This is an eigenvalue problem: find the functions $\psi$ on which $\hat{H}$ acts simply by multiplying by a constant. Those functions are the **stationary states** (or energy eigenstates); the constants $E$ are the **energy eigenvalues**. The time dependence has been factored out completely — a rotating phase clipped onto each state — and all the interesting physics now lives in the spatial equation.

---

## What the Constant $E$ Actually Means

The letter $E$ was not chosen at random. It is the energy, in a precise, operational sense. For any state $\Psi$, the expectation value of the Hamiltonian is

$$\langle \hat{H} \rangle = \int \Psi^*(x,t)\,\hat{H}\,\Psi(x,t)\,dx.$$

For a stationary state $\Psi_n = \psi_n(x)\,e^{-iE_n t/\hbar}$, plug in. The phase $e^{+iE_n t/\hbar}$ from the complex conjugate and the phase $e^{-iE_n t/\hbar}$ from the state multiply to 1. The eigenvalue equation $\hat{H}\psi_n = E_n\psi_n$ pulls $E_n$ out of the integral. Normalization makes $\int|\psi_n|^2\,dx = 1$. The result:

$$\langle \hat{H} \rangle = E_n.$$

And that is not merely the average energy. The variance is zero too: $\langle \hat{H}^2\rangle - \langle \hat{H}\rangle^2 = 0$. Every single energy measurement on a particle in state $\psi_n$ returns exactly $E_n$ — no spread, no uncertainty, no distribution. That is what it means to be an eigenstate of an operator. You are in a state of definite energy.

There is also a consistency check lurking here. If $E$ had a nonzero imaginary part, the factor $|e^{-iEt/\hbar}|^2 = e^{2\,\mathrm{Im}(E)\,t/\hbar}$ would drive the probability to grow or decay over time. Conservation of probability forbids it. Self-adjoint Hamiltonians — which is what real potentials and physical boundary conditions hand you — have only real eigenvalues. The $E_n$ are always real.
...
What "stationary" does *not* mean: the wave function itself is not time-independent. $\Psi_n$ is a complex function spinning in the complex plane at angular frequency $E_n/\hbar$. Its real part goes as $\psi_n(x)\cos(E_n t/\hbar)$, its imaginary part as $-\psi_n(x)\sin(E_n t/\hbar)$ — two oscillations 90 degrees out of phase, like the two components of a clock hand. The wave function is a clock. The probability density is the clock's shadow on the floor. The shadow holds still while the clock keeps spinning.

The simulation for this chapter makes the point visible. For a single eigenstate, the Re $\Psi$ and Im $\Psi$ panels animate while the $|\Psi|^2$ panel sits frozen. If $|\Psi|^2$ moves while only one eigenstate is populated, the phase is not canceling correctly and the code is wrong.

---

## Why the Laser Cavity Is the Same Problem

Walk into a basement laser lab. On the optical table is a cavity — two mirrors facing each other — with a gain medium between them. A pump beam goes in. A single, monochromatic, phase-coherent beam comes out.

Why specific frequencies and not others? Because the cavity sustains only the modes — the standing waves — that fit exactly between the mirrors. A wave that fails to vanish at both mirror surfaces interferes destructively with itself on every round trip and dies. A wave that does vanish at both surfaces builds up coherently, and that is the output you see.

The cavity is selecting, from a continuous range of possible frequencies, the discrete set that satisfies its boundary conditions. Quantization in quantum mechanics is the same phenomenon, at a smaller scale and with a different wave equation. The confining potential supplies the walls; only the spatial modes that meet the boundary conditions survive; those modes are the stationary states; their discrete frequencies are the discrete energies. The Schrödinger equation is the wave equation that tells you which modes fit.

---

## Global Phase and Relative Phase

A global phase — multiplying the entire wave function by $e^{i\alpha}$ for any real $\alpha$ — is unobservable. It cancels in every probability density $|\Psi|^2$ and in every expectation value. States are defined only up to a global phase.

The rotating phase $e^{-iE_n t/\hbar}$ of a stationary state is a global phase for that single state. It is unobservable. That is exactly why a stationary state is stationary — the rotating phase carries no information you can measure.
...
A weighted average of the eigenvalues, with weights $|c_n|^2$. The cross terms — the ones carrying the time dependence — all vanish by orthogonality of the eigenstates. The result is exactly time-independent. The probability distribution sloshes back and forth across the box while the total energy never budges. It is conservation of energy, spoken in the quantum language.
...
Suppose $\hat{H}$ has a complete, orthonormal set of eigenstates $\{\psi_n\}$ with eigenvalues $\{E_n\}$. Complete means any physically reasonable wave function can be written as a superposition of the $\psi_n$. Orthonormal means $\langle\psi_m|\psi_n\rangle = \delta_{mn}$. Both properties come free for the self-adjoint operators quantum mechanics uses — they follow from the spectral theorem of functional analysis, a mathematical theorem, not a new quantum postulate.

For the infinite well, the completeness of the eigenstates is the Fourier sine series theorem. Fourier proved it (for the heat equation) in 1822. The quantum application adds only the probabilistic interpretation and the phase rotation; the completeness is a century older than quantum mechanics. There is no quantum mystery here — only old mathematics in new clothes.
...
This is the complete general solution of the TDSE for any time-independent potential. Three steps: solve the eigenvalue problem for $\{\psi_n, E_n\}$; project the initial condition to find $\{c_n\}$; attach the rotating phases. Step one is the hard one. Steps two and three are a calculation.

Anyone who stops at step one has found the stationary states and missed everything that happens when the particle is not in one. Finding the eigenstates is not the end of the problem; it is the end of the setup.
