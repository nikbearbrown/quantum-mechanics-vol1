...
There is a moment in learning physics when the thing you have been deriving starts to feel real. For most students it does not arrive when they solve the harmonic oscillator analytically. It arrives when they type a potential into a computer, press a button, and watch the wave function appear.

This chapter engineers that moment on purpose. You are going to build a configurable one-dimensional Schrödinger solver. Hand it any potential $V(x)$ on a grid, and it will find the bound-state energies and eigenfunctions, or time-evolve an initial wave packet forward in time. Once it runs, you have a machine that makes the Schrödinger equation computable for any problem in one dimension.

But building the machine is the lesser half. The other half is making sure the machine is right. A program that produces plausible-looking output is not the same as a program that computes correct physics. The discipline of checking — running the solver on a problem whose answer you already know and demanding numerical agreement — is what separates physical simulation from numerical theater. This chapter teaches that discipline as explicitly as it teaches the algorithms.

By the end you will have a sandbox that reproduces every bound-state spectrum in this book, verified against the one result that checks everything at once: the infinite square well, whose energies satisfy $E_n = n^2\pi^2\hbar^2/2mL^2$ with no fitting parameters, no adjustable constants, and no excuses.
...
The sandbox runs in two modes. Before you write a line of code, get clear on what each one computes.

**Eigensolver mode.** Given a potential $V(x)$ on a spatial grid, find the discrete bound-state energies $E_n$ and the matching eigenfunctions $\psi_n(x)$. The output is a list of energy levels and wave-function shapes — the stationary states, the ones whose probability distribution does not change in time. Every analytic result in Chapters 4 through 10 is a special case of this one computation.

**Time-evolution mode.** Given an initial wave function $\Psi(x, 0)$ and a potential $V(x)$, propagate $\Psi(x,t)$ forward by solving the time-dependent Schrödinger equation $i\hbar\,\partial_t\Psi = \hat{H}\Psi$. The output is an animation of the probability density drifting, spreading, bouncing off walls, or tunneling through barriers.

Both modes ride the same infrastructure: $N$ uniformly spaced grid points $x_j = x_\text{min} + j\,h$ with $h = (x_\text{max} - x_\text{min})/(N-1)$, a complex-valued array of length $N$ for the wave function, and a real-valued array of length $N$ for the potential. Everything that follows is about what to do with those arrays.
...
On the grid, approximate the second derivative with the central-difference stencil:

$$\psi''(x_j) \approx \frac{\psi_{j+1} - 2\psi_j + \psi_{j-1}}{h^2}.$$

Impose this at every interior grid point at once and you get a matrix equation

$$\mathbf{H}\vec{\psi} = E\,\vec{\psi},$$

with $\mathbf{H}$ real, symmetric, and tridiagonal:

$$H_{jj} = \frac{\hbar^2}{mh^2} + V_j, \qquad H_{j,j\pm1} = -\frac{\hbar^2}{2mh^2}.$$

Write the kinetic hopping coefficient as $t_k = \hbar^2/(2mh^2)$. Then the diagonal is $2t_k + V_j$ and the off-diagonals are $-t_k$. With $V_j = 0$ everywhere, the matrix is pure kinetic energy — the coupling between neighboring grid points that lets the wave function propagate.

For a hard-wall problem, enforce $\psi_0 = \psi_{N-1} = 0$ by building only the $(N-2)\times(N-2)$ interior submatrix; the boundary rows and columns are simply dropped.

Diagonalize and you get $N-2$ eigenvalues and eigenvectors. The eigenvalues are the energies; the eigenvectors, each normalized so $\sum_j|\psi_j|^2 \cdot h = 1$, are the discretized wave functions. The central-difference approximation puts an error into the $n$-th eigenvalue scaling as $(nh/L)^2$ — for $N = 500$ points and the ground state, below $10^{-6}$ in fractional terms. More than good enough for any physical purpose here.
...
The number to keep in your head for debugging: the diagonal is $\hbar^2/(mh^2)$, which is $2t_k$, not $t_k$. The off-diagonal is $-t_k = -\hbar^2/(2mh^2)$. Confuse the two and your ground-state energy comes out a factor of two off.

---

## Two Paths to Eigenvalues

The matrix eigensolver finds every eigenvalue at once and is the right tool when you want the full spectrum — but it needs a linear-algebra library. There is an alternative that needs nothing past arithmetic.

**The Numerov shooting method** uses a sharper approximation of $\psi''$ to reach $O(h^6)$ errors — four orders better than central difference — from a single three-point recursion. Rewrite the TISE as $\psi'' = f(x)\psi$ where $f_j = (2m/\hbar^2)(V_j - E)$. The Numerov recursion steps the wave function forward one point at a time:

$$\psi_{j+1} = \frac{2\psi_j(1 - \frac{5}{12}h^2 f_j) - \psi_{j-1}(1 + \frac{1}{12}h^2 f_{j-1})}{1 + \frac{1}{12}h^2 f_{j+1}}.$$

Each step is a few multiplications and divisions. No matrix, no library.

The shooting strategy: guess an energy $E$, integrate from the left boundary with $\psi_0 = 0$, $\psi_1 = \epsilon$, and at the same time from the right with $\psi_{N-1} = 0$, $\psi_{N-2} = \epsilon$. At the midpoint an eigenvalue sits precisely where the logarithmic derivatives of the left and right solutions agree — equivalently, where a certain Wronskian determinant changes sign. Sweep $E$ upward, find the sign changes, and bisect to pin each eigenvalue to whatever precision you want.

The trade-off is plain: Numerov finds eigenvalues one at a time, wants careful bracketing, and is marginally more code than calling `math.eigs()`. For three to five eigenstates it is faster to implement and more instructive — you are integrating the Schrödinger equation directly rather than hiding behind a black box. For twenty or more states, load the library.

For this capstone, start with Numerov. If you need all eigenvalues at once, add math.js (from CDN) and call `math.eigs()` on the tridiagonal matrix. Both routes validate against the same benchmark.

---

## Time Evolution: Why the Stepper Choice Is Not Optional

The time-dependent Schrödinger equation is $i\hbar\,\partial_t\Psi = \hat{H}\Psi$. For a time-independent Hamiltonian the exact solution is $\Psi(t) = e^{-i\hat{H}t/\hbar}\Psi(0)$, and the operator $e^{-i\hat{H}t/\hbar}$ is unitary — it preserves the norm of the wave function exactly, for all time. Any numerical scheme that is not unitary is fundamentally wrong, because a non-unitary stepper either manufactures or destroys probability.

**Why explicit Euler is forbidden.** The explicit Euler update is $\Psi^{n+1} = \Psi^n - (i\Delta t/\hbar)\mathbf{H}\Psi^n$. The update matrix is $\mathbf{I} - (i\Delta t/\hbar)\mathbf{H}$, with eigenvalues $1 - i\Delta t E_k/\hbar$ for each energy eigenvalue $E_k$. The modulus of each is $\sqrt{1 + (\Delta t E_k/\hbar)^2} > 1$. Every energy component grows exponentially at every step. The normalization indicator climbs past 1 within fifty steps and the simulation blows up. The indicator in the sandbox corner catches this at once — if it reads above 1.001 after ten steps, the time-stepper is wrong. Explicit Euler is unconditionally unstable for the Schrödinger equation, and you must never use it.
...
This is the Cayley approximation to the exact propagator. Because $\mathbf{H}$ is Hermitian, the left and right matrices are complex conjugates of each other, and the update is exactly unitary — normalization holds at every step regardless of $\Delta t$. The scheme is second-order in both time and space, unconditionally stable, and a natural fit for hard-wall boundary conditions because the tridiagonal structure of $\mathbf{H}$ carries straight into the linear system. Each step solves a tridiagonal system, which the Thomas algorithm dispatches in $O(N)$ operations.

### Split-Step Fourier

For free space or slowly varying potentials on a large periodic domain, the split-step Fourier method is faster and spectrally accurate in space. The Hamiltonian splits as $\hat{H} = \hat{T} + \hat{V}$. Since $\hat{T}$ and $\hat{V}$ do not commute, the exact propagator $e^{-i\hat{H}\Delta t/\hbar}$ cannot be factored exactly. The Trotter-Suzuki decomposition gives a second-order approximation:

$$e^{-i\hat{H}\Delta t/\hbar} \approx e^{-i\hat{V}\Delta t/2\hbar}\,e^{-i\hat{T}\Delta t/\hbar}\,e^{-i\hat{V}\Delta t/2\hbar}.$$

The algorithm per step: multiply $\psi_j$ pointwise by $e^{-iV_j\Delta t/2\hbar}$; Fourier transform; multiply $\hat{\psi}_m$ pointwise by $e^{-i\hbar k_m^2\Delta t/2m}$; inverse Fourier transform; multiply once more by $e^{-iV_j\Delta t/2\hbar}$. Each of the three multiplications applies a phase of modulus exactly 1, so the method is exactly unitary — normalization is machine-precision exact.

There is one mandatory detail about the FFT $k$-grid that wrecks any implementation that ignores it. After an FFT of a length-$N$ array, the output index $m$ is *not* the physical wave vector. The correct mapping is

$$k_m = \frac{2\pi}{Nh}\times\begin{cases} m & m < N/2 \\ m - N & m \geq N/2 \end{cases}$$

The second half of the FFT output (indices $N/2$ through $N-1$) corresponds to *negative* wave vectors. Apply the kinetic phase using the raw index $m$ instead of the physical $k_m$ and you hand the wrong kinetic energy to every negative-momentum component. The simulation looks fine for the first few steps — the error lives in the high-frequency tails — and then gradually rots the whole wave function. The fix is five lines, and the CLAUDE.md amendment below spells it out.
...
Building the sandbox is the smaller job. The larger one is running the validation suite and confirming the simulation computes correct physics. Here is what that means in practice.

**Units.** Every displayed quantity must carry the right units. This is not cosmetics. A program that displays energy in the wrong units is computing the wrong physics. Pick one system — SI with distances in nanometers and energies in electron-volts works well — label every axis, and stay consistent. Sanity check: for an infinite square well of width $L = 1$ nm and an electron, the ground-state energy is $E_1 = \pi^2\hbar^2/2m_eL^2 \approx 0.376$ eV. If your code reports 376 eV or 0.000376 eV, there is a units error.

**Normalization.** For every eigenstate the solver hands back, verify
$$\sum_{j=0}^{N-1}|\psi_j|^2 \cdot h = 1.000 \pm 0.001.$$

Mind the $h$ weighting. Some eigensolvers return unit-norm vectors in the Euclidean sense ($\sum|\psi_j|^2 = 1$) rather than the physics sense ($\sum|\psi_j|^2 h = 1$). To convert, divide the eigenvector by $\sqrt{h}$. Forget the $h$ and your normalization integral reads $1/h$ instead of 1 — for $h = 0.004$ nm, a reading of 250 instead of 1. The corner indicator catches this immediately.

During time evolution the same indicator must stay within $\pm 0.001$ of $1.000$ at every animation frame. Drift above 1 means the stepper is manufacturing probability; drift below 1 means it is destroying it. Both are wrong.
...
Report a comparison table: for $n = 1, 2, 3, 4, 5$, the analytic energy, the numerical energy, and the fractional error. That error should be below $10^{-5}$ for $n = 1$ and below 1% for $n = 10$ with $N = 500$.

There is a dimensionless check that sidesteps units entirely: the ratios $E_n/E_1$ should be exactly $n^2$. Confirm $E_2/E_1 = 4.000$, $E_3/E_1 = 9.000$, $E_4/E_1 = 16.000$. If those hold to three decimals, the eigenvalue algorithm is working whether or not you have the right value for $\hbar$ in electron-volts — the ratios depend only on the numerics, not on any physical constant.
...
**Harmonic oscillator.** Set $V(x) = \frac{1}{2}m\omega^2 x^2$ on a grid wide enough to hold the first ten states. The analytic spectrum is $E_n = \hbar\omega(n + \frac{1}{2})$ — equally spaced levels, ground state at $\hbar\omega/2$. Verify: the level spacing is uniform to within 1%; the ground-state wave function is Gaussian; the ground-state uncertainty product $\sigma_x\sigma_p = \hbar/2$ (the oscillator ground state saturates the Kennard bound). That last check connects straight back to Chapter 9 and closes a loop that has been open since Chapter 1.

**Free-particle time evolution.** Set $V = 0$. Initialize a Gaussian wave packet with center $x_0$, width $a$, mean wavenumber $k_0$. The analytic evolution gives a centroid at $x_0 + (\hbar k_0/m)t$ and a width $\sigma(t)^2 = a^2/2 + \hbar^2t^2/(2m^2a^2)$. Run the simulation and compare. Both centroid and width should match the analytic formula to within 1% over several spreading times.

**Energy conservation.** Under time evolution with any time-independent $V(x)$, compute $\langle\hat{H}\rangle = \sum_j\Psi_j^*(t)(\mathbf{H}\vec{\Psi})_j \cdot h$ each frame. It must not drift by more than 0.1% over the window. If it grows, the stepper is not unitary.

---

## What Each Previous Chapter Built

The sandbox does no new physics. Every feature is a tool you already own.

The Born rule from Chapter 3 is why you display $|\Psi|^2$ rather than $\Psi$, and why the normalization integral must equal 1. The TISE from Chapter 4 is what the matrix eigensolver solves. The argument that quantization falls out of boundary conditions — made for the infinite well in Chapter 5 — reproduces itself automatically in the matrix formulation: Dirichlet conditions enforce $\psi_0 = \psi_{N-1} = 0$, and the only solutions are the discrete eigenvectors. The harmonic oscillator spectrum of Chapter 7 comes back numerically by setting $V_j = \frac{1}{2}m\omega^2 x_j^2$. The Gaussian wave-packet dynamics of Chapter 8 — group velocity, spreading, the $1/\sigma_0^2$ spreading rate — play out in the time-evolution panel. The operators and expectation values of Chapter 9 give $\sigma_x$, $\sigma_p$, and $\langle\hat{H}\rangle$ from the numerical eigenstates. The tunneling of Chapter 6 shows up when you time-evolve a packet against a finite barrier and watch the transmitted fraction emerge on the far side.

The synthesis is the whole point. You are not building anything new. You are building one machine that runs everything you already know, and then proving it agrees with what you already know.
...
Every failure mode below has a specific symptom in the normalization indicator or the validation table. Learn to read the indicator before you debug anything else.

**$h$ vs. $h^2$ in the kinetic coefficient.** Write `h` instead of `h*h` in the denominator of $t_k$ and the kinetic energy is wrong by a factor of $1/h$. The ground-state energy scales as $1/N$ instead of $1/N^2$. The ratio $E_2/E_1$ is still 4 (the error is uniform across modes), but $E_1$ itself is completely wrong. The validation table catches this on the first line.

**Unnormalized eigenvectors.** math.js returns eigenvectors normalized Euclidean-style: $\sum_j|\psi_j|^2 = 1$. The physics normalization is $\sum_j|\psi_j|^2 h = 1$. For $h = 0.004$ nm the Euclidean-normalized vector has physics-norm $1/h = 250$. Multiply by $1/\sqrt{h}$ to fix it. Symptom: the indicator reads $250$ or $0.004$ instead of $1$.

**Wrong FFT $k$-ordering.** The second half of the FFT output wraps to negative wave vectors. Applying the kinetic phase with the raw index $m$ gives the wrong energy to every negative-momentum component. Symptom: time evolution looks correct for ten steps, then grows an oscillation at the grid scale. Fix: use the $k_m$ formula above.

**Explicit Euler.** Normalization climbs above 1 within fifty steps; the indicator flags it at step ten. Fix: Crank-Nicolson or split-step. There is no scenario in which explicit Euler is acceptable for the Schrödinger equation.

**Grid too narrow.** For the harmonic oscillator, if the grid does not reach at least $\pm 5 x_0$ (with $x_0 = \sqrt{\hbar/m\omega}$ the characteristic length), the wave functions are clipped at the boundary and the eigenvalues come out wrong. Symptom: eigenvalues slightly too high, wave functions visibly hitting the edge. Fix: widen the grid.

**Spurious reflections in time evolution.** The wave packet reaches the grid edge and reflects, producing interference that has nothing to do with the physics. Fix: make the grid five to ten packet-widths wider than necessary, so the packet never reaches the boundary within the simulation window. Alternatively, add a complex absorbing potential $-i\gamma(|x| - x_\text{abs})^2$ near both edges — but understand that the normalization indicator will then decrease over time, which is correct physics (probability flowing out of the domain), not a bug.

---

## What Comes After

The sandbox as built is one-dimensional and non-relativistic. The physics it cannot handle points directly at what comes next.

For periodic potentials — an array of identical wells, a crystal lattice — the eigenstates are not bound states pinned in one well. They are Bloch waves spanning the whole lattice, and the spectrum forms bands separated by gaps. The tridiagonal matrix still works, but you need periodic boundary conditions (making it circulant) and Bloch's theorem to read the results. That is the band-structure problem, the content of solid-state physics.

For two dimensions, the Hamiltonian matrix becomes $(N_x N_y) \times (N_x N_y)$. For $N_x = N_y = 500$ that is a $250000 \times 250000$ matrix no browser can fully diagonalize by any available method. Iterative eigensolvers — Lanczos, Arnoldi — find the lowest few eigenvalues without ever building the full matrix. Time evolution in 2D via split-step works cleanly with two sequential FFT passes. The infrastructure is unchanged from the 1D version; the algorithms compose.

For relativistic particles, $\omega = \sqrt{(\hbar k)^2 c^2 + m^2c^4}/\hbar$ replaces the free-particle $\omega = \hbar k^2/2m$. The dispersion is no longer quadratic; group and phase velocities relate differently; and for fermions the wave function carries spinor components that the Dirac equation mixes. The split-step method still applies — the kinetic phase in step 3 just uses the relativistic dispersion — but the physics of the output is richer.

The harder truth is that most quantum-mechanics problems in research are neither one-dimensional nor exactly solvable. The hydrogen atom, the helium atom, the benzene molecule — none of their many-body Hamiltonians have clean analytic solutions. The tools in this sandbox — finite-difference discretization, matrix diagonalization, unitary time evolution — are the foundation of every serious quantum simulation code in existence. Density functional theory, quantum Monte Carlo, tensor network methods: all of them solve discretized Schrödinger equations, all need unitary time steppers, and all validate against exactly the benchmarks in this chapter.

You have been doing quantum mechanics in one dimension with a single electron. The machines that design semiconductor devices, model drug-target binding, and simulate quantum computers do quantum mechanics in thousands of dimensions with millions of electrons. The logic is the same. The scale is the difference.
