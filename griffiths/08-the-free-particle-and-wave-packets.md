...
Begin with the most elementary problem in quantum mechanics. No potential anywhere — a particle drifting through empty space. You know how to write down the Schrödinger equation, and you have every reason to expect a tidy answer.

Inside a region where $V = 0$, the time-independent equation is

$$-\frac{\hbar^2}{2m}\frac{d^2\psi}{dx^2} = E\psi,$$

and the solutions arrive at once: $\psi_k(x) = Ae^{ikx}$ with $k = \sqrt{2mE}/\hbar$. Every positive real $k$ works, every momentum $p = \hbar k$ is a legitimate eigenstate. The time-dependent version is
...
Constant. Everywhere. The probability density at $x = 0$ equals the density at $x = 10^{26}$ meters. Integrate over all of space: $\int_{-\infty}^\infty |A|^2\,dx = \infty$. The plane wave cannot be normalized on the whole real line for any nonzero $A$. A free particle with perfectly sharp momentum $p = \hbar k$ is everywhere at once — smeared uniformly across all of space, with no preference for any point.

This is not a glitch to be patched. It is the uncertainty principle in plain view. Sharp momentum means $\sigma_p = 0$, and the Kennard inequality $\sigma_x\sigma_p \geq \hbar/2$ then forces $\sigma_x = \infty$. Non-normalizability is just infinite position uncertainty wearing a different hat.

So what does one do with a free particle? Build something physically realizable: a **wave packet**, a superposition of plane waves with nearby momenta arranged to interfere constructively in one region and cancel everywhere else. The result is a localized, normalizable object that moves and spreads in a way that meets classical mechanics — in the right limit.
...
The general solution of the free-particle Schrödinger equation is a superposition over all plane waves:
...
Hand the machine any initial wave function: Fourier-transform to get $\phi(k)$, attach the phase $e^{-i\omega(k)t}$ to each component, and transform back. Free-particle time evolution is exact and complete in this form.

Why is the packet normalizable when none of its ingredients are? If $\phi(k)$ clusters near $k_0$ with spread $\Delta k$, the Fourier theorem guarantees that $\Psi(x,0)$ is localized with spatial spread $\Delta x \sim 1/\Delta k$, and a localized function is normalizable. The plane waves annihilate one another everywhere except near the packet's center. Destructive interference accomplishes what no single wave could.

The physical reading of $\phi(k)$: by the Born rule in momentum space, $|\phi(k)|^2\,dk$ is the probability of finding momentum between $\hbar k$ and $\hbar(k+dk)$. And here is the crucial fact: $|\phi(k)|^2$ is **time-independent** — the momentum distribution never changes during free propagation. The phase of $\phi(k)$ evolves as $e^{-i\omega(k)t}$, but its modulus stays put. Whatever momentum distribution you set up, you keep forever. The packet spreads in position; it does not spread in momentum.
...
A single plane wave $e^{i(kx-\omega t)}$ carries a crest — a surface of constant phase — at $kx - \omega t = \text{const}$. That crest moves at
...
The classical velocity is $v_{cl} = p/m$. The phase velocity is exactly half of it. The crests travel at half the speed you would classically pin on the particle. Something is off — or rather, the phase velocity was never the right thing to watch.

The particle is not a single crest. The particle is the *envelope* — the blob of probability. To find how the envelope moves, take a packet sharply peaked at $k_0$ and Taylor-expand the dispersion relation:

$$\omega(k) = \omega_0 + \omega_0'(k-k_0) + \tfrac{1}{2}\omega_0''(k-k_0)^2 + \cdots$$

Substitute into the superposition integral. The factor $e^{i(k_0 x - \omega_0 t)}$ pulls out as a carrier wave, and what remains, integrated over the envelope, becomes a function of $x - \omega_0' t$. The envelope is centered where that combination vanishes, at $x = \omega_0' t$. The **group velocity** is
...
That is the classical velocity. The packet's center of mass moves exactly as Newton's first law says it should. In terms of expectation values of position, the free-particle quantum packet is classical.

Compare the two speeds:

$$\frac{v_g}{v_{ph}} = \frac{\hbar k_0/m}{\hbar k_0/2m} = 2.$$

The group velocity is exactly twice the phase velocity. So in the simulation: the envelope (the $|\Psi|^2$ blob) advances at $v_g$, while the individual crests inside it — the wiggles of Re $\Psi$ — advance at $v_{ph} = v_g/2$. The envelope outpaces its own internal oscillations. Crests keep entering the packet from behind and exiting at the front. You can watch it happen. It is no artifact; it is what the mathematics predicts, and experiment confirms it.

The phase velocity carries no direct physical meaning for particle motion. It is not where the particle is, not how fast the particle moves, not a speed any detector in the path would register. The group velocity is the physical speed. The phase velocity is an internal property of the wave structure.
...
Nonzero. Different Fourier components travel at slightly different group velocities — higher-$k$ components faster than lower-$k$ ones. Over time the components pull apart and the packet spreads.

For a Gaussian initial state — the natural choice, since it saturates the uncertainty bound and yields an exactly doable integral — the spreading comes out in closed form. Take

$$\Psi(x,0) = \left(\frac{1}{\pi\sigma_0^2}\right)^{1/4}\exp\!\left(-\frac{x^2}{2\sigma_0^2}\right)e^{ik_0 x},$$

a Gaussian of width $\sigma_0$ centered at the origin with mean wavenumber $k_0$. Its Fourier transform is also Gaussian:

$$\phi(k) = \left(\frac{\sigma_0^2}{\pi}\right)^{1/4}\exp\!\left(-\frac{\sigma_0^2(k-k_0)^2}{2}\right),$$

with momentum-space width $\Delta k = 1/(2\sigma_0)$, giving $\sigma_x(0)\,\sigma_p = \sigma_0 \cdot \hbar/(2\sigma_0) = \hbar/2$. The Gaussian begins right at the uncertainty bound — minimum uncertainty, as tight as quantum mechanics permits.

Attach $e^{-i\omega(k)t}$ and do the Gaussian integral by completing the square. The result:
...
The packet stays Gaussian for all time, but its width grows. A few things to read off at once.

The center sits at $x = v_g t = (\hbar k_0/m)t$ — classical motion, precisely. The momentum distribution $|\phi(k)|^2$ never appears, because it has not changed. Only the position distribution has spread.

At $t = 0$, $\sigma_x\sigma_p = (\sigma_0/\sqrt{2}) \cdot (\hbar\Delta k\sqrt{2}) = \hbar/2$. For $t > 0$, $\sigma_x$ grows while $\sigma_p$ holds fixed, so $\sigma_x\sigma_p > \hbar/2$: the packet drifts away from minimum uncertainty as it spreads.

The spreading is caused by **dispersion** — $d^2\omega/dk^2 \neq 0$ — not by the uncertainty principle. The uncertainty principle constrains the state at each instant; it says nothing about dynamics. A light pulse in vacuum has $\omega = ck$, so $d^2\omega/dk^2 = 0$, and it propagates without spreading at all. A coherent state of the harmonic oscillator (Chapter 7) also refuses to spread, despite living in a dispersive system — there the restoring potential cancels the dispersion exactly. The free particle has no potential, no restoring force, and the spreading is unavoidable.

Define the **doubling time** — when $\sigma_x$ grows by a factor of $\sqrt{2}$ — by setting $\sigma(t)^2 = 2\sigma_0^2$:

$$t_{2x} = \frac{2m\sigma_0^2}{\hbar}.$$

It scales as $\sigma_0^2$: a packet twice as wide takes four times as long to double. And it is independent of $k_0$ — the mean momentum sets where the packet goes, not how fast it spreads. A fast packet and a slow packet of equal initial width spread at identical rates.
...
An electron is released from a quantum dot of diameter 10 nm, initially localized to $\sigma_x(0) = 2$ nm. The confining potential is switched off. How long until the packet's width doubles?
...
About 70 femtoseconds. An ultrafast pump-probe experiment resolves 10–100 fs, so this spreading is observable. The electron does not ooze outward over microseconds; it smears across the quantum dot in less time than light takes to cross a human hair.

Check the limiting cases. Halve the dot — $\sigma_x(0) = 1$ nm — and the doubling time drops to about 17 fs, four times shorter, because $t_{2x} \propto \sigma_0^2$. Double the dot to 4 nm and the doubling time grows to about 280 fs. Tighter initial confinement means a broader momentum distribution, which means faster dispersion of the Fourier components.

One more check: the mean momentum $k_0$ is nowhere in $t_{2x}$. An electron ejected from the dot at 1 eV and one ejected at 100 eV, both starting at the same width, double their spatial width at the same moment. Translation speed is set by $k_0$; the spreading rate is set entirely by $\sigma_0$ and $m$.

---

## The Scales That Make Quantum Mechanics Invisible

Feed a 1 mg grain of sand into the same formula, localized to $\sigma_x(0) = 1\,\mu\text{m}$:

$$t_{2x} = \frac{2\times10^{-6}\,\text{kg}\times(10^{-6}\,\text{m})^2}{10^{-34}\,\text{J\,s}} \approx 2\times10^{16}\,\text{s.}$$

That is roughly $6\times10^{8}$ years — hundreds of millions of years, far past any lab experiment. The quantum spreading is present in principle and indistinguishable from zero in practice. Quantum mechanics has no separate rule for large objects; it uses the same formula, and the formula returns an absurd timescale. The classical limit is not a philosophical choice. It is a number.

The dividing line is the ratio of the doubling time to whatever timescale you care about. For an electron in an atom, $t_{2x}$ runs from femtoseconds to attoseconds — quantum effects rule completely. For a marble, $t_{2x}$ outlasts the age of the universe — classical mechanics is exact for every practical purpose. The crossover between the two is smooth and computable, not mysterious.
...
The free particle is the cleanest test of the Fourier structure of quantum mechanics, and it teaches three things that carry into every harder problem.

First: energy eigenstates are not always normalizable. The infinite square well's eigenstates were localized by the walls; the free particle's are not. Physical states are always normalizable, but they are not always energy eigenstates. A physical free particle is a superposition — a wave packet — not a plane wave.

Second: time evolution is a phase rotation in Fourier space. Each $\phi(k)$ component picks up the factor $e^{-i\omega(k)t}$, and the modulus $|\phi(k)|^2$ is untouched. So the momentum distribution is conserved for the free particle — it cannot change with no force to change it. The classical counterpart is conservation of momentum in the absence of a force. The quantum statement is the same fact, expressed as the constancy of $|\phi(k)|^2$.

Third: the relationship between localization and momentum spread is exact and quantitative. To pin a particle within $\Delta x$, you must superpose momenta over a range $\Delta p \sim \hbar/\Delta x$. Tight localization demands broad momentum content; broad momentum content means the components travel at a range of speeds and separate quickly. You cannot localize a particle and also stop it from spreading — the two demands are in direct conflict. The uncertainty principle is not a shortcoming of measurement technology; it is a structural property of waves.

The Fourier relation $\Delta x \cdot \Delta k \sim 1$ is a theorem of harmonic analysis, proved in the nineteenth century for entirely classical reasons. Quantum mechanics did not invent it. What quantum mechanics did was identify $p = \hbar k$ and read $|\phi(k)|^2$ as a probability distribution. The mathematics of wave packets was already on the shelf; what was new was the physical interpretation of what, exactly, was waving.
