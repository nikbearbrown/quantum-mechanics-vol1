...
In 1993, physicists at IBM's Almaden Research Center spent several days nudging individual iron atoms across a copper surface with the tip of a scanning tunneling microscope. One atom at a time, they assembled 48 of them into a ring — a structure they named a quantum corral — and then imaged the electron density trapped inside. What appeared was not a smooth puddle of charge. It was a bull's-eye of concentric rings, a standing-wave pattern in the probability density, exactly what the Schrödinger equation predicts for an electron confined to a circular domain.

Crommie, Lutz, and Eigler did not put the rings there. The walls did. Confine an electron and it can occupy only the modes that fit the container. The modes look like standing waves because they are standing waves. The allowed energies are discrete because only certain spatial frequencies satisfy the geometry at the boundaries. The quantization was not assumed, postulated, or slipped in by hand. It dropped out of the mathematics of confinement.

This chapter works through the one-dimensional version of the same argument — the infinite square well — where the geometry is plain enough to carry the derivation all the way through in closed form, from the Schrödinger equation to the energy spectrum to the dynamics of superpositions. The aim is not to memorize formulas. It is to see, step by step, exactly where the discreteness comes from.
...
A guitar string fixed at both ends vibrates at a fundamental frequency and its integer harmonics. The reason is purely geometrical: a mode survives only if it vanishes at both endpoints. A half-wavelength fits. A full wavelength fits. Three half-wavelengths fit. But one-and-a-third half-wavelengths cannot vanish at both walls at once — it contradicts itself at the boundary and dies. The surviving frequencies are the ones that fit, and the fitting condition is discrete.

The analogy with a quantum particle in a box runs this deep and then halts. For the guitar string, the modes are literal physical displacements of a medium. For the electron, the "wave" is the wave function $\psi(x)$ — a complex-valued function whose squared modulus gives the probability of finding the particle at position $x$. Nothing is vibrating. The wave is not a wave in space; it is a wave of probability amplitude in the space of possible positions. The boundary conditions are the same mathematical structure, but the physical meaning of what is waving could hardly be more different.
...
Where the potential is infinite, the wave function must vanish. This is not an extra assumption; it is what $V = \infty$ forces in the time-independent Schrödinger equation (TISE). If $\psi \neq 0$ where $V = \infty$, the equation $-(\hbar^2/2m)\psi'' + V\psi = E\psi$ cannot hold: the left side would be infinite while the right side is finite. So the wave function is zero outside the well, and continuity of $\psi$ then demands:

$$\psi(0) = 0, \qquad \psi(L) = 0.$$

These are the boundary conditions. They are the walls, written in the language of the wave function.

Inside the well, $V = 0$, and the TISE is simply

$$-\frac{\hbar^2}{2m}\frac{d^2\psi}{dx^2} = E\psi.$$

Now ask: which values of $E$ are allowed?
...
**Can $E$ be negative?** If $E < 0$, write $\kappa^2 = -2mE/\hbar^2 > 0$. The equation becomes $\psi'' = \kappa^2\psi$, with general solution $\psi = Ae^{\kappa x} + Be^{-\kappa x}$ — a sum of real exponentials. That cannot vanish at both $x = 0$ and $x = L$ unless $A = B = 0$, which is no particle. If $E = 0$, the equation is $\psi'' = 0$, so $\psi = ax + b$; the boundary conditions kill both constants. So $E < 0$ and $E = 0$ are out. Only $E > 0$ yields a non-trivial solution.
...
At this stage any positive $k$ is allowed. No quantization yet.

**Apply the boundary condition at $x = 0$.** Substituting: $\psi(0) = A\sin(0) + B\cos(0) = B = 0$. The cosine term dies. The solution reduces to $\psi(x) = A\sin(kx)$. Still no quantization.

**Apply the boundary condition at $x = L$.** Now $\psi(L) = A\sin(kL) = 0$. Either $A = 0$ — no particle, discard — or

$$\sin(kL) = 0.$$

This single equation is where quantization walks in. A sine vanishes only at integer multiples of $\pi$. So $kL = n\pi$ for $n = 1, 2, 3, \ldots$ The continuous set of all positive real $k$ has collapsed to a discrete ladder indexed by a positive integer.

This is not a postulate. It is not an assumption about angular momentum or a new law bolted on to patch a problem. It is the only way a sine wave can vanish at both walls simultaneously. Bohr postulated quantization in 1913; Schrödinger derived it in 1926, from a differential equation and two boundary conditions.
...
The energies scale as $n^2$. The first few ratios: $E_1 : E_2 : E_3 : E_4 = 1 : 4 : 9 : 16$. Nothing lives between these values. For $n = 0$: $k_0 = 0$ gives $\psi = A\sin(0) = 0$ — no particle, excluded. For negative $n$: $\sin(-n\pi x/L) = -\sin(n\pi x/L)$, which gives the same probability density as positive $n$ — the same physical state up to an unobservable sign.
...
So $E_2 \approx 1.51$ eV, $E_3 \approx 3.39$ eV. These are energies on the scale of chemistry and optics — visible photons carry roughly 2 eV, and room-temperature thermal energy is $k_BT \approx 0.025$ eV. The level spacings dwarf the thermal fluctuations, so quantum effects are not merely relevant here; they dominate.
...
The eigenstates satisfy $\langle\psi_m|\psi_n\rangle = \delta_{mn}$ — they are orthonormal. The proof needs a single trigonometric identity:
...
No numerical eigensolver. No Gram-Schmidt. Exact, analytic, and a direct consequence of classical Fourier analysis. Fourier established it in 1822 for the heat equation — more than a century before quantum mechanics existed. The mathematics of standing waves on a bounded interval was already fully in hand; what quantum mechanics added was the physical interpretation.
...
The Heisenberg uncertainty principle says why. If $E = 0$, the kinetic energy is zero, the momentum is zero, and so the momentum uncertainty $\sigma_p = 0$. But the particle is confined to a box of width $L$, so its position uncertainty obeys $\sigma_x \leq L$. The Kennard inequality $\sigma_x\sigma_p \geq \hbar/2$ then demands $L \cdot 0 \geq \hbar/2$, which is false. A particle with zero momentum uncertainty cannot also be confined in space. The uncertainty principle forbids $E = 0$.

This is not a quirk of quantum mechanics — it is a consequence of the wave nature of matter. A guitar string stretched flat has zero energy and zero frequency. The lowest non-trivial mode has positive frequency, positive energy. The same logic governs any wave confined to a finite domain.

Quantitatively: for the ground state, $\sigma_p = \pi\hbar/L$ (from $\langle p^2\rangle = \pi^2\hbar^2/L^2$ with $\langle p\rangle = 0$ by symmetry), and $\sigma_x \approx 0.181L$. So $\sigma_x\sigma_p \approx 0.568\hbar \approx 1.136 \times (\hbar/2)$ — about 14% above the minimum the uncertainty principle allows. The ground state of the infinite well sits close to the most tightly localized state quantum mechanics permits.
...
Dynamics appear only when you superpose two or more eigenstates of different energy. Take the equal-weight combination of the first two:
...
The cross integral wants integration by parts. Expand $\psi_1\psi_2$ with the product-to-sum identity, integrate $\int_0^L x\cos(n\pi x/L)\,dx$ by parts, and the result is
...
Constant. The probability is sloshing; the energy is not. The $|c_n|^2$ — the weights on each eigenstate — are fixed by the initial state and never change. What evolves is the relative phase between the terms, which modulates the interference pattern in $|\Psi|^2$ but leaves the energy budget untouched.
