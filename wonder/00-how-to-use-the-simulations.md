...
Before this book hands you a single new piece of physics, you are going to get a blue wave packet drifting across your browser, animating, alive. You will build three small Markdown files (`CLAUDE.md`, `DESIGN.md`, `PROJECT.md`), feed Claude a four-move prompt — Show, Say, Constrain, Verify — and out comes the simulation. Think of this chapter as installing the workbench, not doing the carpentry. Every later exercise reuses these three files. The actual physics the packet is performing — dispersion, group velocity, spreading — arrives, properly explained, in Chapters 1 and 2. Today it is just the thing we practice on.
...
Let me show you a picture you are going to meet over and over in this book, and then admit that I am not going to explain it yet. A blue filled curve drifts to the right. Orange ripples slide backward through it, against the drift, like foam on a wave that runs the wrong way. In the corner sits a number: `1.000`. What does any of it mean? Nothing, yet — and that is on purpose. Chapter 1 will tell you. The point of meeting it *now*, before you have a single equation, is this: quantum mechanics is brutally hard to learn from sentences alone. The objects have no cousin in the everyday world. You cannot pick one up, tilt it, prod it. So this book gives you a third thing to study alongside the words and the math — a simulation you built yourself, running on physics you can derive, that talks back when you move a slider.

So here is the honest framing. The problem this chapter solves is not a physics problem at all. It is a plumbing problem. Before you can lean on simulations to learn physics, you need a dependable, repeatable way to *make* them. The tool is Claude, or any capable language model. The method is the four-move prompt. And the repeatability — the part that keeps you from reinventing everything every week — comes from three small Markdown files you write once and drag along through the whole book. That is the workbench we are bolting together today.

When you close this chapter, a file called `00-wave-packet.html` will be sitting on your machine. Open it. Press play. Watch the blue blob drift. Watch the orange ripples slip backward through it. Sit with the mild bewilderment of seeing something you cannot yet name. That feeling is the right feeling. Chapter 1 will give it a name.

---

## The Brutalist System — Three Files, One Apparatus

The word *Brutalist* comes from architecture, where it names a style that refuses to hide its bones — it shows you the raw concrete, the steel, the pipes, instead of dressing them up. We are borrowing the attitude, not the buildings. In this book the rules that govern the simulations live in plain Markdown files you can open, read, edit, and version-control. Nothing is hidden inside a tool you do not own and cannot inspect. That is the whole idea: expose the structure.
...
**SVG, not Canvas.** Think about what we are actually drawing. A one-dimensional wave function is a curve. A probability density is a curve. A potential is a curve. SVG draws curves the way the format was meant to — one `<path>` element per curve, a clean vector description that scales without going to mush. Canvas hands you a bucket of pixels and makes you rebuild the axes by hand. D3 v7 is built around SVG, so the match is natural and, more to the point, the resulting code stays readable.

**Functions, not classes.** Here is the shape of the thing. The simulation core is, at heart, a pure function: hand it parameters, get back an array of values — `computeWavefunction(x, t, params)`. Wrap that in a class and you bury the structure and invite a pile of lifecycle questions (when do you instantiate it, when do you tear it down?). A plain function has nothing to hide. It is easier to read, easier to change, easier to debug.
...
`DESIGN.md` governs what the simulation *looks like*: colors, fonts, axis labels, slider styling, colormap choices. Why split it off from `CLAUDE.md`? Because looks and behavior break in completely different ways, and you want to know which kind of break you are staring at. If you decide the palette is ugly, you edit `DESIGN.md` and only `DESIGN.md` — the physics never feels a thing.
...
Students forget this file exists. Don't — and here is the reason, because it is not obvious. Claude has no memory from one session to the next. Come back in three weeks to extend your Chapter 3 simulation and the model has forgotten everything: every parameter, every convention, every fact you painstakingly verified. `PROJECT.md` *is* that memory, written down where it survives. Without it, every extension prompt starts from a blank slate. With it, the extension prompt is five lines.

### Why Three Files, Not One

The obvious objection: why not just throw it all into one big `CLAUDE.md` and be done?

Token economy is the first answer. By the time you reach Chapter 7, a simulation request has no business forcing the model to re-read every rule about slider styling. So the Chapter 7 prompt loads `CLAUDE.md` (behavior) and `PROJECT.md` (state), and merely *names* `DESIGN.md` without quoting it. The visual rules are still binding — they are simply not burning up your context window for no reason.

The same split pays you back when something breaks. The simulation looks wrong: ask yourself, is this a *behavior* bug (wrong physics, wrong animation) or a *looks* bug (wrong color, wrong layout)? Answer that, and you already know which file to open. So the three-file split is as much a debugging tool as a budgeting one.

**The "bureaucracy" misconception.** I can hear the objection that this is a lot of ceremony for some sliders. It isn't. The files are the entire reason every chapter's simulation comes out consistent and debuggable. Skip them and you will rediscover the same conventions every single week and end up with a dozen simulations that look like they came from a dozen different people. The files write the rules down once so you can stop arguing with yourself about them.

---

## The Four-Move Prompt — Show, Say, Constrain, Verify

Here is the mental shift that makes prompting work: a prompt to Claude is a *specification*, not a wish. You are not asking nicely and hoping. You are stating, testably, what you want. The four-move structure below is the one this book uses for every LLM Exercise. It is short, it is checkable, and it is why most chapter simulations come out right on the first or second try.
...
Read it as two ingredients glued together. The Gaussian envelope $\exp(-x^2/(2a^2))$ is what pins the particle near $x = 0$, with a width set by $a$. The plane-wave carrier $\exp(i k_0 x)$ is what sets the packet moving — it stamps in a central wavenumber $k_0$, which means a central momentum $p_0 = \hbar k_0$. The prefactor out front is just bookkeeping: it makes the whole thing normalize, so that $\int |\psi(x,0)|^2\,dx = 1$.
...
Look hard at that relation, because the spreading is hiding inside it. It is *quadratic* in $k$ — and that means different wavenumbers march forward at different rates. A packet is a chorus of many wavenumbers; if they all kept step, the shape would hold, but they don't, so the shape comes apart. The packet spreads. The closed-form result (Griffiths §2.4, Problem 2.21) is:
...
And notice what that last equality is telling you: $p_0/m$ is just the old classical velocity, momentum over mass. So the center of the quantum blob obeys Newton. That is a relief, and it is no accident — we will come back to why.
...
So here is the strange part, and I want you to actually look at it on the screen. The envelope — the blob, the thing that carries the particle — moves at $v_g$. The crests *inside* the envelope move at half that, $v_p = v_g/2$. Sit in the rest frame of the blob and the crests appear to crawl *backward*: born at the leading edge, sliding back through, dying at the trailing edge. Now this is unsettling. How can the insides run backward while the whole thing moves forward? Because the crests are not matter. They are phase — pure bookkeeping about the oscillation pattern. The blob carries the momentum; the crests carry nothing.
...
**The lesson.** On these scales, a quantum particle smears out *fast*. Swap the electron for a proton of the same width and it spreads $m_p/m_e \approx 1836$ times more slowly — its doubling time stretches out to roughly 27 ps. Try a marble and the number runs longer than the age of the universe. So spreading is not some universal feature you would notice everywhere; it is only visible for things that are light and small, where the action ($mv \cdot \Delta x$) is in the same league as $\hbar$. That comparison, action against $\hbar$, is the whole tell for when quantum mechanics shows its hand.

**The dead end.** You might be tempted to skip the analytic formula and just step the wave function forward numerically with explicit Euler. Resist it. Explicit Euler is not a unitary integrator — which is a fancy way of saying it leaks probability — and the normalization drifts off 1 within a few dozen frames. The normalization indicator catches it instantly, which is precisely why `CLAUDE.md` forbids explicit Euler. The analytic form is both exact and cheap; there is nothing to gain by doing it the hard, wrong way.

**The limit.** And note the fine print: every word of this assumes a *free* particle. Put it in a potential and the packet does something else entirely — bounces, tunnels, splits. That is Chapter 5 onward.

---

## Common Misconceptions

**"Phase velocity is the speed of the particle."** It is tempting, and it is wrong. Phase velocity belongs to a single, non-normalizable plane wave — an idealization, not a real particle. A real, localizable particle moves at the group velocity $v_g = p/m$. The simulation hands you the proof for free: watch the orange ripples slip *backward* through the forward-drifting blob. If phase velocity were "the speed," the particle would appear to travel backward while its probability cloud sailed forward. That is nonsense. The ripples are bookkeeping about the oscillation pattern; they haul no momentum.

**"The Brutalist files are bureaucracy."** Strip away `CLAUDE.md` and every prompt cheerfully rediscovers the same failures — wrong dispersion relation, Canvas instead of SVG, explicit Euler, a dropped imaginary part. The file is a written-down memory of every way this has gone wrong before. Strip away `PROJECT.md` and you lose the parameter conventions halfway through the semester and burn thirty minutes rebuilding context you already had. The files are the *opposite* of bureaucracy: they are the reason one prompt works where seven would have failed.

**"A description is as good as the equation."** It is not, and the gap is enormous. "A Gaussian wave packet evolving in time" leaves the model free to grab the wrong dispersion relation, drop the $\gamma(t)$ factor, set $\mathrm{Im}\,\psi = 0$, or compute $|\psi|^2$ from a real function. Paste the actual closed-form complex equation and you turn a seven-try slog into a one-try success. Show the artifact.
